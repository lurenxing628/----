...
function SMOverview({ report, source, onTab }) {
  ...
  const items = [
    { tab: 'backups', icon: 'folder-open', title: '备份与恢复', action: '查看备份与恢复',
    ...
    { tab: 'config', icon: 'square-pen', title: '自动维护规则', action: '查看自动维护规则',
  ...
  return <div className="sm-overview-layout">
    <section className="sm-section sm-maintenance">
      <div className="sm-section-head"><h3>{source === 'sample' ? '管理样例 · 待处理事项' : '本机维护事项'}</h3><span className="sm-meta">备份、日志与维护规则</span></div>
...
function SMFilters({ kind, filters, onChange, disabled }) {
  ...
  return <div className="sm-filters">
    ...
    {kind === 'logs' && select('level', '级别', ['ERROR', 'WARNING', 'INFO'].map(key => [key, model.LEVELS[key] || key]))}
    {kind === 'logs' && select('file', '来源', ['aps_error.log', 'aps.log', 'launcher.log', 'OperationLogs'].map(key => [key, model.SOURCES[key] || key]))}
...
function SMRecordDetail({ row, onClose, kind }) {
  ...
  return <section id="sm-record-detail" className="sm-detail" tabIndex="-1" ref={ref} aria-label="管理样例详情" onKeyDown={event => { if (event.key === 'Escape') onClose(); }}>
    ...
    <div className="sm-detail-meta"><time>{row.time}</time><SMStatus state={row.status} /><span>{model.SOURCES[row.file] || row.file || row.filename || '未生成文件'}</span></div>
    <h4>{row.summary}</h4><pre>{row.body}</pre>
    {kind === 'backups' && <details className="sm-rules"><summary>恢复要做的检查</summary><p>正式恢复会先存一份恢复前副本，再检查恢复后的数据表；恢复失败和还原失败分别留记录。这里显示的是样例数据，不会真的执行恢复。</p></details>}
  </section>;
}
...
function SMWorkbench({ theme, onToggleTheme, onSetTheme }) {
  ...
  const exportFile = (name, mime, content, message) => {
    ...
    catch (_) { setNotice({ tone: 'danger', text: '导出没有完成，本机数据没有任何改动。请用 Chrome 打开后重试。' }); }
  };
  const onTab = next => { setTab(next); setNotice(null); document.getElementById('sm-tab-' + next).focus(); };
  const exportLogs = (filters, count) => exportFile('管理样例日志.csv', 'text/csv;charset=utf-8', model.sampleLogCSV(filters), '已生成 ' + count + ' 条筛选后的管理样例日志 CSV（含所有分页）');
  ...
  return <div className={'sm-workbench' + (compact ? ' sm-compact' : '')} data-source={source}>
    <header className="sm-header"><div><h2>系统管理</h2><p>本机备份恢复、日志与自动维护</p></div>
      <div className="sm-actions"><ControlButton className="sm-button sm-icon-button" size="sm" title="重新检查" aria-label="重新检查" onClick={() => { recheck(); setNotice({ tone: 'notice', text: '已重新检查当前页面依赖，没有读取本机数据。' }); }}><SMIcon name="refresh-cw" /></ControlButton>
        <SMExport disabled={!downloadReady} onClick={() => { const current = recheck(); exportFile('当前原型诊断.json', 'application/json;charset=utf-8', model.diagnosticJSON(current), '已生成当前原型诊断 JSON（不含样例与正式日志）'); }}>导出当前诊断 JSON</SMExport></div>
    </header>
    ...
    <MetricStrip columns={4} className="sm-metrics"><Metric label="当前页面检查" value={localReady === null ? '待检查' : localReady + ' / ' + report.checks.length} helper="仅页面依赖与资源" />
      <Metric label="本机数据读取" value="未连接" helper="未发起本机状态读取" /><Metric label="数据库状态" value="未知" helper="本原型不读取本机数据" /><Metric label="备份健康" value="未知" helper="没有本机校验结果" /></MetricStrip>
...
function SystemManagementScreen(props) {
  ...
  if (!window.APSSystemWorkbench || !ui || !ds || !['MetricStrip', 'Metric', 'DataTable', 'TransferButton', 'ControlButton'].every(key => typeof ui[key] === 'function') || typeof ds.Table !== 'function' || typeof ds.Button !== 'function') return <section className="sm-workbench"><h2>系统管理</h2><p role="alert">页面没有加载完整，系统管理暂时不可用。请刷新页面后重试。</p></section>;

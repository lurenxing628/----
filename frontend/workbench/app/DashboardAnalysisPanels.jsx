(function () {
  ...
  function Delivery({ data, selected, onSelect, onCompare }) { return <>
    ...
    <section aria-label="影响批次"><div className="dy-heading"><h3>影响批次</h3><Button reasonDisplay="inline" icon="git-compare-arrows" onClick={onCompare}>对比调整方案</Button></div>
  ...
  function Downtime({ data, selected, onSelect }) { return <>
    ...
    <section aria-label="直接重叠工序"><h3>直接重叠的工序</h3><div className="dy-scroll"><table className="dy-analysis-table"><caption className="wb-sr-only">直接重叠的工序</caption><thead><tr><th scope="col">批次 / 工序</th><th scope="col">原计划开始</th><th scope="col">原计划结束</th><th scope="col">重叠小时</th></tr></thead>
      ...
      {!data.overlaps.length && <window.WorkbenchListControls.EmptyState kind="empty" title="当前读取范围未确认直接重叠，来源异常仍单独列示。" />}
    </section>
  ...
  function Material({ data }) {
    ...
    return <section aria-label="待排批次与齐套日期"><h3>待排批次与齐套日期</h3><div className="dy-context">待排 {value(p.count)} 批 · 已读取 {p.known_count} 批</div><Issues issues={p.issues} />
      ...
      {!p.items.length && <window.WorkbenchListControls.EmptyState kind="empty" title={p.count === 0 ? '当前没有待排批次。' : '待排批次来源未能完整读取。'} />}
  ...
  function Actual({ data, navigate }) { return <section aria-label="工序执行偏差"><h3>工序执行事实与定额对照</h3><div className="dy-scroll"><table className="dy-analysis-table"><caption className="wb-sr-only">工序执行偏差</caption>
    <thead><tr><th scope="col">批次 / 工序</th><th scope="col">定额加工小时</th><th scope="col">有效加工小时</th><th scope="col">超耗判断</th><th scope="col">现场记录</th></tr></thead><tbody>{data.execution.map(row => {
      ...
      return <tr key={source.task_ref}><td>{row.subject}</td><td>{value(hours.quota_processing_hours)} h</td><td>{value(hours.effective_processing_hours)} h</td>
        <td>{hours.overrun === true ? '已确认超耗' : hours.overrun === false ? '未超耗' : '无法评估'}</td><td><Button reasonDisplay="inline" icon="square-pen" onClick={() => navigate({ view: 'field', context, enabled: true })}>现场报工</Button>

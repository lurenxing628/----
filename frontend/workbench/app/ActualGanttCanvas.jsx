(function () {
  ...
  function DenseRow({ row, model, width, viewport, left, selected, onSelect, onHover, renderMark }) {
    ...
    function hit(event) {
      ...
      return marks.find(mark => { const start = (mark.start - model.start) * scale - left, end = (mark.end - model.start) * scale - left;
    ...
    return <><canvas ref={ref} className="fg-row-canvas" data-actual-canvas role="button" tabIndex={0} style={{ left, width: viewport }} aria-label={M.describe(row.item, model.labels).join('；')}
      onClick={e => { const mark = hit(e); if (mark) onSelect(row.item, mark.report); }}
      ...
      onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onSelect(row.item, row.reports[0]); } }} />

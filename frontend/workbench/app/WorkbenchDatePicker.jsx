(function () {
  'use strict';
  const M = window.WorkbenchDatePickerModel, C = window.APSCalendarContract;
  const { Icon } = window.ResourceControls;
  const weekdays = ['一', '二', '三', '四', '五', '六', '日'];
  function NavButton({ name, icon, ...props }) {
    return <button {...props} type="button" className="btn wb-picker-nav" title={name} aria-label={name}><Icon name={icon} /></button>;
  }
  function NumberField({ label, value, maximum, width = 2, onChange, initial }) {
    const id = React.useId();
    const invalid = value !== '' && Number(value) > maximum;
    function change(next) { if (new RegExp('^\\d{0,' + width + '}$').test(next)) onChange(next); }
  ...
  function DateGrid({ view, cursor, selected, props, root, onBrowse, onChoose }) {
    const first = view.slice(0, 7) + '-01', leading = M.weekday(first), count = C.monthDays(Number(view.slice(0, 4)), Number(view.slice(5, 7)));
    ...
    function keydown(event, day) {
      const offsets = { ArrowLeft: -1, ArrowRight: 1, ArrowUp: -7, ArrowDown: 7, Home: -M.weekday(day), End: 6 - M.weekday(day) };
      let next;
      if (Object.prototype.hasOwnProperty.call(offsets, event.key)) next = M.moveDay(day, offsets[event.key]);
      else if (['PageUp', 'PageDown'].includes(event.key)) next = M.moveMonth(day, (event.key === 'PageUp' ? -1 : 1) * (event.shiftKey ? 12 : 1));
      else if (['Enter', ' '].includes(event.key)) { event.preventDefault(); event.stopPropagation(); onChoose(day); return; }
      else return;
    ...
    return <div className="wb-picker-grid" ref={root} role="grid" aria-label={view.slice(0, 4) + ' 年 ' + Number(view.slice(5, 7)) + ' 月'}
      ...
      {Array.from({ length: 6 }, (_, week) => <div role="row" key={week} style={{ display: 'contents' }}>
        {dates.slice(week * 7, week * 7 + 7).map((day, index) => {
          ...
          return <button key={day} type="button" role="gridcell" data-date={day} data-outside={outside || undefined}
            className={'wb-picker-day' + (outside ? ' is-outside' : '') + (day === selected ? ' is-selected' : '')}
            aria-label={day} title={day + (allowed ? '' : '（不可选）')} aria-selected={day === selected} aria-disabled={!allowed}
            aria-current={day === M.today() ? 'date' : undefined} tabIndex={day === cursor ? 0 : -1} data-picker-initial={day === cursor || undefined}
            onFocus={() => { if (day !== cursor) onBrowse(day, false, true); }} onKeyDown={event => keydown(event, day)}
            onClick={() => { if (allowed) onChoose(day); }}>{Number(day.slice(8))}</button>;
        })}</div>)}
    </div>;
  }
  function MonthGrid({ view, cursor, selected, props, root, onBrowse, onChoose, browseMonth }) {
    const year = view.slice(0, 4), selectedMonth = selected.slice(0, 7), currentMonth = M.today().slice(0, 7);
    const availability = React.useMemo(() => Array.from({ length: 12 }, (_, index) => M.monthSelectable(props, year + '-' + M.pad(index + 1))),
      [year, props.type, props.value, props.valueAttribute, props.min, props.max, props.step]);
    function keydown(event, key) {
      const month = Number(key.slice(5)), offsets = { ArrowLeft: -1, ArrowRight: 1, ArrowUp: -3, ArrowDown: 3,
        Home: -((month - 1) % 3), End: 2 - (month - 1) % 3, PageUp: -12, PageDown: 12 };
      if (Object.prototype.hasOwnProperty.call(offsets, event.key)) {
        ...
        event.preventDefault(); event.stopPropagation(); onBrowse(boundary ? key + '-01' : M.moveMonth(key, offsets[event.key]), true);
      }
      ...
      {Array.from({ length: 4 }, (_, row) => <div role="row" style={{ display: 'contents' }} key={row}>
        {Array.from({ length: 3 }, (_, column) => {
          ...
          return <button type="button" role="gridcell" key={key} data-date={key} data-view-month={key === browseMonth || undefined} className={'wb-picker-day' + (key === selectedMonth ? ' is-selected' : '')}
            aria-label={key} aria-selected={key === selectedMonth} aria-current={key === currentMonth ? 'date' : undefined}
            aria-disabled={!allowed} tabIndex={key === cursor.slice(0, 7) ? 0 : -1} data-picker-initial={key === cursor.slice(0, 7) || undefined}
            title={key + (key === browseMonth ? '（当前浏览）' : '') + (allowed ? '' : '（不可选）')} onKeyDown={event => keydown(event, key)}
            onFocus={() => { if (key !== cursor.slice(0, 7)) onBrowse(key + '-01', false, true); }}
            onClick={() => { if (allowed) onChoose(key); }}>{number} 月</button>;
        })}</div>)}
    </div>;
  }
  function DatePickerBody(props) {
    const { type, value = '', onCommit, onClose, label } = props;
    const initial = M.seed(props), units = M.precision(props), root = React.useRef(null), focus = React.useRef(false);
    const [view, setView] = React.useState(initial), [cursor, setCursor] = React.useState(initial);
    const [pickerView, setPickerView] = React.useState(type === 'month' ? 'months' : type === 'time' ? 'time' : 'days');
    const monthAnchor = React.useRef(initial), showMonths = pickerView === 'months';
    const [year, setYear] = React.useState(initial.slice(0, 4)), [yearError, setYearError] = React.useState('');
    ...
    const [time, setTime] = React.useState(() => M.readTime(value, units)), [edited, setEdited] = React.useState(false);
    const hasDate = type !== 'time', hasTime = ['time', 'datetime-local'].includes(type);
    const clock = M.timeValue(time, units), candidate = type === 'datetime-local' ? selected && clock ? selected + 'T' + clock : '' : type === 'time' ? clock : selected;
    const check = M.validate(props, candidate), id = React.useId();
    ...
    function browse(next, shouldFocus, keepView) {
      focus.current = shouldFocus; setCursor(next);
      if (!keepView) { setView(next); setYear(next.slice(0, 4)); setYearError(''); }
    }
    function choose(next) {
      if (hasDate && (!/^\d{1,4}$/.test(year) || Number(year) < 1 || M.pad(Number(year), 4) !== view.slice(0, 4))) { applyYear(); return; }
      const allowed = type === 'month' ? M.validate(props, next).valid : M.daySelectable(props, next);
      if (!allowed) return;
      setEdited(true); setSelected(next);
      if (type !== 'month') setPickerView('days');
      if (type === 'datetime-local') browse(next, false);
      else onCommit(M.validate(props, next).value);
    }
    function chooseMonth(key) {
      if (type === 'month') { choose(key); return; }
      if (!yearApplied) { applyYear(); return; }
      if (!M.monthSelectable(props, key)) return;
      const [targetYear, targetMonth] = M.monthParts(key), targetDay = Math.min(Number(monthAnchor.current.slice(8)), C.monthDays(targetYear, targetMonth));
      setPickerView('days'); browse(M.dateKey(targetYear, targetMonth, targetDay), true);
    }
    function toggleMonths() {
      const anchor = yearApplied ? cursor : applyYear();
      if (!anchor) return;
      if (showMonths) { setPickerView('days'); browse(monthAnchor.current, true); }
      else { monthAnchor.current = anchor; setPickerView('months'); browse(anchor, true); }
    }
    function applyYear() {
      if (!/^\d{1,4}$/.test(year) || Number(year) < 1 || Number(year) > 9999) { setYearError('年份须为 1 至 9999。'); return; }
      const month = Number(view.slice(5, 7)), day = Math.min(Number(cursor.slice(8)), C.monthDays(Number(year), month));
      const next = M.dateKey(Number(year), month, day);
      browse(next, false); return next;
    }
  ...
  function WorkbenchDatePicker(props) {
    return <DatePickerBody key={JSON.stringify([props.type, props.value, props.valueAttribute, props.min, props.max, props.step])} {...props} />;
  }

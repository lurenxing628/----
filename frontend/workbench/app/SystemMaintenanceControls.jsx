(function () {
  ...
  function Button({ children, icon, className = 'btn', ...props }) {
    ...
    return <window.ResourceControls.Button {...props} icon={icon === 'rotate-ccw' ? undefined : icon === 'save' ? 'check' : icon === 'trash-2' ? 'x' : icon}
      ...
      className={className + ' sm-button' + (children ? '' : ' sm-icon-button')}>{icon === 'rotate-ccw' && <SMIcon name="rotate-ccw" />}{children}</window.ResourceControls.Button>;
  }
  function Styles() {
    return <style>{`.sm-maintenance-workspace .sm-field,.sm-maintenance-workspace .sm-meta,.sm-maintenance-workspace .sm-note{font-size:13px}
      ...
      @media(max-width:800px){.sm-maintenance-workspace .sm-filters{grid-template-columns:repeat(2,minmax(0,1fr))}}`}</style>;
  }
  function ErrorBox({ error }) {
    return error ? <p className="sm-notice sm-error" role="alert">{error.message || String(error)}{error.code && <span> · {error.code}</span>}</p> : null;
  }
  ...
  function Confirm({ action, row, reason, onClose, onConfirm }) {
    ...
    return <window.ResourceControls.Modal title={A.actions[action]} icon={action === 'create' ? 'plus' : action === 'delete' ? 'x' : 'history'} onClose={onClose}
      ...
      <div className="modal-b form scroll" style={{ overflowWrap: 'anywhere' }}>
        {row && <p><strong>{row.filename}</strong><br />文件修改时间 {row.time.replace('T', ' ')} · {row.size_bytes} 字节<br />文件存在，尚无本次完整性校验证据。</p>}
  ...
  function Outcome({ command }) {
    ...
    return <section className="sm-section sm-maintenance-outcome" aria-label="维护原请求结果" style={{ padding: '12px 20px', background: 'var(--ui-card-bg)', borderBottom: '1px solid var(--ui-border)', overflowWrap: 'anywhere' }}>
      ...
      {intent && <p className="sm-meta">原请求：{intent.request_key}</p>}
      <ErrorBox error={storageError || error} />
      {busy && <p role="status">正在等待维护结果，未重新提交。</p>}
      {op ? <div role="status"><p><strong className={op.state === 'succeeded' ? 'sm-tone-success' : 'sm-tone-warning'}>{labels[op.state]}</strong> · {op.message} · {op.code}</p>
        <p>外置维护记录：{op.job_ref} · {op.updated_at.replace('T', ' ')}<br />业务审计：{op.audit_persisted ? '已留存' : '未确认留存'}{op.replayed ? ' · 查询原结果' : ''}</p>
        {op.filename && <p>目标文件：{op.filename}</p>}{op.protection_filename && <p>保护副本：{op.protection_filename}</p>}
        <details className="sm-rules"><summary>维护阶段</summary>{op.history.map((step, index) => <p key={index}>{step.time.replace('T', ' ')} · {labels[step.state]}</p>)}</details>
        ...
        <p>配置回执：{result.command.receipt_ref}{result.command.replayed ? ' · 查询原结果' : ''}</p></div> : result && result.kind === 'rejected' ? <p role="status">后端明确拒绝，本次未提交。核对错误后可确认结果。</p> : null}
  ...
  function Preferences({ theme, onSetTheme, pageSize, onPageSize, compact, onCompact }) {
    return <section className="sm-section sm-preferences"><div className="sm-section-head"><h3>页面偏好</h3></div><div className="sm-session-settings">
      ...
      <label className="sm-inline-label"><input type="checkbox" checked={compact} onChange={event => onCompact(event.target.checked)} />紧凑行距</label>
    </div></section>;
  }

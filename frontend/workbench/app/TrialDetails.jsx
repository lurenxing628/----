(function () {
  ...
  function References({ task }) {
    return <details className="tt-refs"><summary>永久引用与原始依据</summary><dl>{[['任务', task.task_ref], ['行', task.row_ref], ['原任务', task.source_task_ref], ['原行', task.source_row_ref],
  ...
  function Editor({ data, task, commands, onEditing, onRecheck }) {
    ...
    function open() { setForm({ machine_ref: external ? null : task.machine_ref, operator_ref: external ? null : task.operator_ref, start: task.start }); setEditing(true); onEditing(true); setReviewed(true); }
    ...
    return <><U.Issues rows={task.edit_context.blocked_reasons} />{!editing ? <U.Button icon="square-pen" disabled={!editable || commands.blocked} onClick={open}>调整此工序</U.Button> :
      <form onSubmit={submit} className="tt-editor"><h4>调整工序</h4>{['machine', 'operator'].map((kind, i) => <label key={kind}>{i ? '调整人员' : '调整设备'}
        ...
        <div className="tt-tools"><U.Button icon="check" type="submit" disabled={!editable || !reviewed || commands.blocked}>保存调整</U.Button>
          <U.Button icon="x" disabled={commands.busy} onClick={close}>取消编辑</U.Button><U.Button icon="refresh-cw" disabled={commands.busy || !!commands.key} onClick={onRecheck}>重读工序</U.Button></div>
      </form>}</>;
  }
  function Detail({ data, selected, commands, onSelect, onEditing, onRecheck }) {
    ...
    return <aside className="tt-detail" aria-label="工序详情"><div className="tt-heading"><h3>工序详情</h3><U.Button icon="x" aria-label="关闭工序详情" onClick={() => onSelect(null)} /></div>
      ...
      <Editor key={task.task_ref} {...{ data, task, commands, onEditing, onRecheck }} />

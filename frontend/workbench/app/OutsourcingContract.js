(function () {
  ...
  function check(ok, message = '外协读到的数据不完整，请刷新后重试。') { if (!ok) throw new Error(message); }
  ...
  function stamp(v) { const s = typeof v === 'string' && v.length === 16 ? v + ':00' : v; check(time(s), '请按 2026-09-13 08:30 这样填写日期和时间。'); return s; }
  ...
  function target(t, publicData = false) {
    check(object(t) && ['single', 'merged'].includes(t.kind) && ref(t.batch_ref) && ref(t.supplier_ref) && Array.isArray(t.operation_refs)
      ...
      && (t.kind === 'single') === (t.operation_refs.length === 1), '单工序要选 1 道，合并发出至少选 2 道，而且不能重复。');
    if (publicData) {
  ...
  function query(kind, value = {}) {
    ...
    check(object(value) && Object.keys(q).every(k => allowed.includes(k)) && Number.isSafeInteger(q.page) && q.page >= 1 && q.page <= 1000000
      ...
      && (q.page === 1 || text(q.snapshot_ref)), '外协的筛选或翻页条件无效，当前范围没有变化。请重新选择。'); return q;
  }
  function envelope(v, q, preview = false) {
    ...
    if (q && q.snapshot_ref) check(q.snapshot_ref === v.meta.snapshot_ref, '数据已更新，请刷新后重试。刚才的选择已保留。'); return v.data;
  }
  function page(p, rows, q) { check(object(p) && p.number === q.page && p.size === q.size && count(p.total) && p.pages === Math.ceil(p.total / p.size)
    && Array.isArray(rows) && rows.length === Math.min(p.size, Math.max(0, p.total - (p.number - 1) * p.size)), '读到的页码和数量与当前范围不一致，请刷新后重试。'); }
  function catalog(v, kind, q, selected) {
    ...
    if (kind === 'targets') {
      ...
      row(d.item); check(d.item.outsourcing_ref === selected, '详情或历史和所选登记对不上，页面没有切换。请刷新后重试。');
      if (kind === 'history') {
  ...
  function receipt(v, intent) {
    ...
    check(v && v.ok === true && v.result === 'committed' && /^[a-f0-9]{32}$/.test(v.receipt_ref) && typeof v.replayed === 'boolean' && Array.isArray(v.warnings)
      ...
      && text(d.local_operator) && time(d.confirmed_at) && d.refresh_required === true, '保存结果和这条外协登记对不上，结果还不确定。请点「查询结果」，不要重复提交。');

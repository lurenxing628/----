(function () {
  'use strict';
  const C = window.APSResourceContract, object = C.object;
  ...
  function operation(row) {
    return object(row) && ref(row.ref) && row.operation_ref === row.ref && text(row.business_code) && text(row.label)
      && (Number.isSafeInteger(row.sequence) || text(row.sequence)) && nullableText(row.source) && nullableText(row.status)
      && typeof row.completed === 'boolean' && row.completed === (row.status === 'completed')
      && ['setup_hours', 'unit_hours', 'external_days'].every(key => finite(row[key]))
      && ['machine_ref', 'operator_ref', 'supplier_ref', 'op_type_ref'].every(key => row[key] === null || ref(row[key]))
      && object(row.resources) && (row.external_group === null || object(row.external_group) && ref(row.external_group.ref) && finite(row.external_group.total_days))
      && issueRows(row.issues) && typeof row.editable === 'boolean';
  }
  function entity(row) {
    return C.entity(row) && ref(row.ref) && context(row.write_context) && object(row.fields)
      && finite(row.fields.quantity) && fields.slice(1).every(key => nullableText(row.fields[key]))
      && ref(row.relationships.part_ref) && text(row.relationships.part_no) && text(row.relationships.part_name)
      && ['operation_count', 'completed_count', 'gap_count', 'plan_reference_count', 'execution_reference_count', 'material_requirement_count'].every(key => count(row.relationships[key]))
      && Array.isArray(row.operations) && row.operations.every(operation) && new Set(row.operations.map(op => op.ref)).size === row.operations.length
      && row.relationships.operation_count === row.operations.length && row.relationships.completed_count === row.operations.filter(op => op.completed).length
      && row.all_operations_complete === (!!row.operations.length && row.operations.every(op => op.completed))
      && typeof row.protected === 'boolean' && issueRows(row.issues);
  }
  function list(result, scope) {
    C.query(result, 'list'); const d = result.data, p = d.page;
    if (!d.entities.every(entity) || !context(d.create_context) || !d.create_context || p.number !== scope.page || p.size !== scope.size
        || p.pages !== Math.max(1, Math.ceil(p.total / p.size)) || p.sort.length !== 1 || p.sort[0].field !== scope.sort || p.sort[0].direction !== scope.direction
        || scope.snapshot_ref && result.meta.snapshot_ref !== scope.snapshot_ref) throw C.failure('批次列表字段或范围不完整，请重新读取。');
    return result;
  }
  function detail(result, expected) {
    ...
    if (!entity(d) || d.ref !== expected || !object(d.materials) || !Array.isArray(d.materials.requirements)
        ...
        || !object(d.template)) throw C.failure('批次详情与实际对象不一致或字段不完整。');
    return result;
  }
  function receipt(result, action, expected) {
    if (C.receipt(result) !== 'terminal') throw C.failure('批次保存结果尚未核实。');
    const data = result.data;
    const valid = ['bulk_confirm', 'import_confirm'].includes(action) ? Array.isArray(data.items) && data.items.length === data.count && data.items.every(row => ref(row.entity_ref) && ['committed', 'unchanged', 'skipped'].includes(row.result))
      : ref(data.entity_ref) && (action === 'create' || data.entity_ref === expected);
    if (!valid) { const error = C.failure('回执与批次对象不一致，结果待核实。'); error.committed = 'unknown'; throw error; }
    return result;
  }
  function preview(result, action, expectedRef, input) {
    const data = result && result.data, name = 'batch.' + action + '_confirm';
  ...
  function draft(entity) {
    return entity ? Object.fromEntries(fields.map(key => [key, entity.fields[key] == null ? '' : String(entity.fields[key])]))
      : { business_code: '', part_ref: '', quantity: '', due_date: '', priority: 'normal', ready_status: 'yes', ready_date: '', remark: '' };
  }
  function input(value, original) {
    ...
    if (original) return { fields: patch };
    return { business_code: value.business_code.trim(), part_ref: value.part_ref, fields: patch };
  }
  function inputErrors(value, original) {
    const errors = [];
    if (!original) {
      if (!value.business_code.trim()) errors.push({ path: 'business_code', message: '请填写批次号。' });
      if (!ref(value.part_ref)) errors.push({ path: 'part_ref', message: '请选择图号。' });
    }
    for (const key of fields) {
      if (original && String(original.fields[key] == null ? '' : original.fields[key]) === value[key]) continue;
      const next = value[key];
      if (key === 'quantity' && (!/^[0-9]+$/.test(next) || !Number.isSafeInteger(Number(next)) || Number(next) <= 0))
        errors.push({ path: 'fields.quantity', message: '数量必须是正整数。' });
      if (key.endsWith('_date') && next !== '' && !/^\d{4}-\d{2}-\d{2}$/.test(next))
        errors.push({ path: 'fields.' + key, message: '日期格式应为 YYYY-MM-DD。' });
    }
    return errors;
  }

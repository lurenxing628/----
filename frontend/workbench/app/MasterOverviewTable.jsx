(function () {
  ...
  function Pager({ page, onPage, onSize, disabled, detail = false }) {
    return <window.WorkbenchListControls.Pager page={page} sizes={detail ? [10] : [20, 50, 100]} onPage={onPage} onSize={onSize} disabled={disabled} unit="条" label={detail ? '详情' : '主数据'} />;
  }
  function Table({ data, selected, onSelect, onMaintain, onFilter, onClear, onRetry, loading, error, navigation }) {
    ...
    return <div className="wb-table-frame" data-sticky-head data-sticky-actions><table className="wb-table mo-table" aria-label="主数据清单" aria-busy={loading}
      style={{ minWidth: columns.reduce((sum, item) => sum + item[2], 64) }}><caption className="wb-sr-only">主数据清单</caption><colgroup>{columns.map(([key, , width]) => <col key={key} style={{ width }} />)}<col style={{ width: 64 }} /></colgroup>
      ...
      <tbody>{rows.map(item => <tr key={item.key} aria-selected={!!selected && selected.key === item.key} data-master-ref={item.entity_ref || item.ref}>{columns.map(([key]) => <td key={key} className={key === 'business_code' ? 'wb-col-key' : ''}>
        ...
        <td className="wb-col-actions"><Button className="btn mo-icon" icon="arrow-right" aria-label={'维护 ' + item.business_code} reasonDisplay="inline" reason={item.target.unavailable_reason || (!navigation ? '维护导航尚未接入。' : '')} disabled={loading} onClick={() => onMaintain(item.target)} /></td></tr>)}</tbody></table>
      {!rows.length && <window.WorkbenchListControls.EmptyState kind={loading ? 'loading' : error ? 'error' : filtered ? 'filtered' : 'empty'} title={loading ? '正在读取主数据' : error ? '主数据读取失败' : '当前范围没有记录'} action={error ? <Button onClick={onRetry}>重试读取主数据</Button> : filtered ? <Button onClick={onClear}>清除筛选并查看清单</Button> : null} />}</div>;
  }

(function () {
  'use strict';
  const C = window.APSResourceContract, P = window.APSProcessContract, M = window.ResourceTableFilterModel;
  const ref = value => typeof value === 'string' && /^[0-9a-f]{48}$/.test(value);
  const token = value => typeof value === 'string' && /^[A-Za-z0-9_-]{32}$/.test(value);
  const fields = { business_code: '图号', label: '零件名称', route_raw: '原始路线', route_parsed: '路线解析标记', remark: '备注', operation_count: '工序数量', external_group_count: '外协组数量' };
  ...
  function scope(value = {}) {
    if (!C.object(value) || value.query !== undefined && typeof value.query !== 'string'
        || value.stage !== undefined && value.stage !== null && value.stage !== '' && !['route', 'source', 'hours', 'ready'].includes(value.stage)) throw C.failure('工艺列表范围不完整。');
    const result = { query: value.query || '', sort: P.ordering(value), column_filters: filters(value.column_filters) };
    if (value.stage) result.stage = value.stage;
    return result;
  }
  function facetScope(value, column) {
    const result = scope(value); delete result.sort; delete result.column_filters[column];
    return result;
  }
  function selection(refs, allowEmpty = false) {
    if (!Array.isArray(refs) || !allowEmpty && !refs.length || !refs.every(ref) || new Set(refs).size !== refs.length)
      throw C.failure('请重新选择零件；不能用图号代替原零件记录。');
    return refs.slice();
  }
  ...
  function createInput(draft) {
    ...
    ['business_code', 'label'].forEach(key => {
      ...
      if (!value[key]) errors.push({ path: key, message: key === 'business_code' ? '请填写图号。' : '请填写零件名称。' });
    });
    ['route_raw', 'remark'].forEach(key => { if (typeof draft[key] !== 'string') errors.push({ path: key, message: '请填写文字或留空。' }); else if (draft[key] !== '') value[key] = draft[key]; });
  ...
  function deletePreview(raw, request) {
    const expected = selection(request.refs), result = window.APSResourceMaterial.create('part', '零件').preview(raw, 'bulk', expected);
    const d = result.data;
    if (!C.object(d.scope) || M.signature(scope(d.scope)) !== M.signature(scope(request.scope))
        || !Array.isArray(d.columns) || !d.columns.length || !d.columns.every(row => C.object(row) && C.own(fields, row.key) && typeof row.label === 'string' && row.label.trim())
        || new Set(d.columns.map(row => row.key)).size !== d.columns.length) throw C.failure('删除预检与原筛选范围或业务列不一致，请重新预检。');
    if (result.data.rows.some(row => row.action !== 'delete' || !['delete', 'rejected'].includes(row.result))) throw C.failure('删除预检含有其他操作，本批未提交。');
    const validFacts = value => value === null || C.object(value) && Object.keys(value).every(key => C.own(fields, key)
      && (value[key] === null || typeof value[key] === 'string' || Number.isFinite(value[key])));
    if (result.data.rows.some(row => !validFacts(row.before) || row.after !== null || Object.keys(row.changes).length
        || row.before && Object.keys(row.before).some(key => !d.columns.some(column => column.key === key))))
      throw C.failure('删除预检中的原零件资料不完整，本批未提交。');
    return result;
  }

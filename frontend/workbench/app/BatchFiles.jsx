(function () {
  ...
  function BatchFiles({ adapter, mode: operation, scope, selected, snapshot, command, onClose, onCommitted, disabled }) {
    ...
    async function work(action) {
      ...
      try {
        if (action === 'template') saveDownload(await adapter.downloadTemplate(), '批次导入模板.xlsx');
          ...
          if (!data || data.operation !== 'batch.import_confirm' || data.mode !== mode || !Array.isArray(data.rows) || !Array.isArray(data.deleted)
              || typeof data.can_confirm !== 'boolean' || data.can_confirm && (!data.write_context || !B.context(data.write_context))) throw C.failure('读到的导入预检结果不完整，没有写入批次。请再点一次「导入预检」。');
          ...
          const downloaded = await adapter.downloadExport(result.data.export_ref); saveDownload(downloaded, '批次清单.xlsx');
    ...
    return <Modal title={importing ? '批量维护批次' : '导出批次清单'} icon="box" locked={command.locked || busy} onClose={onClose} footer={<>
      ...
      {importing ? !done && <>{!preview ? <Button transfer="import" disabled={locked || !file} onClick={() => work('preview')}>导入预检</Button>

(function () {
  ...
  const labels = { accepted: '已受理', checking: '检查中', protecting: '创建保护副本', restoring: '恢复中', verifying: '校验中',
    rolling_back: '回滚中', succeeded: '已完成', failed: '操作失败', rolled_back: '恢复失败，已回滚',
    rollback_failed: '回滚失败，需人工核查', recovery_required: '结果未知，需人工核查' };
  const origins = { selected_backup: '来自所选备份', protection_backup: '已回滚到恢复前保护副本', unchanged: '原数据库未被替换', unconfirmed: '未知，不能确认当前数据库内容' };
  function check(ok) { if (!ok) throw new Error('恢复状态不完整或相互矛盾，系统保持只读；请保留现场。'); }
  ...
  function download(host, result, error) {
    ...
    link.href = url; link.download = 'aps-restore-maintenance-diagnostic.json';

(function () {
  ...
  function Calendar({ resource }) {
    return <details><summary>时段与班表</summary><div className="tt-subtable"><U.Table label="占用时段" rows={resource.segments} size={10}
      ...
      <h4>可工作时段</h4>{resource.calendar.windows === null ? <p>真实班表不可用</p> : <U.Table label="可工作时段" rows={resource.calendar.windows} size={10}
      ...
      <div role="tabpanel">
        ...
        {tab === 'capacity' && <><p className="tt-muted">仅此试调占用，非全厂利用率。{U.timeLabel(data.capacity.start)} 至 {U.timeLabel(data.capacity.end)}</p>
          {data.capacity.reason && <p className="tt-notice">{data.capacity.reason}</p>}<U.Table label="资源占用" rows={data.capacity.resources} columns={[
            ['资源', r => (r.resource_type === 'machine' ? '设备 ' : '人员 ') + name(r.resource_ref)], ['安排（小时）', r => U.number(r.arranged_hours)], ['实际占用（小时）', r => U.number(r.occupied_hours)],
            ['重叠时间（小时）', r => U.number(r.overlap_hours)], ['可用（小时）', r => U.number(r.available_hours)], ['班表外时间（小时）', r => U.number(r.outside_available_hours)],
            ['本范围占用率', r => r.utilization === null ? '暂无数据' : U.number(r.utilization * 100) + '%'], ['依据', r => <Calendar resource={r} />]]} /></>}
        {tab === 'history' && <><p className="tt-muted">本草稿的每次调整记录，不是正式采用记录。</p><U.Table label="调整记录" rows={data.change_history.slice().reverse()} columns={[

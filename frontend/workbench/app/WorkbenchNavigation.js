(function () {
  ...
  function validateBoot(boot) {
    ...
    check(object(boot.titles) && Array.isArray(boot.enabled_views) && boot.enabled_views.length > 0
      && new Set(boot.enabled_views).size === boot.enabled_views.length
      && boot.enabled_views.every(id => typeof id === 'string' && typeof boot.titles[id] === 'string')
      && Object.keys(boot.titles).every(id => boot.enabled_views.includes(id)));
    ...
    const seen = new Set(), groups = new Set(), usedIcons = new Set();
    boot.nav_groups.forEach(group => {
      check(object(group) && typeof group.title === 'string' && group.title.trim() && !groups.has(group.title)
        && Array.isArray(group.items) && group.items.length > 0);
      groups.add(group.title);
      group.items.forEach(item => {
        check(object(item) && boot.enabled_views.includes(item.id) && !seen.has(item.id)
          && item.label === boot.titles[item.id] && icons.includes(item.icon) && !usedIcons.has(item.icon));
        seen.add(item.id); usedIcons.add(item.icon);
      });
    });
    Object.keys(boot.view_aliases).forEach(id => check(boot.enabled_views.includes(id) && !seen.has(id)
      && seen.has(boot.view_aliases[id])));
    check(boot.enabled_views.every(id => seen.has(id) || own(boot.view_aliases, id)));
    check(typeof boot.help_url === 'string' && boot.help_url.startsWith('/') && !boot.help_url.startsWith('//')
      && new URL(boot.help_url, location.origin).origin === location.origin);
  }
  function historyView(page) {
    const context = page.context || {};
    return ['analysis', 'gantt', 'delay'].includes(page.view) && context.source === 'run_history'
      && !own(context, 'run_ref') && !own(context, 'candidate_ref');
  }
  ...
  function helpUrl(boot, page) {
    ...
    const url = new URL(boot.help_url, location.origin); url.searchParams.set('src', href(boot, page.view));
    return url.pathname + url.search;
  }
  ...
  function restore(page, done) {
    ...
    function applyContainers(main) {
      ...
      scrollContainers(main).forEach(node => {
        ...
        const left = position(saved[0]), top = position(saved[1]);
    ...
    function apply() {
      ...
      const reached = panes && Math.abs(window.scrollY - top) <= 1 && Math.abs(window.scrollX - left) <= 1
  ...
  function guardHistory(boot, { hasDirty, confirmLeave, onRestore, onError }) {
    let accepted, pending = null, disposed = false;
    function capture() { return { page: read(boot), state: history.state, url: location.href }; }
    function sync() { if (!pending) accepted = capture(); }
    sync();
    ...
    function accept() { pending = null; accepted = capture(); onRestore(); }
    function confirm(entry) {
      ...
      Promise.resolve().then(confirmLeave).then(allowed => {
        ...
        entry.phase = 'replay'; history.go(entry.delta);
      }).catch(error => { if (!disposed && pending === entry) recover(error); });
    }
    function pop() {
      if (disposed) return;
      try {
        const target = read(boot), delta = target.key - accepted.page.key;
        if (pending) {
          ...
          check(delta !== 0); history.go(-delta); return;
        }
        if (!hasDirty()) { accept(); return; }
        check(delta !== 0);
        pending = { phase: 'return', delta, target, url: location.href };
        history.go(-delta);
      } catch (error) { recover(error); }
    }
    window.addEventListener('popstate', pop);
    return { sync, busy: () => !!pending, dispose: () => { disposed = true; window.removeEventListener('popstate', pop); } };
  }
  window.WorkbenchNavigation = { read, href, helpUrl, remember, navigate, replaceContext, restore, validateBoot, title, historyView, guardHistory };
})();

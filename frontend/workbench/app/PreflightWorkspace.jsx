(function () {
  ...
  function Details({ data }) {
    ...
    return <details className="pf-detail"><summary>逐工序检查 · {data.tasks.length} 道</summary>
      <div className="pf-results wb-table-frame" data-sticky-head data-sticky-actions><table className="wb-table" aria-label="排产前检查明细"><caption className="wb-visually-hidden">排产前检查明细</caption><colgroup><col style={{ width: '18%' }} /><col style={{ width: '20%' }} /><col style={{ width: '15%' }} /><col /></colgroup>
  ...
  function PreflightWorkspace({ onNavigate, initialContext, renderRunPanel, actions }) {
    ...
    return <div className="plana preflight-workspace" data-preflight-workspace><Styles />
      ...
      <ol className="pf-stepper" aria-label="执行排产步骤">{['选批次与窗口', '检查', '计算'].map((label, index) => <li key={label} aria-current={currentStep === index + 1 ? 'step' : undefined} data-step-state={currentStep > index + 1 ? 'complete' : currentStep === index + 1 ? 'current' : 'upcoming'}><span aria-hidden="true">{index + 1}</span>{label}</li>)}</ol>
      ...
      <ErrorBox error={error} />{needsRecheck && <p className="pf-recheck" role="status">排产参数已变化，请重新检查后再开始计算。</p>}{busy && <p role="status">正在读取批次、资源及执行事实；未创建运行。</p>}
      {data && <><p className="pf-muted" role="status">检查时间：{window.WorkbenchFormat.dateTime(result.meta.as_of)} · 输入有效至 {window.WorkbenchFormat.dateTime(data.input_expires_at)} · 日历未验证</p>

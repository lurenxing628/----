(function () {
  ...
  const detailFields = {
    skill_details: { op_type_code: '工种编号', skill_level: '技能级别', is_primary: '主要标记', created_at: '创建时间' },
    machine_authorizations: { machine_code: '设备编号', operator_code: '工号', skill_level: '技能级别', is_primary: '主要标记', created_at: '创建时间' }
  };
  ...
  function columns(data) {
    if (!Array.isArray(data.columns) || !data.columns.length || !data.columns.every(item => C.object(item)
      ...
      throw C.failure('预检缺少完整业务列名，本批未提交。');
    ...
    if (!data.rows.every(row => facts(row.before) && facts(row.after) && Object.keys(row.changes).every(key => keys.has(key)
      ...
      throw C.failure('预检明细包含未说明的字段或非公开关系内容，本批未提交。');
  }

(function () {
  ...
  function Records({ api, selected, revision, onEdit, onClose, blocked }) {
    ...
    return <section className="os-detail" aria-label="外协登记与历史"><div className="os-heading"><h4>登记详情与核实历史</h4><div className="os-tools">
      ...
      <ErrorBox error={read.error} />{read.loading && <div className="os-empty" role="status">正在读取原登记和同一快照历史</div>}
      {item && <div data-outsourcing-detail={item.outsourcing_ref}><P.Target target={item.target} /><Issues issues={item.issues} /><P.Facts facts={item} />
        ...
        {data.history.items.map((h, i) => <details className="os-history" key={h.fact_ref} data-fact-ref={h.fact_ref}>
          ...
          <P.Facts facts={h.after} before={h.before} /><p>{h.reason}</p><div className="os-muted">系统记录人 {h.local_operator} · 历史引用 {h.fact_ref}</div>
        </details>)}
  ...
  function Content({ batchRef, outsourcingRef, onUpdated }) {
    ...
    return <section className="outsourcing-live" aria-label="真实外协登记" data-outsourcing-workspace data-ready={!!data}><window.OutsourcingStyles />
      ...
      <ErrorBox error={read.error} />{read.loading && <div className="os-empty" role="status">正在读取外协真实登记</div>}
      {data && <><div className="os-muted">数据截至 {P.when(result.meta.as_of)} · 工厂本地时间</div><div className="os-scroll"><table className="os-table"><thead><tr>
        <th>批次 / 成员</th><th>供应商 / 状态</th><th>实际发出</th><th>计划 / 实际回厂</th><th>操作</th></tr></thead><tbody>{data.items.map(r => <tr key={r.outsourcing_ref} data-outsourcing-ref={r.outsourcing_ref} data-selected={selected === r.outsourcing_ref}>
          <td><b>{P.value(r.target.batch.business_code)} · {P.value(r.target.batch.label)}</b><div className="os-muted">{r.target.kind === 'merged' ? '合并发出' : '单工序'} · {r.target.operations.map(o => P.value(o.business_code)).join('、')}</div></td>
          ...
          <td>{P.when(r.sent)}</td><td>{P.when(r.planned)}<div className="os-muted">{P.when(r.returned)}</div></td>
          <td><Button className="mini" icon="search" aria-label={'查看外协登记 ' + r.target.operations.map(o => P.value(o.business_code)).join('、')} onClick={() => open(r.outsourcing_ref)}>详情</Button></td></tr>)}</tbody></table></div>
        {!data.items.length && <div className="os-empty">{data.page.total ? '当前页没有登记。' : '当前筛选没有外协登记。'}</div>}

(function () {
  ...
  function input(value, record, action) {
    ...
    fields.forEach(key => {
      ...
      if (['completed_quantity', 'effective_processing_hours'].includes(key)) {
        ...
        v = v === '' ? null : Number(v);
      } else if (key === 'actual_start' || key === 'actual_end') v = v ? v.length === 16 ? v + ':00' : v : null;
  ...
  window.FieldContract = { states, fields, ref, task, report, query, blocked, draft, input, saveFile, display, date, quantity, pieceLabel, planQuantity, quantityReasons };
})();

(function () {
  ...
  function fail(message) { throw new Error(message || '主数据协议不完整，未使用样例或默认数量替代。'); }
  ...
  function scope(value) {
    ...
    if (!['issues', 'entities'].includes(result.view) || !['all'].concat(domains.map(row => row[0])).includes(result.domain)
      ...
      || typeof result.query !== 'string' || result.query.length > 1000 || !object(result.column_filters)) fail('主数据筛选范围无效。');
  ...
  function target(value) {
    ...
    if (value.view === 'batches') { if (!ref(c.entity_ref)) fail('批次目标缺少永久引用。'); return value; }
  ...
  function page(value, rows, size) {
    if (!object(value) || !count(value.total) || !Number.isSafeInteger(value.number) || value.number < 1 || !Number.isSafeInteger(value.pages)
      ...
      || rows.length !== Math.min(size, Math.max(0, value.total - (value.number - 1) * size))) fail('主数据分页总数无法核实。');
  }
  function list(result, expected, snapshot) {
    ...
    domains.forEach(([id], index) => {
      ...
      if (!object(domain) || domain.id !== id || typeof domain.loaded !== 'boolean'
        || (domain.loaded ? !count(domain.count) || !count(domain.attention) || !count(domain.unknown) || domain.attention > domain.count || domain.unknown > domain.count : domain.count !== null || domain.attention !== null || domain.unknown !== null)) fail('主数据域数量无法核实。');
    });
    ...
    if (['entities', 'issues', 'affected', 'known_relation_pairs'].some(key => !count(stats[key])) || !(stats.relations === null || count(stats.relations))
      ...
      || overview.complete !== overview.domains.every(item => item.loaded)) fail('主数据总览数量无法核实。');
    return result;
  }
  function detail(result, expected, selection, section, snapshot) {
    ...
    if (canonical(scope(data.scope)) !== canonical(scope(expected)) || result.meta.snapshot_ref !== snapshot
      ...
      || !Array.isArray(data.rows) || !object(data.counts) || ['issues', 'relations', 'fields'].some(key => !count(data.counts[key]))) fail('详情与所选实体或快照不一致。');

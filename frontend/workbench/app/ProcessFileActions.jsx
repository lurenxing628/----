(function () {
  ...
  function ProcessFileActions({ adapter, kind, mode, request, onClose, onCommitted, disabled = false }) {
    ...
    const [groups, setGroups] = React.useState([]), [zero, setZero] = React.useState(false), [ack, setAck] = React.useState(false), [now, setNow] = React.useState(Date.now());
    ...
    function invalidate() { checkedPreview.current = null; setJob(null); setGroups([]); setZero(false); setAck(false); setError(null); setDownload({ busy: false }); }
    ...
    if (data && importing && !reason) { try { F.confirmInput(data, groups, zero); } catch (failure) { reason = C.message(failure); } }
    ...
    function confirm() {
      ...
      try { command.submit('process_' + kind + '_import', 'confirm', data.preview_ref, data.write_context, F.confirmInput(data, groups, zero)); }
        ...
        <div className="modal-b scroll rm-body">
          ...
          {!recovery && !done && <><div className="rm-format"><span className="seclabel">文件格式</span><div className="seg" role="group" aria-label="文件格式">{['xlsx', 'csv'].map(value => <button type="button" key={value} className={format === value ? 'on' : ''} aria-pressed={format === value} disabled={controlsDisabled} onClick={() => { invalidate(); setFormat(value); }}>{value === 'xlsx' ? 'Excel (.xlsx)' : 'CSV (.csv)'}</button>)}</div></div>
            {importing ? <><div className="tmpl-row"><span className="tmpl-ico"><Icon name="file-input" /></span><div className="tmpl-t">{label}空白模板.{format}</div><Button transfer="template" disabled={controlsDisabled} onClick={() => downloadFile(true)}>下载模板</Button></div>
              ...
              {original.target_ref && <p>只核对并修改当前零件，不接受文件中的其他零件。</p>}</> : <fieldset disabled={controlsDisabled} style={{ border: 0, padding: 0, margin: 0 }}><legend className="seclabel">导出范围</legend>
          ...
          {data && importing && !done && <><p className="iohint">{data.instructions}</p><window.ProcessFilePreview data={data} groups={groups} onGroups={setGroups} disabled={controlsDisabled} />
            {!done && data.zero_review_required && <label className="rm-check"><input type="checkbox" checked={zero} disabled={controlsDisabled} onChange={event => setZero(event.target.checked)} />已复核单件工时为 0 的记录，确认保留 0。</label>}

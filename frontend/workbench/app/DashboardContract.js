(function () {
  ...
  const categories = { all: '全部风险', delivery: '交期风险', actual: '执行偏差', external: '外协回厂', downtime: '停机影响', material: '齐套缺口', candidate: '候选方案' };
  const statuses = { new: '待分析', following: '跟进中', awaiting_verification: '待验证', closed: '已关闭' };
  const states = { loaded: '已读取', no_data: '无数据', no_official_plan: '无正式计划', unavailable: '无法评估', not_connected: '尚未接入' };
  const fields = ['owner', 'deadline', 'action', 'remark', 'completed_at', 'completion_evidence', 'evidence_reference_text', 'evidence_ref'];
  const labels = { status: '处置状态', owner: '责任人', deadline: '责任期限', action: '处置行动', remark: '原因 / 核实备注', completed_at: '完成时间', completion_evidence: '具体完成结果', evidence_reference_text: '可核对凭据', evidence_ref: '已核验附件引用' };
  ...
  function check(ok, message = '值班台响应不完整或来源不一致，未显示替代结果。') { if (!ok) throw new Error(message); }
  function scope(value = {}) {
    ...
    check(object(value) && Object.keys(q).every(k => ['category', 'status', 'query', 'sort', 'direction', 'page', 'size', 'source', 'snapshot_ref'].includes(k))
      ...
      && q.source === 'production' && (q.snapshot_ref === undefined || text(q.snapshot_ref)), '值班台查询条件无效，未扩大原范围。');
  ...
  function item(row) {
    ...
    if (row.category === 'external') {
      ...
      const n = row.navigation[0]; check(object(n) && n.view === 'outsourcing' && object(n.context) && Object.keys(n.context).length === 1
        ...
        && n.enabled === (row.source_state === 'current'), '外协导航不是原登记，未改指其他对象。');
  ...
  function external(s) {
    ...
    check(s.kind === 'outsourcing_receipts' && s.tracking_basis === 'manual_receipt_facts'
      ...
      && keys.every(k => connected ? count(s[k]) : s[k] === null), '外协汇总不是独立登记事实，未套用通用处置。');
  ...
  function catalog(v, query) {
    ...
    external(d.categories.external); check(d.categories.external.handling_supported || d.items.every(row => row.category !== 'external'), '外协处置尚未接入，未混入通用历史。');
  ...
  function detail(v, q, selected, historyPage) {
    const d = envelope(v, q); item(d.item); check(d.item.item_ref === selected, '详情不是原条目，未改指其他对象。');
  ...
  function receipt(v, intent) {
    const d = v && v.data; check(v && v.ok === true && ['committed', 'unchanged'].includes(v.result) && /^[a-f0-9]{32}$/.test(v.receipt_ref)
      ...
      && (v.result === 'committed' ? ref(d.history_ref) : d.history_ref === null), '回执与原处置不一致，结果尚未确认，请查询原请求。'); return v;
  }
  ...
  function failure(v, status, intent) {
    ...
    const error = new Error(valid ? e.message : '响应无法核实，请保留原请求。'); error.code = valid ? e.code : 'invalid_response'; error.fields = valid ? e.fields : [];

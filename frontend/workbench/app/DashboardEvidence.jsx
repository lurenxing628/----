(function () {
  ...
  const labels = {
    ...
    ready_status: '齐套状态', due_date: '交期', planned_finish: '计划完工', delay_days: '预计晚交天数',
    planned_start: '正式安排开始', planned_end: '正式安排结束', first_actual_start: '首次实际开始', confirmed_finish: '确认完工时间',
    finish_deviation_minutes: '完工偏差（分钟）', overlap_hours: '停机重叠（小时）', delay_after_reschedule_hours: '重排后延期（小时）',
    execution_state: '执行状态', data_quality: '事实完整性', hours: '工时依据', effective_processing_hours: '有效加工小时',
    ...
    active: '当前是否存在', subject: '对象', note: '备注', processing_hours: '加工小时', available: '是否可用',
    basis: '计算依据', setup_or_elapsed_hours_included: '是否包括准备或停留时间'
  };
  const values = { yes: '已齐套', no: '未齐套', partial: '部分完成', unreported: '尚未报工', started: '已开始', complete: '完整',
    paused: '已暂停', exception: '异常', incomplete: '尚不完整', invalid: '待核对', legacy: '旧事实需核对', available: '已读取',
    unknown: '未知', missing: '缺少来源', unavailable: '无法读取', on_time: '预计准时', overdue: '预计晚交',
  ...
  function Evidence({ source }) {
    ...
    return <><window.WorkbenchReference entries={{ '正式计划编号': source.plan_ref, '批次编号': source.batch_ref }} />
      ...
      {source.downtimes && <Structure title="停机窗口依据" value={source.downtimes} />}

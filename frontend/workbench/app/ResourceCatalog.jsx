(function () {
  ...
  function CatalogList({ kind, list, scope, setScope, onOpen, disabled }) {
    ...
    </form><ErrorBox error={list.error} />{list.loading && <EmptyState kind="loading" title="正在读取目录" />}
      {data && <><Issues issues={list.result.warnings} /><div className="wb-table-frame rc-list-scroll" data-sticky-head data-sticky-actions><table className="tbl wb-table rc-list"><caption className="wb-visually-hidden">{M.names[kind]}目录</caption><thead><tr>
        ...
        {!data.entities.length && <EmptyState kind={scope.query || scope.status ? 'filtered' : 'empty'} title="当前范围没有目录记录"
          hint="可清除搜索和状态筛选后查看全部目录。" action={scope.query || scope.status ? <Button disabled={disabled} onClick={() => { setSearch(''); filter({ query: '', status: '' }); }}>清除筛选</Button> : undefined} />}
        <Pager page={data.page} sizes={[20]} unit="条" label="目录" disabled={disabled || list.loading}
  ...
  function ResourceCatalog({ kind, adapter, onClose, onCommitted }) {
    ...
    const guardOwner = window.WorkbenchGuards.useDirtyGuard({ owner: 'resource-catalog-' + formId,
      ...
      locked: command.locked, message: '资源目录有尚未保存的修改。' });
    ...
    async function submit(event) {
      event.preventDefault(); if (locked || done || !editor || reason) return;
      try {
    ...
    return <div className="plana resource-catalog" data-resource-catalog={kind} ref={root}>
      <Modal title={title} icon={kind === 'shift_profile' ? 'clock-3' : 'folder-open'} locked={locked} guardOwner={guardOwner} onClose={options => requestClose('close', options)}
        footer={<><Button onClick={() => requestClose()} reason={locked ? '操作尚未核实，请保留当前页面。' : ''}>{lastReceipt || done ? '完成并返回' : '关闭'}</Button>
        ...
        <div className="modal-b form scroll">
          ...
          {editor && !done && <form id={formId} onSubmit={submit} noValidate><Editor kind={kind} editor={editor} error={error || command.error} disabled={locked} onChange={change} onValidationError={setError} /></form>}
          ...
          {command.intent && command.locked && <window.WorkbenchReference entries={{ '请求编号': command.intent.request_key }} />}
          {editor && !done && !command.locked && <div className="rc-pattern">
            <Button icon="refresh-cw" disabled={busy} onClick={() => { setReview(null); load(editor.action, editor.ref, true); }}>重新读取最新资料</Button>

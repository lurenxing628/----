(function () {
  ...
  function Config({ api, revision, command, active = true, ...preferences }) {
    ...
    const reason = command.locked ? '原维护请求尚未确认，请先核实结果。' : !ready ? '请先重新读取正式配置快照。' : '';
    ...
    return <div className="sm-configuration" style={{ maxWidth: 'none' }}>
      ...
      <section className="sm-section sm-maintenance-config"><div className="sm-section-head"><h3>本机自动维护配置</h3><C.Button icon="refresh-cw" aria-label="重新读取配置" busy={request.loading}
        ...
        {incoming && base && incoming !== base && changed && <div className="sm-notice" role="status">已读取新快照，当前草稿尚未覆盖。请核对已存值变化后再保存。
        ...
        {draft && base && <form className="sm-config-form" noValidate onSubmit={event => { event.preventDefault(); setValidated(true);
          ...
          {A.fields.filter(field => field.group === group).map(field => <div className="sm-config-row" key={field.key} style={{ gridTemplateColumns: 'minmax(140px, 1fr) minmax(140px, 1fr)' }}>
            <label htmlFor={'sm-maintenance-' + field.key}>{field.label}</label><div>
              ...
              {base.dirty_fields.includes(field.key) ? <small className="sm-error">旧配置异常：{base.dirty_reasons[field.key]}{base.stored_values[field.key] !== null ? ' · 原值：' + base.stored_values[field.key] : ''}</small>
                : base.defaulted_fields.includes(field.key) ? <small className="sm-meta">缺省值，尚未保存</small> : <small className="sm-meta">已存值：{field.switch ? base.values[field.key] === 'yes' ? '启用' : '关闭' : base.values[field.key]}</small>}
            </div>
          </div>)}
        </fieldset>)}</div>
          <div className="sm-form-footer"><span className="sm-meta">{changed ? '有未保存修改' : base.dirty_fields.length || base.defaulted_fields.length ? '含异常或缺省项，尚待核对保存' : '当前值已读取'}</span><div className="sm-actions">
          ...
          </div></div>{validated && !validation.valid && <p className="sm-error" role="alert">配置校验未通过，请修正标出的字段。</p>}
        </form>}
        <details className="sm-rules sm-config-help"><summary>生效范围与自动维护规则</summary><p>自动维护由正常访问请求触发检查，检查间隔不保证指定时刻执行。操作日志清理不清空运行文件日志。</p><p>主题、每页条数、紧凑行距属于页面偏好，不写入业务配置。</p></details>
      </section>
      {replace && <window.ResourceControls.Modal title="放弃当前草稿并重新读取？" icon="history" onClose={() => setReplace(false)}
        footer={<><C.Button onClick={() => setReplace(false)}>保留草稿</C.Button><C.Button icon="refresh-cw" onClick={refreshNow}>放弃并重读</C.Button></>}><div className="modal-b form"><p>未保存修改将被丢弃，不会写入业务配置。</p></div></window.ResourceControls.Modal>}
    </div>;
  }

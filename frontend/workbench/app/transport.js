(function () {
  ...
  async function decode(response) {
    ...
    if (payload.schema_version !== 1 || !payload.meta || payload.meta.source !== 'production'
        || payload.meta.time_basis !== 'factory_local'
        || typeof payload.meta.snapshot_ref !== 'string' || !payload.meta.snapshot_ref
  ...
  async function read(url, signal) {
    ...
    if (target.origin !== location.origin || !target.pathname.startsWith('/api/workbench/v1/'))
      throw new Error('数据地址不属于本机工作台。');
    ...
    try {
      var response;
      try { response = await fetch(target.href, { signal: controller.signal, credentials: 'same-origin', headers: { Accept: 'application/json' } }); }
      catch (error) {
        if (error.name === 'AbortError') throw error;

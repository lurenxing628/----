(function () {
  ...
  function Candidates({ catalog, initialContext = {}, onState, selectedBatch, onSelectBatch, issueBatchRef = null }) {
    ...
    const caption = data ? { reference: data.candidate.candidate_ref, label: '比较方案', name: data.candidate.label || '候选名称未记录',
      status: data.baseline.available ? '已保存候选 · 受理时正式基线对照' : '已保存候选 · 无受理时正式基线' } : null;
    ...
    return <section aria-label="已保存候选同范围比较" data-dashboard-candidates data-comparison-ready={!!data}>
      <div className="dy-heading"><h3>同一受理范围下比较</h3><Button reasonDisplay="inline" icon="refresh-cw" aria-label="重新读取候选比较" onClick={refresh} /></div>
      <label className="dy-run-picker">排产运行<select aria-label="选择排产运行" value={choice.run_ref || ''} onChange={event => changeRun(event.target.value)}><option value="">请选择运行</option>
        {choice.run_ref && !catalog.runs.some(row => row.run_ref === choice.run_ref) && <option value={choice.run_ref}>原选中的排产运行</option>}
      ...
      {list.loading && <p role="status">正在读取原运行的已保存候选目录。</p>}
      ...
      </label>)}{!options.candidates.length && <window.WorkbenchListControls.EmptyState kind="empty" title="原运行尚无已保存候选结果。" />}</fieldset>}
      ...
      {data && <><div className="dy-context"><span>{window.WorkbenchFormat.dateTime(data.time_scope.range_start)} 至 {window.WorkbenchFormat.dateTime(data.time_scope.range_end)} · 左闭右开</span>
        ...
        <div className="dy-heading"><h3>{data.candidate.label || '候选名称未记录'}</h3><Button reasonDisplay="inline" icon="chart-gantt" onClick={() => setSummary(true)}>查看方案摘要</Button></div>
        <details className="dy-evidence"><summary>受理时排产约束</summary><dl className="dy-facts"><div><dt>齐套检查</dt><dd>{data.generation.input.ready_check ? '开启' : '关闭'}</dd></div>
          ...
          <div><dt>已开工策略</dt><dd>保留已记录实际及受保护安排</dd></div></dl><window.WorkbenchReference entries={{ '运行编号': data.generation.run_ref }} /></details>

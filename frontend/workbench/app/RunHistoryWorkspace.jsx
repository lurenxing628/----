(function () {
  ...
  function Session({ adapter, start, onNavigate }) {
    ...
    return <div className="plana run-history-workspace" data-run-history-workspace aria-busy={read.busy}><C.Styles />
      <header className="rh-heading"><div><h2 className="wb-page-title">排产历史</h2><span className="rh-muted wb-page-context">运行记录 · 只读</span></div><div className="rh-tools">
      ...
      {read.busy && <window.WorkbenchListControls.EmptyState kind="loading" title="正在读取排产历史" hint="当前筛选结果尚未返回" />}
      {data && <><div className="rh-source"><span>目录共 {window.WorkbenchFormat.number(data.run_count, { digits: 0 })} 次 · 受理日期按工厂本地时间，含起止日</span>
        ...
        {data.runs.length ? <C.Table runs={data.runs} canNavigate={typeof onNavigate === 'function'} onOpen={open} /> : <window.WorkbenchListControls.EmptyState kind={data.run_count === 0 ? 'empty' : 'filtered'}
          ...
          action={data.run_count > 0 && <C.Button icon="chevron-left" onClick={() => data.page.total > 0 ? change({ page: 1 }, true) : apply(A.scope({ size: query.size }))}>{data.page.total > 0 ? '返回第一页' : '清除历史筛选'}</C.Button>} />}
  ...
  function RunHistoryWorkspace({ initialContext = {}, onNavigate, adapter }) {
    ...
    catch (error) { return <div className="plana run-history-workspace" data-run-history-workspace><C.Styles /><h2 className="wb-page-title">排产历史</h2><C.ErrorBox error={error} /></div>; }

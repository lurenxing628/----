  ...
  function Catalog({ adapter, selectedRef, onSelect, autoSelect = false, disabled = false, actions }) {
    ...
    React.useLayoutEffect(() => {
      if (collapsed && restoreFocus.current && section.current) {
        restoreFocus.current = false; section.current.querySelector('[aria-label="展开计划列表"]').focus();
      }
    }, [collapsed]);
    ...
    return <section ref={section} className="plan-catalog" aria-label="排产方案列表" data-collapsed={collapsed}>
      ...
      {(read.error || paused) && <div className="plan-pager"><span>{paused ? '计划列表读取已取消。' : '计划列表没有读成功，已选的计划没有变。'}</span><Button icon="refresh-cw" onClick={() => refresh()}>刷新列表</Button></div>}
      ...
      {!collapsed && <div className="plan-catalog-scroll wb-table-frame" data-wb-scroll-key="plan-catalog" data-sticky-head data-sticky-actions><table className="wb-table" aria-label="可选排产方案" aria-busy={read.loading}>
        ...
        <tbody>{data && data.plans.map((plan, row) => <tr key={plan.plan_ref || 'unavailable-' + row} aria-selected={!!plan.plan_ref && plan.plan_ref === selectedRef}>
          ...
          <td>{({ complete: '记录完整', partial: '部分记录', invalid: '无效记录', unknown: '暂无数据' })[plan.completeness]}
            {plan.blocked_reasons.length > 0 && <div className="plan-muted">{plan.blocked_reasons.map(reason => reason.message).join('；')}</div>}</td>
        </tr>)}{(!data || !data.plans.length) && <tr><td colSpan={4}><window.WorkbenchControls.EmptyState kind={read.loading ? 'loading' : 'empty'} title={read.loading ? '正在读取计划列表…' : paused ? '读取已取消' : read.error ? '计划列表读取失败' : '本页没有计划记录。'} /></td></tr>}</tbody>
      </table></div>}
      {data && !collapsed && <div className="plan-pager">
        ...
        <window.WorkbenchControls.Pager mode="cursor" page={index + 1} label="计划列表" disabled={disabled} busy={read.loading} hasPrevious={index > 0} hasNext={data.page.has_more}

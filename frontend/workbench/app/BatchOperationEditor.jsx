(function () {
  ...
  function BatchOperationEditor({ adapter, entity, operation, source, command, onCommitted, onClose, disabled }) {
    ...
    return <Modal title={'工序 ' + operation.sequence + ' · ' + operation.label} icon="wrench" locked={command.locked} onClose={onClose}
      footer={<><Button onClick={onClose} disabled={command.locked}>{done ? '关闭' : '取消'}</Button>{!done && <Button form={form} type="submit" icon="check" className="btn primary"
        disabled={locked || !catalogs || !!mismatch} reason={B.reason(entity.write_context, 'operation_update', source)}>保存工序</Button>}</>}>
      <form id={form} className="modal-b form" onSubmit={submit} noValidate><div className="fgrid batch-fields">
        {keys.map(key => <Field key={key} label={names[key]}>{key.endsWith('_ref') ? <select value={draft[key]} disabled={locked || !catalogs} onChange={event => setDraft({ ...draft, [key]: event.target.value })}>
      ...
      </div>{merged && <p>合并外协组 {operation.external_group.business_code} · 整组周期 {B.label('', operation.external_group.total_days)} 天（只读）</p>}

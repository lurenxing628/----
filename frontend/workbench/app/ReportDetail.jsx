(function () {
  ...
  function Evidence({ record }) {
    ...
    return <details className="rw-limitations" style={{ overflowWrap: 'anywhere' }}><summary>{record.record_kind_label} · {record.report_no || record.event_label} · {time(record.event_time)}</summary>
      ...
      {record.record_kind === 'legacy_event' ? <><h4>历史原始记录</h4><window.ReportEvidence.StructuredFacts value={record.legacy_evidence} /></> : <>
        <p>记录人：{text(record.local_operator)}；经办人：{text(record.declared_operator)}</p>
        ...
        {record.correction_history.map(revision => <details key={revision.revision_ref}><summary>{window.WorkbenchTerms.report_actions[revision.action] || revision.action} · {time(revision.recorded_at)}</summary>
          ...
          <window.ReportEvidence.TableFrame caption="报工更正前后的值"><window.APSWorkbenchUI.DataTable className="rw-record-detail" rowKey="field"
            ...
            rows={fields.map(([key, label]) => ({ field: key, label, before: revision.before === null ? '新增，无原值' : text(revision.before[key]), after: text(revision.after[key]) }))} /></window.ReportEvidence.TableFrame>

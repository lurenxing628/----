(function () {
  ...
  function ordering(scope) {
    ...
    if (!Array.isArray(rows) || rows.length > columns.length || !rows.every(row => C.object(row)
        && columns.some(column => column.key === row.field) && ['asc', 'desc'].includes(row.direction)
        && Object.keys(row).every(key => ['field', 'direction'].includes(key))) || new Set(rows.map(row => row.field)).size !== rows.length
        || Array.isArray(value) && scope.direction && scope.direction !== 'asc') throw C.failure('工艺排序不完整，请重新选择排序。');
    return rows;
  }
  ...
  function entity(value) {
    ...
    return ['route_raw', 'route_parsed', 'remark'].every(key => nullableText(value.fields[key])) && issues(value.issues)
      && ['batch_count', 'operation_count', 'internal_count', 'external_count', 'unclassified_count'].every(key => count(value.relationships[key]))
      && C.object(w) && ['legacy', 'managed'].includes(w.origin) && ['route', 'source', 'hours', 'ready'].includes(w.stage) && typeof w.ready === 'boolean'
      && stamp(w.route, ['present', 'missing', 'unconfirmed', 'confirmed'])
      && stamp(w.source, ['unconfirmed', 'locked', 'confirmed']) && stamp(w.hours, ['unconfirmed', 'locked', 'confirmed'])
      && (w.ready === (w.stage === 'ready')) && (!w.ready || ['route', 'source', 'hours'].every(key => w[key].state === 'confirmed'));
  }
  function stamp(value, states = ['unconfirmed', 'confirmed']) {
    return C.object(value) && states.includes(value.state) && nullableText(value.confirmed_at) && nullableText(value.confirmed_by)
      && (value.state !== 'confirmed' || token(value.confirmed_at));
  }
  function writeContext(value, action) {
    if (value === null) return !action;
    return C.object(value) && token(value.write_token) && C.object(value.capabilities) && Array.isArray(value.blocked_reasons)
      && (!action || value.capabilities['process.' + action] === true);
  }
  function operation(row) {
    return C.object(row) && ref(row.ref) && storedSequence(row.sequence) && text(row.label) && source(row.source)
      && ['op_type_ref', 'supplier_ref', 'external_group_ref'].every(key => nullableRef(row[key]))
      && ['op_type_label', 'supplier_label'].every(key => nullableText(row[key]))
      && ['setup_hours', 'unit_hours', 'external_days'].every(key => number(row[key])) && text(row.status) && issues(row.issues)
      && C.own(row, 'external_days_source') && [null, 'operation', 'group'].includes(row.external_days_source)
      && (row.external_days_source === 'operation' ? row.external_days !== null && row.external_days >= 0
        && (row.source !== 'external' || row.external_days > 0) : row.external_days === null)
      && C.object(row.confirmation) && stamp(row.confirmation.source) && stamp(row.confirmation.hours);
  }
  function externalGroup(row) {
    return C.object(row) && ref(row.ref) && storedSequence(row.start_sequence) && storedSequence(row.end_sequence)
      && nullableText(row.merge_mode) && number(row.total_days) && nullableRef(row.supplier_ref)
      && nullableText(row.supplier_label) && nullableText(row.remark) && issues(row.issues);
  }
  function uniqueRefs(rows) { return new Set(rows.map(row => row.ref)).size === rows.length; }
  ...
  function cycleReferences(entity) {
    ...
    return entity.operations.every(row => row.external_days_source !== 'group' || row.source === 'external' && row.status === 'active'
      && groups.has(row.external_group_ref) && !invalid.has(row.external_group_ref)
      && groups.get(row.external_group_ref).merge_mode === 'merged' && groups.get(row.external_group_ref).total_days > 0
      && !groups.get(row.external_group_ref).issues.length
      && !row.issues.some(item => ['sequence_invalid', 'external_group_invalid', 'external_group_part_mismatch'].includes(item.code)));
  }
  ...
  function list(result, scope) {
    C.query(result, 'list');
    const d = result.data, p = d.page;
    if (!d.entities.every(entity) || !uniqueRefs(d.entities) || !capabilities(d.capabilities)
        || d.create_context !== null && (!writeContext(d.create_context) || !C.object(d.create_context))
        || !C.object(d.metrics) || !C.object(d.metrics.counts) || !stages.every(row => count(d.metrics.counts[row[2]]))
        || p.number !== scope.page || p.size !== scope.size
  ...
  function preview(result, expectedRef, body) {
    ...
    if (!C.object(d) || d.part_ref !== expectedRef || d.mode !== body.mode || !text(d.route_raw) || !text(d.normalized_input)
        ...
        || !d.operations.every(row => C.object(row) && sequence(row.sequence) && text(row.op_type_name)
          && nullableRef(row.op_type_ref) && source(row.source_suggestion) && nullableRef(row.supplier_ref)
          && nullableText(row.supplier_label) && number(row.external_days) && (text(row.basis) || C.object(row.basis) || Array.isArray(row.basis)) && issues(row.issues))
        || !Array.isArray(d.diagnostics) || !d.diagnostics.every(row => C.object(row) && text(row.code) && text(row.message)
          && ['error', 'warning'].includes(row.severity) && (!C.own(row, 'sequence') || sequence(row.sequence)))
        || !C.object(d.counts) || !['operations', 'recognized', 'unknown'].every(key => count(d.counts[key]))
        || d.counts.operations !== d.operations.length || d.counts.recognized + d.counts.unknown !== d.counts.operations
        || !C.object(d.baseline) || !count(d.baseline.operation_count) || !count(d.baseline.external_group_count)
  ...
  function previewBody(mode, routeRaw, rows, snapshot) {
    let body;
    if (mode === 'text') body = { mode, route_raw: routeRaw };
    else {
      const fields = [];

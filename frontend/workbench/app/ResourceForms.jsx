(function () {
  ...
  function Feedback({ command, excludePaths = [] }) {
    ...
    return <>
      {['sending', 'checking'].includes(phase) && <p role="status">{phase === 'sending' ? '正在提交，请勿重复保存…' : '正在查询上次操作的结果…'}</p>}
      {phase === 'pending' && <div role="status" className="match-note" style={{ display: 'block' }}>
        <p>{window.WorkbenchTerms.outcomes.pending('保存')}</p>
        <Button icon="history" onClick={command.check}>{window.WorkbenchTerms.actions.query_result}</Button>
      </div>}
      {phase === 'done' && <p role="status">{command.result.result === 'partial' ? '部分操作完成，请核对逐项结果。' : command.result.result === 'unchanged' ? '内容没有变化，已确认。' : window.WorkbenchTerms.outcomes.done('保存')}</p>}
    ...
    return <div className="field full"><label>既有设备授权（只读）</label>
      {Array.isArray(facts) ? facts.length ? <div className="chipline">{facts.map((item, index) => <span className="chip" key={item.ref || index} style={{ whiteSpace: 'normal' }}>
        ...
        Number.isSafeInteger(entity.relationships.machine_authorization_count) ? <span className="fhint">已登记 {entity.relationships.machine_authorization_count} 项设备授权，明细未读取。</span> :
          C.own(entity.relationships, 'machine_refs') ? <Relation entity={entity} field="machine_refs" /> : <span className="fhint">既有设备授权还没读取，不会按技能推算。</span>}
  ...
  function ResourceForms({ adapter, kind, category, action = 'create', entity: initialEntity, acceptedEntity, writeContext, source, command,
    ...
    async function submit(event) {
      ...
      try {
        ...
        if (invalidNumbers.length) throw C.failure('请核对标红的数字。', invalidNumbers.map(element => ({ path: 'fields.' + element.name, message: '请输入有效数字，无效内容不会当成未填写提交。' })));
    ...
    async function catalog(field, reload) {
      ...
      const completed = result => {
        ...
        if (C.receipt(result) === 'terminal') { reload(); if (result.result === 'partial') setError(C.failure(field.label + '只完成了一部分，请点「维护' + field.label + '」核对逐项结果。')); }
    ...
    return <Modal title={adjustingStock ? '调整库存' : (action === 'create' ? '新增' : action === 'delete' ? '删除' : '编辑') + C.resourceName(kind, opCategory)} icon={icons[kind]} onClose={close} guardOwner={guardOwner} locked={command.locked || catalogBusy || contextBusy} suspended={catalogBusy}
      footer={<><Button onClick={close} reason={command.locked ? '结果还没确认，暂时不能关闭。' : contextBusy ? '正在读取最新资料。' : ''} disabled={catalogBusy}>{done ? '关闭' : '取消'}</Button>
        {!done && <Button form={formId} type="submit" icon={action === 'delete' ? 'minus' : 'check'} className={'btn primary wb-action wb-primary'} reason={reason || confirmReason} busy={disabled}>

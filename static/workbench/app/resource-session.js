(function () {
  ...
  function useCommand(adapter) {
    ...
    React.useEffect(() => {
      if (store.state.restored && store.state.intent) {
        store.state.restored = false;
        check();
      }
    }, [store]);
    ...
    async function submit(kind, action, ref, context, input, category) {
      ...
      try {
        const result = await adapter.command(kind, action, ref, {
          ...
          write_token: context.write_token,
          input
        ...
        publish(store, {
          intent,
          result,
          phase: status === 'terminal' ? 'done' : status === 'rejected' ? 'rejected' : 'pending',
          error: status === 'rejected' ? result : null
        });
      } catch (error) {
        publish(store, {
          intent,
          phase: error && error.committed === false ? 'rejected' : 'pending',
          error
        });
      } finally {
        store.running = false;
  ...
  function calendarSummary(value) {
    ...
    const config = value => object(value) && ['known', 'not_configured', 'unavailable'].includes(value.status) && (value.status === 'known' ? nonnegative(value.value) : value.value === null);
    return object(value) && value.time_basis === 'factory_local' && typeof value.factory_today === 'string' && typeof value.basis === 'string' && typeof value.week_start === 'string' && typeof value.week_end === 'string' && config(value.standard_hours) && config(value.holiday_default_efficiency) && issues(value.holiday_default_efficiency.issues) && object(value.stats) && ['configured_days', 'default_days', 'unavailable_days'].every(key => nonnegative(value.stats[key])) && ['work_days', 'rest_days', 'effective_hours', 'normal_effective_hours', 'urgent_effective_hours'].every(key => nullable(value.stats[key])) && Array.isArray(value.stats.known_rest_dates) && Array.isArray(value.days) && value.days.length === 7 && new Set(value.days.map(day => day && day.date)).size === 7 && value.days.every((day, index) => object(day) && typeof day.date === 'string' && day.weekday === index && typeof day.explicit === 'boolean' && issues(day.issues) && (day.status === 'unavailable' ? day.effective === null : day.status === 'known' && object(day.effective) && ['hours', 'efficiency', 'effective_hours', 'normal_effective_hours', 'urgent_effective_hours'].every(key => nonnegative(day.effective[key])) && ['is_working', 'is_rest', 'allow_normal', 'allow_urgent', 'crosses_midnight'].every(key => typeof day.effective[key] === 'boolean') && typeof day.effective.window_start === 'string' && typeof day.effective.window_end === 'string'));
  }
  ...
  function useCounts(adapter, revision) {
    const read = useSummary(adapter, revision),
      data = read.result && read.result.data;
    const names = {
      process: 'part',
      op_int: 'internal_op_types',
      op_ext: 'external_op_types'
    };
    const error = read.error ? C.message(read.error) : null;
    const counts = Object.fromEntries(Object.keys(C.nodes).map(node => [node, {
      total: data ? data.counts[names[node] || node] : null,
      error
    }]));
    counts.summary = {
      data,

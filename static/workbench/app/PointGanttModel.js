(function () {
  ...
  function bounds(start, end, hasPoints) {
    if (start === null || !hasPoints) return {
      start,
      end
    };
  ...
  function tracks(items, start, end, width) {
    const lanes = [],
      heap = [],
      scale = Math.max(1, width) / (end - start);
    for (const item of items.slice().sort((a, b) => a.start - b.start || String(a.task.task_ref || a.task.row_ref).localeCompare(String(b.task.task_ref || b.task.row_ref)))) {
      ...
      lanes[slot.lane].push(item);
      slot.end = x + gap;
      let i = heap.length;
      heap.push(slot);
      while (i > 0) {
        const p = i - 1 >> 1;
        if (heap[p].end <= slot.end) break;
        heap[i] = heap[p];
        i = p;
      }
      heap[i] = slot;
  ...
  function visible(items, start, end, scale) {
    ...
    while (low < high) {
      ...
      if (items[mid].start < start - pad) low = mid + 1;else high = mid;
    }
    const result = [];
  ...
  function candidateRows(model, width) {
    ...
    function append(source, items, key) {
      ...
      for (const item of items) (row.baseline ? baselineLocations : locations).set(row.baseline ? item.segment.row_ref : item.task.row_ref, {
        row: rows.length,
        top,
        item
      });
    ...
    for (const row of model.rows) {
      if (!row.point) {
        append(row, row.items, row.key);
        continue;
      }
      if (done.has(row.id)) continue;
      done.add(row.id);
      const lanes = tracks(groups.get(row.id), model.start, model.end, width);
      lanes.forEach((items, lane) => append({
        ...row,
        lane,
        laneCount: lanes.length
      }, items, row.id + ':point:' + lane));
    }
    return {
      ...model,
      rows,

(function () {
  ...
  const executionLabels = {
    ...
    known_completed_quantity: '已知完成数量',
  ...
  const matching = (tasks, query) => {
    ...
    return needle ? tasks.filter(t => searchText(t).includes(needle)) : tasks;
  };
  const resource = (t, mode) => mode === 'batch' ? {
    ref: t.batch_ref,
    label: t.batch_label
  } : t[mode];
  function title(t) {
  ...
  function pop(heap) {
    ...
    while (i * 2 + 1 < heap.length) {
      let c = i * 2 + 1;
      if (c + 1 < heap.length && heap[c + 1].end < heap[c].end) c++;
      if (heap[c].end >= last.end) break;
      heap[i] = heap[c];
      i = c;
    }
    heap[i] = last;
  ...
  function layout(data, mode, query, width = 1000) {
    const groups = new Map(),
      rows = [],
      locations = new Map(),
      tasks = matching(data.tasks, query);
    ...
    for (const task of tasks) {
      const r = resource(task, mode),
        id = r && r.ref || 'missing:' + task.row_ref;
      if (!groups.has(id)) groups.set(id, {
        id,
        label: r && r.label || kindLabels[mode] + '未记录',
        items: []
      });
    ...
    for (const group of groups.values()) {
      ...
      const lanes = [...normal, ...G.tracks(group.items.filter(i => P.isPoint(i.task)), bounds.start, bounds.end, width)];
      lanes.forEach((items, lane) => {
        const row = {
          ...
          normalLaneCount: normal.length,
          point: P.isPoint(items[0].task)
        };
        items.forEach(item => locations.set(item.task.row_ref, {
          row: rows.length,
          top,
          item
        }));

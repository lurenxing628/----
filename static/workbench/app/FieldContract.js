(function () {
  ...
  function query(result, mode, expected) {
    if (!result || result.ok !== true || result.schema_version !== 1 || !result.meta || result.meta.source !== 'production' || result.meta.time_basis !== 'factory_local' || !result.meta.snapshot_ref || !result.meta.as_of || !C.object(result.data)) throw C.failure('现场记录协议不完整，未使用样例替代。');
    const d = result.data;
  ...
  function blocked(context, action) {
    if (!context || !context.write_token || !context.capabilities || context.capabilities[action] !== true) return (context && context.blocked_reasons || []).map(item => item.message).join('；') || '当前上下文不可写，请刷新并核对正式计划。';
    return '';
  }

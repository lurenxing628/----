(function () {
  ...
  function deadlines(item, asOf) {
    ...
    return {
      finishLate: e.execution_state === 'complete' && instant(e.confirmed_finish) - end > 600000,
  ...
  function filter(data, view, asOf) {
    const labels = names(data),
      query = view.query.trim().toLowerCase();
    return data.items.filter(item => (!query || searchText(item, labels).includes(query)) && (view.late === 'all' || deadlines(item, asOf)[view.late]) && (!view.onlySelected || item.task.task_ref === view.selected));
  }
  ...
  function tracks(reports) {
    ...
    reports.slice().sort((a, b) => instant(a.actual_start) - instant(b.actual_start) || a.report_ref.localeCompare(b.report_ref)).forEach(report => {
      ...
      const heap = owners.get(key),
        start = instant(report.actual_start);
      const index = heap.length && heap[0].end <= start ? pop(heap).index : result.length;
      if (!result[index]) result[index] = [];
      result[index].push(report);
      push(heap, {
        end: report.actual_end ? instant(report.actual_end) : Infinity,
        index
      });
    });
    return result;
  ...
  function layout(data, view, asOf) {
    ...
    function group(item, owner) {
      ...
      if (!groups.has(ref)) groups.set(ref, {
        id: ref,
        label: view.mode === 'batch' ? ref : labels.get(ref) || views[view.mode] + '未填写',
        members: new Map()
      });
      const members = groups.get(ref).members;
      if (!members.has(item.task.task_ref)) members.set(item.task.task_ref, {
        item,
        baseline: false,
        reports: [],
        legacy: [],
    ...
    items.forEach(item => {
      ...
      const e = item.execution;
      if (!e) return;
      e.reports.forEach(report => group(item, {
        machine_ref: report.actual_machine_ref,
        operator_ref: report.actual_operator_ref
      }).reports.push(report));
      e.legacy_facts.forEach(fact => {
        if (fact.actual_machine_ref || fact.actual_operator_ref) group(item, {
          machine_ref: fact.actual_machine_ref,
          operator_ref: fact.actual_operator_ref
        }).legacy.push(fact);
      });
    ...
    groups.forEach(g => {
      ...
      g.members.forEach(member => {
        const {
          ...
          ts = tracks(member.reports);
        if (!ts.length) ts.push([]);
        ts.forEach((reports, i) => {
          const row = {
            key: g.id + ':' + item.task.task_ref + ':' + i,
            group: g,
            item,
            reports,
            baseline: i === 0 && member.baseline,
            kind: 'actual',
            height: 88,
            track: i + 1,
        ...
        if (member.remaining) append({
          key: g.id + ':' + item.task.task_ref + ':remaining',
          group: g,
          item,
          reports: [],
          baseline: false,
    ...
    return {
      ...
      locations,
      reportLocations,
      executionAvailable: !!data.availability && data.availability.state === 'available',
      start: axis.start,
      end: axis.end,
      asOf: instant(asOf)
    };
  ...
  function visibleRows(rows, top, bottom) {
    ...
    while (low < high) {
      ...
      if (rows[mid].top + rows[mid].height <= top) low = mid + 1;else high = mid;
    }
    const result = [];
    for (let at = low; at < rows.length && rows[at].top < bottom; at++) result.push(rows[at]);
    return result;
  }
  function marks(row) {
    const result = [],
      t = row.item.task,
      e = row.item.execution;
    if (row.baseline) {
      const point = window.PointContract.isPoint(t);
      result.push({
        key: 'plan',
        kind: point ? 'plan-point' : 'plan',
        start: instant(t.start),
        end: instant(t.end),
        y: point ? 64 : 74,
        height: point ? 24 : 7
      });
    ...
    row.reports.forEach(report => {
      ...
      result.push({
        ...
        kind: point ? 'point' : 'actual',
        report,
        start: instant(report.actual_start),
        end: instant(report.actual_end || report.actual_start),
        y: point ? 20 : 14,
        height: point ? 24 : 38
      });
    });
    if (row.kind === 'remaining' && e.remaining_plan) result.push({
      key: 'remaining',
      kind: 'remaining',
      start: instant(e.remaining_plan.start),
      end: instant(e.remaining_plan.end),
      y: 14,
      height: 34
    });
    return result;
  ...
  function ticks(model, width, left, viewport) {
    ...
    const low = model.start + left / width * (model.end - model.start),
      high = model.start + (left + viewport) / width * (model.end - model.start),
      result = [];
    for (let at = Math.ceil(low / step) * step; at < high; at += step) result.push({
      at,
      x: (at - model.start) / (model.end - model.start) * width,
      label: wire(at)
    });
    return result;
  }
  function describe(item, labels, report) {
    ...
    result.push(states[e.execution_state] + ' · 已知完成 ' + number(e.known_completed_quantity) + ' 件', '整道实际完工：' + time(e.confirmed_finish), '剩余数量：' + number(e.remaining_quantity), e.completion_basis === 'legacy_finish_event' ? '旧完工事件确认完成；旧数量与工时可能未记录' : '完成依据：逐次执行投影');
    if (report) result.push('本次报工：' + report.report_no, '实际：' + time(report.actual_start) + ' → ' + (report.actual_end ? time(report.actual_end) : '本次结束未填写'), '本次数量：' + number(report.completed_quantity) + ' · 有效工时：' + number(report.effective_processing_hours) + 'h', '实际资源：' + (labels.get(report.actual_machine_ref) || '设备未填写') + ' / ' + (labels.get(report.actual_operator_ref) || '人员未填写'), '备注：' + (report.remark || '未填写'));
    return result;
  }
  ...
  function metrics(data) {
    ...
    return {
      ...
      reported: data.items.filter(item => !['complete', 'unreported'].includes(item.execution.execution_state)).length,

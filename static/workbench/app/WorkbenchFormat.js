(function () {
  ...
  function invalid(kind) {
    throw new TypeError('无法显示：' + kind + '格式不正确。');
  }
  function parts(value, dateOnly) {
    ...
    if (year < 1 || month < 1 || month > 12 || day < 1 || day > days[month - 1] || Number(match[4] || 0) > 23 || Number(match[5] || 0) > 59 || Number(match[6] || 0) > 59) return invalid('日期时间');
    return match;
  }
  ...
  function instant(value, {
    ...
    return String(year).padStart(4, '0') + '-' + pad(beijing.getUTCMonth() + 1) + '-' + pad(beijing.getUTCDate()) + ' ' + pad(beijing.getUTCHours()) + ':' + pad(beijing.getUTCMinutes()) + (seconds ? ':' + pad(beijing.getUTCSeconds()) : '');
  }
  ...
  function numberOptions(options, kind) {
    const source = options === undefined ? {} : typeof options === 'number' ? {
      digits: options
    } : options;
  ...
  function percent(ratio, options) {
    ...
    const {
      digits,
      trim
    } = numberOptions(options, '百分比');
    if (typeof ratio !== 'number' || !Number.isFinite(ratio)) return invalid('百分比');
    return signedZero(ratio, digits + 2).toLocaleString('zh-CN', {
      style: 'percent',
      minimumFractionDigits: trim ? 0 : digits,
      maximumFractionDigits: digits
    });
  ...
  window.WorkbenchFormat = Object.freeze({
    dateTime,
    date,
    instant,
    number,

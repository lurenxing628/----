  ...
  function create() {
    ...
    return {
      ...
      async export(context, signal) {
        ...
        if (!output || !output.blob || !output.blob.size || output.contentType.split(';')[0] !== 'text/csv' || !/^attachment;/i.test(output.disposition)) throw window.APSResourceContract.failure('实际甘特下载不是有效的 CSV 附件。');
        return output;
      }
    };
  }

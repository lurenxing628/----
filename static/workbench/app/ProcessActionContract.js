(function () {
  ...
  function listContext(request) {
    if (!request || !token(request.snapshot_ref) || !Number.isSafeInteger(request.page_size) || request.page_size < 1 || request.page_size > 200) throw C.failure('请先重新读取零件列表，再进行操作。');
    return {
  ...
  function createReason(context, source) {
    ...
    if (!C.object(context) || !token(context.write_token) || !C.object(context.capabilities) || context.capabilities['process.create'] !== true) return '新增许可尚未取得，请重新读取列表。';
    return '';
  }
  ...
  function receipt(result, intent, expectedRefs) {
    ...
    if (intent.kind === 'process' && intent.action === 'create' && intent.ref === null) {
      if (!ref(data.entity_ref) || typeof data.business_code !== 'string' || !data.business_code || !C.object(data.workflow) || data.workflow.origin !== 'managed' || data.workflow.ready !== false || data.workflow.stage !== 'route') throw C.failure('新增回执缺少原零件或确认状态，不能按图号打开另一条记录。');
      ...
      if (!Number.isSafeInteger(data.deleted_count) || data.deleted_count < 1 || !Array.isArray(data.rows) || data.rows.length !== data.deleted_count || !data.rows.every(row => C.object(row) && ref(row.entity_ref) && row.result === 'committed') || new Set(data.rows.map(row => row.entity_ref)).size !== data.rows.length || expectedRefs && (data.rows.length !== expectedRefs.length || data.rows.some((row, index) => row.entity_ref !== expectedRefs[index]))) throw C.failure('删除回执没有逐项确认全部原零件，请继续核实；未视为部分成功。');

(function () {
  ...
  function restore(scenarioRef) {
    ...
    C.check(['delivery', 'capacity', 'history', 'adoptions', 'issues', 'tasks', 'unplanned'].includes(saved.tab) && window.TrialAdoptionHistoryAPI.states.includes(saved.status) && Number.isSafeInteger(saved.page) && saved.page > 0 && [10, 20, 50].includes(saved.size) && (!saved.snapshot_ref || /^[A-Za-z0-9_-]{32}$/.test(saved.snapshot_ref)), '采用记录的查看状态恢复不了，请点「刷新采用记录」。');
    return saved;
  }
  ...
  function openPlan(scenarioRef, planRef) {
    ...
    C.check(entry && entry.view === 'trial' && entry.context && entry.context.scenario_ref === scenarioRef && node, '现在不能跳转到正式计划，页面没有切换。请刷新后重试。');

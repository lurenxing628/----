(function () {
  ...
  function WorkbenchSelectMenu({
    ...
    const current = enabled.some(row => row.index === active) ? active : (enabled[0] || {
      index: -1
    }).index;
    React.useLayoutEffect(() => {
      const optionId = id + '-' + current;
      if (current >= 0) owner.setAttribute('aria-activedescendant', optionId);else owner.removeAttribute('aria-activedescendant');
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, (filterable || typed) && /*#__PURE__*/React.createElement("div", {
      className: "wb-select-tools"
    }, filterable && /*#__PURE__*/React.createElement("input", {
      ref: box,
      className: "wb-popup-search",
      type: "search",
      value: filter,
      "aria-label": '筛选' + B.label(owner) + '选项',
      ...
      className: "wb-select-typed",
      role: "status"
    }, "\u6B63\u5728\u8F93\u5165 ", typed)), /*#__PURE__*/React.createElement("div", {
      id: id,
      role: "listbox",
      ...
      role: "status"
    }, rows.length ? '当前筛选没有匹配项' : '暂无选项'), listed.map((row, index) => /*#__PURE__*/React.createElement(React.Fragment, {
      key: row.index
    }, row.group && (!index || listed[index - 1].group !== row.group) && /*#__PURE__*/React.createElement("div", {

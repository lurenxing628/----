(function () {
  ...
  function FieldWorkspace({
    ...
    const [nextDraft, setNextDraft] = React.useState(null),
    ...
    const guardOwner = window.WorkbenchGuards.useDirtyGuard({
      dirty: !!(currentDraft && currentDraft.retained.dirty && command.phase !== 'done'),
      locked: command.locked,
      message: '当前报工有未保存草稿，继续会丢弃这些内容。'
    });
    window.WorkbenchGuards.useDirtyGuard({
      dirty: Array.from(drafts.current.values()).some(value => value.retained.dirty && (!editor || value.editor.taskRef !== editor.taskRef)),
      message: '其他任务有临时保留的报工草稿，离开页面会丢弃这些内容。'
    });
    const retainDraft = React.useCallback(retained => {
      if (!editor) return;
      drafts.current.set(editor.taskRef, {
        editor,
        retained
      });
      renderDrafts();
    ...
    function continueReady(task) {
      ...
      setNextDraft(null);
      setNotice(reason || '已核对最新报工，已打开新的报工草稿。');
      if (!reason) setEditor({
    ...
    return /*#__PURE__*/React.createElement("section", {
    ...
    }, "\u73B0\u573A\u8BB0\u5F55"), /*#__PURE__*/React.createElement("span", {
      className: "field-note wb-page-context"
      ...
      error: read.error
    }), /*#__PURE__*/React.createElement(window.FieldTable, {
      ...
      onOpen: openTask
    }), data && /*#__PURE__*/React.createElement(window.WorkbenchListControls.Pager, {
      page: data.page,
      sizes: [10, 20, 50, 100],
      unit: "\u9053\u5DE5\u5E8F",
      label: "\u73B0\u573A",
    ...
    }), selectedTask && /*#__PURE__*/React.createElement(window.FieldDetail, {
      key: selectedTask.task_ref,
      adapter: api,
      taskRef: selectedTask.task_ref,
      scope: effectiveScope,
      snapshot: snapshot,
      revision: revision,
      command: command,
      editor: editor,
      retained: drafts.current.get(selectedTask.task_ref)?.retained,
      onDraft: retainDraft,
      nextDraft: nextDraft,
      onContinueReady: continueReady,
      onEdit: value => {

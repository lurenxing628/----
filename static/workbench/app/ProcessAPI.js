(function () {
  ...
  function create(resourceAdapter) {
    const base = window.APSResourceAPI.create('process'),
    ...
    function tableRequest(scope) {
      ...
      ['sort', 'column_filters'].forEach(key => {
        if (Array.isArray(value[key]) || window.APSResourceContract.object(value[key])) value[key] = JSON.stringify(value[key]);
      });
      return value;
    }
    ...
    return {
      ...
      list(kind, scope, signal) {
        return base.query(part(kind), tableRequest(scope), signal);
      },
      facets(kind, request, signal) {
        ...
        return base.query(facetPath(kind, request, false), {
          ...query,
      ...
      facetSelection(kind, request, signal) {
        ...
        return base.query(facetPath(kind, request, true), {
          ...query,
          scope: JSON.stringify(scope)
        }, signal);
      },
      bulkPreview(body, signal) {
        return base.preview('process/parts/bulk-preview', body, signal);
      },

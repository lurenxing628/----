(function () {
  ...
  function parameters(scope, allowed) {
    if (!object(scope) || Object.prototype.toString.call(scope) !== '[object Object]' || Reflect.ownKeys(scope).some(key => !allowed.includes(key) || !Object.prototype.propertyIsEnumerable.call(scope, key))) throw C.failure('查询条件不对，这次没有读取计划。筛选条件没有变，请点「刷新」重试。');
  ...
  function catalogScope(scope = {}) {
    ...
    if (!['history', 'scenario'].includes(query.collection) || !Number.isSafeInteger(query.size) || query.size < 1 || query.size > 50) throw C.failure('计划列表只能按历史版本或试调方案查看，每页 1 至 50 条。这次没有读取，请点「刷新」重试。');
    return query;
  }
  function workspaceScope(planRef, scope = {}) {
    ...
    if (start !== end || start && (!localTime(query.range_start) || !localTime(query.range_end) || query.range_start >= query.range_end)) throw C.failure('读取的开始时间和结束时间要一起填，开始必须早于结束。请按 2026-09-13 08:30 这样填。');
    return query;
  }
  function exportScope(planRef, scope) {
    const query = parameters(scope, ['format', 'range_start', 'range_end', 'snapshot_ref']);
    if (!['csv', 'xlsx'].includes(query.format) || !own(query, 'snapshot_ref')) throw C.failure('导出没有拿到文件格式或当前数据版本，这次没有导出。请刷新后重新点「导出」。');
    const {
      format,
      ...read
    } = query;
  ...
  function download(value, format) {
    ...
    if (!exact(value, ['blob', 'contentType', 'disposition']) || !value.blob || !(value.blob.size > 0) || typeof value.contentType !== 'string' || value.contentType.split(';')[0] !== expected || !/^attachment;/i.test(value.disposition)) throw C.failure('导出的文件和所选格式不一致，没有保存。请重新点「导出」。');
    return value;
  }
  ...
  function catalog(result, scope = {}) {
    ...
    const d = result.data,
      p = d && d.page;
    if (!exact(d, ['plans', 'page']) || !Array.isArray(d.plans) || !d.plans.every(plan) || !uniqueRefs(d.plans, 'plan_ref') || !exact(p, ['collection', 'size', 'unit', 'has_more', 'next_cursor']) || p.collection !== query.collection || p.size !== query.size || p.unit !== (query.collection === 'history' ? 'version' : 'scenario') || typeof p.has_more !== 'boolean' || (p.has_more ? !token(p.next_cursor) || !d.plans.length || p.next_cursor === query.cursor : p.next_cursor !== null) || d.plans.some(row => row.kind === 'scenario' !== (query.collection === 'scenario')) || d.plans.filter(row => row.is_current_official).length > 1) throw C.failure('读到的计划列表、类型或分页跟这次查询不一致，没有当作完整列表使用。请点「刷新」重试。');

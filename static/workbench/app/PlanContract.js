(function () {
  ...
  function span(value, pointEnd = false) {
    ...
    return exact(value, ['start', 'end', ...(inclusive ? ['end_inclusive'] : [])]) && (!inclusive || value.end_inclusive === true) && localTime(value.start) && localTime(value.end) && value.start <= value.end;
  }
  function parameters(scope, allowed) {
    if (!object(scope) || Object.prototype.toString.call(scope) !== '[object Object]' || Reflect.ownKeys(scope).some(key => !allowed.includes(key) || !Object.prototype.propertyIsEnumerable.call(scope, key))) throw C.failure('计划读取含未知参数或无效范围，未忽略筛选条件。');
  ...
  function catalogScope(scope = {}) {
    ...
    if (!['history', 'scenario'].includes(query.collection) || !Number.isSafeInteger(query.size) || query.size < 1 || query.size > 50) throw C.failure('计划目录必须选择历史或场景，每页为 1 至 50 个版本或场景。');
    return query;
  }
  function workspaceScope(planRef, scope = {}) {
    ...
    const start = own(query, 'range_start'),
      end = own(query, 'range_end');
    if (start !== end || start && (!localTime(query.range_start) || !localTime(query.range_end) || query.range_start >= query.range_end)) throw C.failure('计划起止时间必须同时提供有效的工厂本地时间，且起点早于终点。');
    return query;
  }
  function exportScope(planRef, scope) {
    const query = parameters(scope, ['format', 'range_start', 'range_end', 'snapshot_ref']);
    if (!['csv', 'xlsx'].includes(query.format) || !own(query, 'snapshot_ref')) throw C.failure('导出必须提供格式和当前工作区的读取快照。');
    const {
      format,
      ...read
    } = query;
  ...
  function download(value, format) {
    ...
    if (!exact(value, ['blob', 'contentType', 'disposition']) || !value.blob || !(value.blob.size > 0) || typeof value.contentType !== 'string' || value.contentType.split(';')[0] !== expected || !/^attachment;/i.test(value.disposition)) throw C.failure('计划导出响应不是所选格式的附件。');
    return value;
  }
  function envelope(result, query) {
    if (!exact(result, ['ok', 'schema_version', 'data', 'meta', 'warnings']) || result.ok !== true || result.schema_version !== 1 || !exact(result.meta, ['request_ref', 'source', 'time_basis', 'snapshot_ref', 'as_of']) || result.meta.source !== 'production' || result.meta.time_basis !== 'factory_local' || !token(result.meta.snapshot_ref) || !token(result.meta.request_ref) || !localTime(result.meta.as_of) || !issues(result.warnings)) throw C.failure('计划读取协议不完整或不是生产数据，未使用样例替代。');
    if (own(query, 'snapshot_ref') && result.meta.snapshot_ref !== query.snapshot_ref) throw C.failure('计划快照与请求不一致，未自动切换到新数据。');
  }
  function plan(value) {
    if (!exact(value, ['plan_ref', 'version', 'kind', 'is_current_official', 'display_name', 'completeness', 'capabilities', 'blocked_reasons']) || !nullableRef(value.plan_ref) || !(value.version === null || int64(value.version)) || !['official', 'candidate', 'scenario'].includes(value.kind) || typeof value.is_current_official !== 'boolean' || !label(value.display_name) || !['complete', 'partial', 'invalid', 'unknown'].includes(value.completeness) || !exact(value.capabilities, ['view', 'export', 'edit_draft', 'adopt', 'report_actual']) || typeof value.capabilities.view !== 'boolean' || value.capabilities.export !== value.capabilities.view || !['edit_draft', 'adopt', 'report_actual'].every(key => value.capabilities[key] === false) || !issues(value.blocked_reasons)) return false;
    const view = value.capabilities.view;
    return (view ? ref(value.plan_ref) && int64(value.version) && value.blocked_reasons.length === 0 && value.completeness !== 'invalid' : value.blocked_reasons.length > 0) && (!value.is_current_official || value.kind === 'official' && view);
  }
  ...
  function catalog(result, scope = {}) {
    ...
    const d = result.data,
      p = d && d.page;
    if (!exact(d, ['plans', 'page']) || !Array.isArray(d.plans) || !d.plans.every(plan) || !uniqueRefs(d.plans, 'plan_ref') || !exact(p, ['collection', 'size', 'unit', 'has_more', 'next_cursor']) || p.collection !== query.collection || p.size !== query.size || p.unit !== (query.collection === 'history' ? 'version' : 'scenario') || typeof p.has_more !== 'boolean' || (p.has_more ? !token(p.next_cursor) || !d.plans.length || p.next_cursor === query.cursor : p.next_cursor !== null) || d.plans.some(row => row.kind === 'scenario' !== (query.collection === 'scenario')) || d.plans.filter(row => row.is_current_official).length > 1) throw C.failure('计划目录、身份或分页与请求不一致，未作为完整目录使用。');
    // A history page counts versions, not role entries; null versions cannot be grouped by guessed identity.
    const units = query.collection === 'scenario' ? d.plans.length : new Set(d.plans.filter(row => row.version !== null).map(row => String(row.version))).size + d.plans.filter(row => row.version === null).length;
  ...
  function resourceDirectory(data) {
    ...
    for (const row of data.resources) {
      if (!exact(row, ['kind', 'ref', 'business_code', 'label']) || !['machine', 'operator', 'supplier'].includes(row.kind) || !ref(row.ref) || !label(row.business_code) || !nullableLabel(row.label)) return false;
  ...
  function baseline(value, data) {
    ...
    if (value.state === 'unavailable') return exact(value, ['state', 'reason_code', 'reason', 'baseline_plan', 'items', 'item_count', 'items_complete']) && ['not_recorded', 'baseline_binding_invalid', 'baseline_unavailable', 'scenario_unavailable', 'baseline_task_invalid', 'no_adoption_baseline', 'adoption_evidence_missing', 'adoption_source_archived', 'adoption_snapshot_invalid', 'adoption_baseline_archived', 'adoption_reference_invalid', 'adoption_baseline_drift', 'adoption_plan_drift'].includes(value.reason_code) && label(value.reason) && value.baseline_plan === null && Array.isArray(value.items) && value.items.length === 0 && value.item_count === 0 && value.items_complete === false;
    const fields = ['start', 'end', 'machine_ref', 'operator_ref'],
      s = value.comparison_scope;
    if (!exact(value, ['state', 'reason_code', 'reason', 'basis', 'baseline_plan', 'comparison_scope', 'compared_fields', 'items', 'item_count', 'items_complete']) || value.state !== 'available' || value.reason_code !== null || value.reason !== null || !['scenario_base', 'candidate_adoption', 'trial_adoption'].includes(value.basis) || data.plan.kind !== (value.basis === 'scenario_base' ? 'scenario' : 'official') || !plan(value.baseline_plan) || !value.baseline_plan.capabilities.view || value.basis !== 'scenario_base' && (value.baseline_plan.kind !== 'official' || value.baseline_plan.is_current_official) || value.baseline_plan.kind === 'scenario' || value.baseline_plan.plan_ref === data.plan.plan_ref || !exact(s, ['scope', 'selection', 'boundary', 'alignment', 'time_basis', 'task_times']) || !same(s.scope, data.scope) || s.selection !== 'either_side_overlap' || s.boundary !== 'half_open' || s.alignment !== 'operation_ref' || s.time_basis !== 'factory_local' || s.task_times !== 'unclipped' || JSON.stringify(value.compared_fields) !== JSON.stringify(fields) || !Array.isArray(value.items) || value.item_count !== value.items.length || value.item_count > 20000 || value.items_complete !== true || !uniqueRefs(value.items, 'operation_ref')) return false;
    ...
    for (const row of value.items) {
      ...
      const before = row.before,
        after = row.after;
      if (before === null && after === null || before !== null && (!task(before, value.baseline_plan.plan_ref) || before.operation_ref !== row.operation_ref) || after !== null && (!task(after, data.plan.plan_ref) || after.operation_ref !== row.operation_ref)) return false;
      ...
      if (row.change !== change || JSON.stringify(row.changed_fields) !== JSON.stringify(changed) || row.before_in_scope !== inScope(before, data.scope) || row.after_in_scope !== inScope(after, data.scope) || !row.before_in_scope && !row.after_in_scope) return false;
  ...
  function projectionIssues(rows, assignment = false) {
    return Array.isArray(rows) && rows.every(row => {
      ...
      return exact(row, ['code', 'message', ...extra]) && label(row.code) && label(row.message) && (assignment ? count(row.operation_count) && row.operation_count > 0 : !extra.length || day(row.policy_date));
    });
  }
  ...
  function intervals(rows, time, extra, validate) {
    return Array.isArray(rows) && rows.every((row, index) => exact(row, ['start', 'end', ...extra]) && localTime(row.start) && localTime(row.end) && row.start < row.end && row.start >= time.range_start && row.end <= time.range_end && (!index || rows[index - 1].end <= row.start) && validate(row));
  }
  function calendarRow(row, time, resource = false) {
    ...
    if (!exact(row, ['state', 'basis', 'windows', 'issues', ...measures, ...extra]) || !projectionIssues(row.issues) || (resource ? !['machine', 'operator'].includes(row.kind) || !ref(row.resource_ref) || !nullableLabel(row.label) || row.basis !== (row.kind === 'machine' ? 'global_calendar_machine_availability' : 'personal_or_operator_shift_calendar') : row.basis !== 'global_calendar')) return false;
    ...
    if (!intervals(row.windows, time, ['policy_date', 'allow_normal', 'allow_urgent', 'efficiency', 'provenance'], window => day(window.policy_date) && typeof window.allow_normal === 'boolean' && typeof window.allow_urgent === 'boolean' && number(window.efficiency) && ['personal_calendar', 'work_calendar', 'domain_default', 'work_calendar_operator_shift', 'domain_default_operator_shift'].includes(window.provenance))) return false;
    return !resource || own(row, 'status') && (row.status === 'active' || row.windows.length === 0) && number(row.downtime_scope_hours) && number(row.downtime_available_hours) && intervals(row.downtime_windows, time, [], () => true);
  }
  ...
  function calendar(value, data) {
    if (!exact(value, ['state', 'plan_ref', 'time_scope', 'global', 'resources', 'issues']) || !states.includes(value.state) || value.plan_ref !== data.plan.plan_ref || !same(value.time_scope, data.time_scope) || !projectionIssues(value.issues) || !calendarRow(value.global, data.time_scope)) return false;
  ...
  function deliveryItem(row) {
    ...
    if (!exact(row, ['batch_ref', 'batch_id', 'part_no', 'part_label', 'planned_finish', 'partial_planned_finish', 'due_date', 'delivery_deadline_exclusive', 'risk', 'is_overdue', 'delay_hours', 'delay_days', 'completeness', 'schedule_complete', ...counts, 'issues']) || !ref(row.batch_ref) || !label(row.batch_id) || !nullableLabel(row.part_no) || !nullableLabel(row.part_label) || !nullableTime(row.planned_finish) || !nullableTime(row.partial_planned_finish) || !(row.due_date === null || day(row.due_date)) || !nullableTime(row.delivery_deadline_exclusive) || !['overdue', 'on_time', 'unknown'].includes(row.risk) || !['complete', 'incomplete', 'unknown'].includes(row.completeness) || typeof row.schedule_complete !== 'boolean' || !counts.every(key => count(row[key])) || row.invalid_task_count > row.task_count || row.scheduled_operation_count > row.task_count || row.scheduled_operation_count + row.unscheduled_operation_count !== row.operation_count || !Array.isArray(row.issues) || !row.issues.every(label) || new Set(row.issues).size !== row.issues.length) return false;
    if (row.schedule_complete ? row.planned_finish === null || row.partial_planned_finish !== null || row.operation_count === 0 || row.unscheduled_operation_count !== 0 || row.invalid_task_count !== 0 : row.planned_finish !== null) return false;
    ...
    return row.schedule_complete && row.delivery_deadline_exclusive !== null && typeof row.is_overdue === 'boolean' && row.is_overdue === row.planned_finish >= row.delivery_deadline_exclusive && row.risk === 'overdue' === row.is_overdue && number(row.delay_hours) && number(row.delay_days) && (row.is_overdue || row.delay_hours === 0 && row.delay_days === 0);
  }
  function delivery(value, data) {
    if (!exact(value, ['state', 'plan_ref', 'scope', 'items', 'batch_count', 'items_complete', 'completeness', 'basis']) || !states.includes(value.state) || value.plan_ref !== data.plan.plan_ref || !same(value.scope, data.scope) || !Array.isArray(value.items) || value.batch_count !== value.items.length || value.items_complete !== true || !uniqueRefs(value.items, 'batch_ref') || !uniqueRefs(value.items, 'batch_id') || !value.items.every(deliveryItem)) return false;
    const basis = {
      kind: 'planned_delivery',
      time_basis: 'factory_local',
      batch_selection: data.scope.range_start === null ? 'plan_task_batches' : 'overlap_or_bad_time_batches',
      completion_scope: 'full_selected_plan',
      operation_scope: 'current_batch_operations',
      due_boundary: 'next_day_exclusive',
      actual_completion: 'not_evaluated',
      actual_delivery: 'not_evaluated'
    };
    if (!same(value.basis, basis)) return false;
    const batches = new Set(data.tasks.map(row => row.batch_id));
    const completeness = value.items.some(row => row.completeness === 'incomplete') ? 'incomplete' : !value.items.length || value.items.some(row => row.completeness === 'unknown') ? 'unknown' : 'complete';
    const known = value.items.filter(row => row.risk !== 'unknown').length;
    return batches.size === value.items.length && value.items.every(row => batches.has(row.batch_id)) && value.completeness === completeness && value.state === (known === value.items.length ? 'available' : known ? 'partial' : 'unavailable');
  }
  function workspaceSpans(data, query) {
    const time = data.time_scope;
    return span(data.plan_span, true) && exact(time, ['range_start', 'range_end', 'selection', 'boundary', 'time_basis']) && time.range_start === (own(query, 'range_start') ? query.range_start : data.plan_span.start) && time.range_end === (own(query, 'range_end') ? query.range_end : data.plan_span.end) && time.selection === 'overlap' && time.boundary === 'half_open' && time.time_basis === 'factory_local';
  }
  function taskSpans(data, query) {
    const explicit = own(query, 'range_start'),
      inclusive = data.plan_span.end_inclusive === true;
    ...
    const whole = !explicit || query.range_start <= data.plan_span.start && (query.range_end > data.plan_span.end || !inclusive && query.range_end === data.plan_span.end);
    if (!data.tasks.length) return data.task_span === null && !whole;
    if (!span(data.task_span)) return false;
    let start = data.tasks[0].start,
      end = data.tasks[0].end;
    for (const row of data.tasks) {
      if (row.start < start) start = row.start;
      if (row.end > end) end = row.end;
    }
    return data.task_span.start === start && data.task_span.end === end && (!whole || start === data.plan_span.start && end === data.plan_span.end);
  }
  function workspace(result, planRef, scope = {}) {
    const query = workspaceScope(planRef, scope);
    envelope(result, query);
    const d = result.data;
    if (!exact(d, ['plan', 'scope', 'time_scope', 'plan_span', 'task_span', 'tasks', 'task_count', 'tasks_complete', 'resources', 'projections']) || !plan(d.plan) || !d.plan.capabilities.view || d.plan.plan_ref !== planRef || !exact(d.scope, ['source', 'kind', 'plan_ref', 'range_start', 'range_end']) || d.scope.source !== 'production' || d.scope.kind !== 'plan_workspace' || d.scope.plan_ref !== planRef || d.scope.range_start !== (own(query, 'range_start') ? query.range_start : null) || d.scope.range_end !== (own(query, 'range_end') ? query.range_end : null) || !workspaceSpans(d, query) || !Array.isArray(d.tasks) || !Number.isSafeInteger(d.task_count) || d.task_count !== d.tasks.length || d.task_count > 10000 || d.tasks_complete !== true || !d.tasks.every(row => task(row, planRef, d.plan_span, own(query, 'range_start') ? d.time_scope : null)) || !uniqueRefs(d.tasks, 'task_ref') || !taskSpans(d, query) || !resourceDirectory(d) || !exact(d.projections, projections) || !baseline(d.projections.baseline, d) || !calendar(d.projections.calendar, d) || !occupancy(d.projections.occupancy, d) || !delivery(d.projections.delivery_risks, d) || !window.PlanProcessOrder.validate(d.projections.process_order, d)) throw C.failure('计划任务、范围或投影协议不完整或串源，未作为完整结果使用。');
    return result;
  }
  window.APSPlanContract = {
    catalogScope,
    workspaceScope,
    exportScope,
    catalog,

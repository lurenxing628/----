(function () {
  ...
  function pending() {
    ...
    try {
      ...
      throw new Error('无法读取试调请求恢复记录，写入已暂停。');
    }
    C.check(value === null || validKey(value), '试调请求恢复记录损坏，未删除或覆盖原记录。');
    return value;
  }
  function reserve() {
    ...
    try {
      ...
      throw new Error('无法保存试调请求恢复记录，本次未发送。');
    }
    C.check(pending() === key, '试调请求恢复记录写入失败，本次未发送。');
    return key;
  }
  function clear(key) {
    ...
    try {
      ...
      throw new Error('原请求已核实，但恢复记录无法清理，请重新查询。');
    }
  }

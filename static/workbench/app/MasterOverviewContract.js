(function () {
  ...
  const columns = {
    entities: common.concat([['status', '检查状态', 92], ['filled_fields', '已填项', 80], ['checked_fields', '检查项', 80], ['relation_count', '关联项', 76], ['issue_count', '待维护项', 82], ['summary', '检查结果', 214]]),
  ...
  function fail(message) {
    throw new Error(message || '读到的基础资料不完整，请刷新后重试。');
  }
  ...
  function target(value) {
    ...
    if (value.view === 'batches') {
      if (!ref(c.entity_ref)) fail('这条批次记录已失效，请重新选择。');
      return value;
    }
  ...
  function page(value, rows, size) {
    if (!object(value) || !count(value.total) || !Number.isSafeInteger(value.number) || value.number < 1 || !Number.isSafeInteger(value.pages) || value.pages !== Math.max(1, Math.ceil(value.total / value.size)) || value.number > value.pages || value.size !== size || rows.length !== Math.min(size, Math.max(0, value.total - (value.number - 1) * size))) fail('基础资料的分页数量对不上，请刷新后重试。');
  }
  function list(result, expected, snapshot) {
    ...
    domains.forEach(([id], index) => {
      ...
      if (!object(domain) || domain.id !== id || typeof domain.loaded !== 'boolean' || (domain.loaded ? !count(domain.count) || !count(domain.attention) || !count(domain.unknown) || domain.attention > domain.count || domain.unknown > domain.count : domain.count !== null || domain.attention !== null || domain.unknown !== null)) fail('资料类别的数量对不上，请刷新后重试。');
    });
    const stats = overview.stats;
    if (['entities', 'issues', 'affected', 'known_relation_pairs'].some(key => !count(stats[key])) || !(stats.relations === null || count(stats.relations)) || stats.entities !== overview.domains.reduce((sum, item) => sum + (item.loaded ? item.count : 0), 0) || stats.affected > stats.entities || stats.issues < stats.affected || stats.affected !== overview.domains.reduce((sum, item) => sum + (item.attention || 0), 0) || stats.relations !== null && stats.relations !== stats.known_relation_pairs || !overview.complete && stats.relations !== null || overview.complete !== overview.domains.every(item => item.loaded)) fail('资料总览的数量对不上，请刷新后重试。');
    return result;
  }
  function detail(result, expected, selection, section, snapshot) {
    envelope(result);
    const data = result.data;
    if (canonical(scope(data.scope)) !== canonical(scope(expected)) || result.meta.snapshot_ref !== snapshot || data.section !== section || !object(data.entity) || data.entity.ref !== selection.entity_ref || data.entity.domain !== selection.domain || !Array.isArray(data.rows) || !object(data.counts) || ['issues', 'relations', 'fields'].some(key => !count(data.counts[key]))) fail('详情和所选资料对不上，页面没有切换。请刷新后重试。');

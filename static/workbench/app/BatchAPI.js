  ...
  function create() {
    ...
    return {
      ...
      list(kind, scope, signal) {
        ...
        return api.preview(root + '/query', scope, signal);
      },
      ...
      selection(scope, signal) {
        return api.preview(root + '/selection', scope, signal);
      },
      ...
      exportPreview(selection, scope, refs, signal) {
        return api.preview(root + '/export-preview', {
          selection,
          scope,
          ...(selection === 'selected' ? {
            refs
      ...
      downloadTemplate(signal) {
        return api.download(root + '/template', {}, signal);
      },
      downloadExport(exportRef, signal) {
        return api.download(root + '/export', {
          export_ref: exportRef
        }, signal);
      },
      preview(action, ref, input, scope, snapshot, signal) {
        if (action === 'bulk') return api.preview(root + '/bulk-preview', {
          input,
          scope,

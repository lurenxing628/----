(function () {
  ...
  function create(kind = 'material', label = '物料') {
    ...
    function envelope(raw) {
      ...
      if (!C.object(raw) || raw.ok !== true || raw.schema_version !== 1 || !C.object(raw.data) || !C.object(raw.meta) || !['production', 'demo'].includes(raw.meta.source) || raw.meta.time_basis !== 'factory_local' || typeof raw.meta.snapshot_ref !== 'string' || !raw.meta.snapshot_ref || !Array.isArray(raw.warnings)) throw C.failure(label + '操作读取协议不完整，未使用样例替代。');
      return raw;
    }
    function preview(raw, mode, expected) {
      const result = envelope(raw),
        data = result.data;
      if (!token(data.preview_ref) || data.operation !== kind + (mode === 'import' ? '.import' : '.bulk_delete') || data.commit_policy !== 'atomic' || typeof data.can_confirm !== 'boolean' || !C.object(data.summary) || !Array.isArray(data.rows) || !C.object(data.write_context) || !C.object(data.write_context.capabilities) || !token(data.write_context.write_token) || !Number.isFinite(Date.parse(data.expires_at))) throw C.failure('预检结果不完整，本批未提交，请重新预检。');
      if (!Object.keys(results).every(key => count(data.summary[key])) || Object.keys(results).reduce((sum, key) => sum + data.summary[key], 0) !== data.rows.length) throw C.failure('预检统计与明细不一致，本批未提交。');
      ...
      if (!data.rows.every(validRow) || new Set(data.rows.map(row => row.row)).size !== data.rows.length || !Object.keys(results).every(key => data.rows.filter(row => row.result === key).length === data.summary[key]) || data.can_confirm && data.summary.rejected !== 0) throw C.failure('预检行内容不完整或统计不一致，本批未提交。');
      if (mode === 'import' && (data.format !== expected || data.mode !== 'upsert' || data.template_version !== 1 || typeof data.file_sha256 !== 'string' || !/^[0-9a-f]{64}$/.test(data.file_sha256))) throw C.failure('导入预览的文件格式或增量模式不一致，本批未提交。');
      if (mode === 'bulk' && (!Array.isArray(expected) || data.rows.length !== expected.length || !data.rows.every((row, index) => row.entity_ref === expected[index]))) throw C.failure('删除预览与明确选中的' + label + '不一致，本批未提交。');
      return result;
    }
    function exportPreview(raw, selection) {
      const result = envelope(raw),
        data = result.data;
      if (!token(data.export_ref) || data.selection !== selection || !count(data.row_count) || !Array.isArray(data.formats) || !data.formats.every(value => ['csv', 'xlsx'].includes(value)) || !Number.isFinite(Date.parse(data.expires_at))) throw C.failure('导出预检内容不完整，未开始下载。');
      return result;
    }
    ...
    function selection(request) {
      if (!Array.isArray(request.refs) || request.refs.some(ref => typeof ref !== 'string' || !/^[0-9a-f]{48}$/.test(ref)) || new Set(request.refs).size !== request.refs.length) throw C.failure('缺少明确的' + label + '选择，不能从页面内容推断。');
    ...
    function listContext(request) {
      ...
      if (!token(snapshot) || !Number.isSafeInteger(size) || size < 1 || size > 200) throw C.failure('当前列表快照或分页信息缺失，请关闭向导并重新读取列表。');
      return {
        scope,
    ...
    function filename(disposition, format) {
      ...
      try {
        ...
        throw C.failure('下载文件名编码无效，未保存文件。');
      }
      if (!name || /[\x00-\x1f\x7f/\\]/.test(name) || !name.toLowerCase().endsWith('.' + format)) throw C.failure('下载文件名与所选格式不一致，未保存文件。');
      return name;
    }
    async function saveDownload(download, format, signal) {
      ...
      if (!download || !(download.blob instanceof Blob) || download.blob.size === 0 || String(download.contentType).split(';')[0].trim().toLowerCase() !== mime) throw C.failure('下载内容或文件类型不正确，未把错误响应保存为文件。');
      const name = filename(download.disposition, format);
      const bytes = new Uint8Array(await download.blob.slice(0, 4).arrayBuffer());
      if (format === 'xlsx' && (bytes[0] !== 80 || bytes[1] !== 75 || bytes[2] !== 3 || bytes[3] !== 4)) throw C.failure('下载内容不是 XLSX 工作簿，未保存文件。');

(function () {
  ...
  const inputClass = 'wb-number-input',
    parentClass = 'wb-number-parent',
    staticClass = 'wb-number-parent-static';
  ...
  function nextValue(input, direction) {
    ...
    if (anchored) {
      ...
      probe.removeAttribute('min');
      probe.removeAttribute('max');
      probe.step = increment === null ? '1' : increment;
      probe.setAttribute('value', input.value || '0');
    }
    probe.value = input.value;
    if (direction > 0) probe.stepUp();else probe.stepDown();
    if (anchored) {
      const min = numberValue(input.ownerDocument, input.min),
        max = numberValue(input.ownerDocument, input.max);
  ...
  function acquire(input, parent) {
    ...
    if (!owner) {
      owner = {
        count: 0,
        hadClass: parent.classList.contains(parentClass),
        hadStatic: parent.classList.contains(staticClass),
  ...
  function readBox(input, parent) {
    const rect = input.getBoundingClientRect(),
      host = parent.getBoundingClientRect();
    ...
    return {
      left: (rect.right - host.left) / scaleX - parent.clientLeft + parent.scrollLeft - 21,
      top: (rect.top - host.top) / scaleY - parent.clientTop + parent.scrollTop + 1,
      right: 'auto',
      bottom: 'auto',
      width: 20,
      height: Math.max(0, rect.height / scaleY - 2)
    };
  }
  function Arrow({
    up
  ...
  function Stepper({
    ...
    React.useLayoutEffect(() => {
      ...
      function refresh() {
        classes.ensure();
        const next = {
          box: readBox(input, parent),
          name: window.APSWorkbenchControlBridge.label(input),
          up: nextValue(input, 1),
          down: nextValue(input, -1)
        };
      ...
      refreshers.set(input, refresh);
      refresh();
      return () => {
        observer.disconnect();
        refreshers.delete(input);
        classes.release();
      };
    ...
    function step(event, direction) {
      ...
      const refresh = refreshers.get(input);
      if (refresh) refresh();
    }
    return ReactDOM.createPortal(/*#__PURE__*/React.createElement("span", {
      ...
      return /*#__PURE__*/React.createElement("button", {
        ...
        title: label,
        "aria-label": label,
        disabled: (direction > 0 ? state.up : state.down) === null,
        onMouseDown: event => event.preventDefault(),
        onClick: event => step(event, direction)
  ...
  function WorkbenchNumberControls() {
    const [entries, setEntries] = React.useState([]),
      refreshers = React.useRef(new Map());
    React.useEffect(() => {
      let frame = 0,
        sequence = 0;
      ...
      function scan() {
        frame = 0;
        const inputs = Array.from(document.querySelectorAll('body.aps-workbench input[type="number"]')).filter(input => input.parentElement && !input.closest('.wb-control-popup'));
        const next = inputs.map(input => {
          if (!identities.has(input)) identities.set(input, ++sequence);
          return {
            input,
            parent: input.parentElement,
            key: identities.get(input)
          };
        });
        setEntries(previous => previous.length === next.length && previous.every((item, index) => item.input === next[index].input && item.parent === next[index].parent) ? previous : next);
        refreshers.current.forEach(refresh => refresh());
      }
      const schedule = () => {
        if (!frame) frame = requestAnimationFrame(scan);
      };
      const ownNode = node => node.nodeType === 1 ? node.matches('.wb-number-stepper') || !!node.closest('.wb-number-stepper') : !!(node.parentElement && node.parentElement.closest('.wb-number-stepper'));
      const observer = new MutationObserver(records => {
        if (records.some(record => !ownNode(record.target) && (record.type !== 'childList' || Array.from(record.addedNodes).concat(Array.from(record.removedNodes)).some(node => !ownNode(node))))) schedule();
      });
      observer.observe(document.body, {
        ...
        attributes: true,
        characterData: true,
        attributeFilter: ['type', 'min', 'max', 'step', 'value', 'readonly', 'disabled', 'class', 'style', 'hidden', 'inert', 'aria-label', 'aria-labelledby', 'title', 'id', 'name', 'data-wb-step']
      });
      function refreshInput(event) {
        const refresh = refreshers.current.get(event.target);
        if (refresh) refresh();
      }
      function keydown(event) {
        const input = event.target;
        if (event.defaultPrevented || event.isComposing || !['ArrowUp', 'ArrowDown'].includes(event.key) || !(input instanceof window.HTMLElement) || !input.matches('body.aps-workbench input[type="number"][data-wb-step]') || !editable(input)) return;
        event.preventDefault();
      ...
      document.addEventListener('change', refreshInput, true);
      ...
      return () => {
        ...
        document.removeEventListener('change', refreshInput, true);
        document.removeEventListener('keydown', keydown);
        document.removeEventListener('reset', schedule, true);
        document.removeEventListener('scroll', schedule, true);
        window.removeEventListener('resize', schedule);
      };
    }, []);
    return /*#__PURE__*/React.createElement(React.Fragment, null, entries.map(entry => /*#__PURE__*/React.createElement(Stepper, {
      key: entry.key,
      ...entry,
      refreshers: refreshers.current
    })));
  }

(function () {
  ...
  const monthParts = value => typeof value === 'string' && /^\d{4}-(0[1-9]|1[0-2])$/.test(value) && Number(value.slice(0, 4)) >= 1 ? [Number(value.slice(0, 4)), Number(value.slice(5))] : null;
  ...
  function moveDay(value, offset) {
    ...
    for (let i = 0; i < Math.abs(offset); i++) {
      ...
      if (day < 1) {
        ...
        day = C.monthDays(year, month);
      }
      if (day > C.monthDays(year, month)) {
        day = 1;
        month++;
        if (month === 13) {
          year++;
  ...
  function nativeField(props) {
    ...
    ['min', 'max', 'step'].forEach(key => {
      if (props[key] !== undefined && props[key] !== null) input.setAttribute(key, String(props[key]));
    });
  ...
  function validate(props, value) {
    ...
    if (!value || !hasShape(props.type, value)) return {
      valid: false,
      message: '请选择完整、有效的日期或时间。'
    };
    const input = nativeField(props);
    input.value = value;
    ...
    if (validity.rangeUnderflow && validity.rangeOverflow) return {
      valid: false,
      message: '时间须在 ' + props.min + ' 至次日 ' + props.max + ' 之间。'
    };
    if (validity.rangeUnderflow) return {
      valid: false,
      message: '不能早于 ' + props.min + '。'
    };
    if (validity.rangeOverflow) return {
      valid: false,
      message: '不能晚于 ' + props.max + '。'
    };
    if (validity.stepMismatch) return {
      valid: false,
      message: '所选值不符合间隔要求，请调整后确认。'
    };
    return {
      valid: false,
      message: '请选择完整、有效的日期或时间。'
  ...
  function daySelectable(props, day) {
    ...
    if (String(props.step).toLowerCase() === 'any') input.value = props.min || '';else {
      input.stepUp(0);
  ...
  function precision(props) {
    ...
    const numericStep = Number(props.step),
      step = Number.isFinite(numericStep) && numericStep > 0 ? numericStep : 60;
    const any = String(props.step).toLowerCase() === 'any';
    const fraction = any || step > 0 && step % 1 !== 0 || values.some(value => /:\d{2}\.\d+$/.test(value));
    return {
      seconds: fraction || step > 0 && step % 60 !== 0 || values.some(value => /\d{2}:\d{2}:\d{2}/.test(value)),
      fraction
    };
  }
  function readTime(value, units) {
    const raw = (value || '').split('T').pop();
    if (!hasShape('time', raw)) return {
      hour: '',
      minute: '',
      second: '',
      millisecond: ''
    ...
    return {
      hour: parts[0],
      minute: parts[1],
      second: units.seconds ? fraction[0] || '00' : '',
  ...
  function seed(props) {
    const current = (props.value || '').slice(0, 10),
      minimum = (props.min || '').slice(0, 10);
    return dateParts(current) ? current : monthParts(current) ? current + '-01' : dateParts(minimum) ? minimum : monthParts(minimum) ? minimum + '-01' : today();
  }
  window.WorkbenchDatePickerModel = {
    types,
    pad,
    monthKey,
    dateKey,

(function () {
  ...
  function check(valid, message = '系统维护数据合同不完整，未按成功或空记录处理。') {
    ...
    return {
      read() {
        ...
        try {
          ...
          throw new Error('本机待核实记录损坏，未清除或重新提交；请保留现场。');
        }
        check(validIntent(value), '本机待核实记录不完整，已阻止新操作；请保留现场。');
        return value;
      },
      begin(action) {
        ...
        check(this.read().request_key === value.request_key, '原请求标识未能持久保存，本次没有发起操作。');
        return value;
      },
      finish(intent) {
        const saved = this.read();
        check(saved && saved.request_key === intent.request_key, '待核实记录已变化，未删除其他请求。');
  ...
  function recordSelection(selection, kind) {
    ...
    check(object(selection) && Object.keys(selection).every(key => ['key', 'backup_ref', 'record_kind'].includes(key)) && (selection.key === undefined || nonempty(selection.key)) && (selection.record_kind === undefined || ['backup_file', 'restore_event', 'cleanup_event'].includes(selection.record_kind)) && (selection.backup_ref === undefined || nonempty(selection.backup_ref) && (selection.record_kind === undefined || selection.record_kind === 'backup_file')), '维护记录的原选择引用无效。');
  ...
  function recordContext(value = {}, kind) {
    ...
    check((value.filters === undefined || object(value.filters)) && Object.keys(filters).every(key => ['query', 'type', 'status', 'level', 'file', 'start', 'end'].includes(key) && text(filters[key]) && filters[key].length <= 200), '维护记录筛选恢复范围无效。');
  ...
  function pageContext(value = {}) {
    ...
    check(['current', 'sample'].includes(source) && ['overview', 'backups', 'logs', 'config'].includes(tab) && [10, 25, 50].includes(size) && object(records) && Object.keys(records).every(key => ['backups', 'logs'].includes(key)), '系统页面恢复状态无效。');
    return {
      source,
      tab,
      page_size: size,
    ...
    return {
      ...
      async read(kind, input, signal) {
        ...
        if (kind === 'config') config(envelope(value));else {
          ...
          const query = input || {};
          check(value.data.page.number === (query.page === undefined ? 1 : query.page) && value.data.page.size === (query.page_size === undefined ? 10 : query.page_size) && (!query.snapshot_ref || value.meta.snapshot_ref === query.snapshot_ref), '维护记录返回的页码、每页数量或快照与原请求不一致。');
        }
        return value;
      },
        ...
        return exchange('/logs/export/' + format + parameters(input), {
          ...
          if (mime === 'application/json' || /^[\s\ufeff]*[\[{]/.test(prefix)) {
            ...
            try {
              ...
              throw new Error('下载返回了异常 JSON，未保存为文件。');
            }
          ...
          check(format === 'zip' ? magic[0] === 80 && magic[1] === 75 && magic[2] === 3 && magic[3] === 4 : magic[0] === 239 && magic[1] === 187 && magic[2] === 191 && prefix.startsWith('来源,工厂本地时间,'), '下载内容与日志文件格式不符。');
      ...
      async downloadBackup(row, input, signal) {
        check(object(row) && row.record_kind === 'backup_file' && !['event_ref', 'event_source'].some(key => Object.prototype.hasOwnProperty.call(row, key)) && nonempty(row.backup_ref) && nonempty(row.filename) && count(row.size_bytes) && object(input) && nonempty(input.snapshot_ref), '请先读取备份清单并选择文件。');
        return exchange('/backups/' + encodeURIComponent(row.backup_ref) + '/download' + parameters(input), {
          ...
          if (mime === 'application/json' || /^[\s\ufeff]*[\[{]/.test(prefix)) {
            ...
            try {
              ...
              throw new Error('备份下载返回异常 JSON，未保存文件。');
            }
            throw errorResponse(payload, response.status);
          }
          ...
          check(/^attachment;/i.test(disposition) && match && decodeURIComponent(match[1]) === row.filename && response.headers.get('X-APS-Backup-Ref') === row.backup_ref, '备份下载返回的文件不是原选择对象。');

(function () {
  ...
  const bindingFields = ['template_operation_ref', 'template_revision', 'template_snapshot', 'part_ref', 'part_no', 'part_name', 'sequence', 'operation_label', 'source', 'old_unit_hours', 'suggested_unit_hours', 'sample_count', 'eligible_sample_count', 'candidate_count', 'excluded_count', 'sample_refs', 'sample_revisions', 'exclusion_reasons', 'method_version'];
  function binding(row) {
    check(object(row) && bindingFields.every(k => Object.prototype.hasOwnProperty.call(row, k)) && ref(row.template_operation_ref) && ref(row.part_ref) && Number.isInteger(row.template_revision) && row.template_revision > 0 && text(row.template_snapshot) && text(row.method_version) && [row.old_unit_hours, row.suggested_unit_hours].every(v => v === null || number(v)) && ['sample_count', 'eligible_sample_count', 'candidate_count', 'excluded_count'].every(k => Number.isInteger(row[k]) && row[k] >= 0) && row.sample_count <= 20 && row.sample_count <= row.eligible_sample_count && row.eligible_sample_count <= row.candidate_count && row.excluded_count === row.candidate_count - row.sample_count && Array.isArray(row.sample_refs) && row.sample_refs.length === row.sample_count && row.sample_refs.every(ref) && new Set(row.sample_refs).size === row.sample_count && Array.isArray(row.sample_revisions) && row.sample_revisions.length === row.sample_count && row.sample_revisions.every((r, i) => object(r) && r.sample_ref === row.sample_refs[i] && text(r.sample_revision) && r.template_revision === row.template_revision && Array.isArray(r.report_revision_refs) && r.report_revision_refs.every(ref)) && Array.isArray(row.exclusion_reasons));
  ...
  function preview(value, original, intent) {
    ...
    if (d.quota_lock !== null) check(object(d.quota_lock) && d.quota_lock.template_operation_ref === original.template_operation_ref && d.quota_lock.locked === true && ref(d.quota_lock.adoption_ref) && number(d.quota_lock.locked_unit_hours));
    return d;
  }
  function receipt(value, intent) {
    const d = value && value.data,
      s = intent && intent.baseline;
    check(object(intent) && key(intent.request_key) && s && value && value.ok === true && value.result === 'committed' && typeof value.receipt_ref === 'string' && /^[a-f0-9]{32}$/.test(value.receipt_ref) && typeof value.replayed === 'boolean' && Array.isArray(value.warnings) && !value.warnings.length && object(d) && ref(d.adoption_ref) && d.template_operation_ref === s.template_operation_ref && d.request_key === intent.request_key && d.reason === intent.input.reason && d.declared_operator === intent.input.declared_operator && d.confirmed === true && text(d.application_operator) && text(d.adopted_at) && text(d.generated_at) && d.old_unit_hours === s.old_unit_hours && d.new_unit_hours === s.suggested_unit_hours && d.template_revision_before === s.template_revision && d.template_revision_after === s.template_revision + Number(s.old_unit_hours !== s.suggested_unit_hours) && d.method_version === s.method_version && d.sample_count === s.sample_count && equal(d.sample_refs, s.sample_refs) && equal(d.sample_revisions, s.sample_revisions) && d.locked === true && d.effect_scope === 'future_template_use_only');
    return value;
  }
  ...
  function create(fetcher = window.fetch.bind(window)) {
    ...
    return {
      async preview(row, values, signal) {
        const original = binding(row),
          intent = input(values);
        return preview(await request(original.template_operation_ref + '/adopt-preview', {
          input: intent
        }, signal), original, intent);
      },
      async adopt(intent, token) {
        check(text(token));
        return receipt(await request(intent.baseline.template_operation_ref + '/adopt', {
          write_token: token,
          request_key: intent.request_key,
          input: input(intent.input, true)
        }), intent);
      },
      async lookup(intent, signal) {
        check(key(intent.request_key) && ref(intent.baseline.template_operation_ref));
        try {
          return receipt(await request(intent.baseline.template_operation_ref + '/adopt/receipts/' + intent.request_key, undefined, signal), intent);
        } catch (e) {

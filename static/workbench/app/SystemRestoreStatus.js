(function () {
  ...
  function describe(host, op) {
    ...
    return {
      ...
      guidance: !stopped ? op && op.terminal ? '当前软件服务可用。确认这条原结果后，重新读取本机资料；旧页面引用不能沿用。' : '原维护请求尚未核实。请等待结果或查询原请求，不要重新提交。' : uncertain ? '请保留原请求标识、备份和保护副本，交给维护人员核查。未核实记录会继续阻止启动；不要重复恢复，也不要自行改维护标记。' : '请先关闭整个 APS 软件，再重新启动。只刷新或关闭浏览器不算重启；重新启动后再读取本机资料。',
      uncertain
    };
  }

(function () {
  ...
  const SELECTOR = '.wb-table-frame',
    EPSILON = 1;
  function flag(frame, name, on) {
    if (on) {
      if (!frame.hasAttribute(name)) frame.setAttribute(name, '');
  ...
  function attach(container) {
    ...
    const register = frame => {
      ...
      registered.set(frame, () => {
        frame.removeEventListener('scroll', scroll);
        resize.unobserve(frame);
        if (table) resize.unobserve(table);
        frame.removeAttribute('data-overflow-left');
        frame.removeAttribute('data-overflow-right');
      });
      update(frame);
    };
    const release = frame => {
      const dispose = registered.get(frame);
      if (dispose) {
        dispose();
    ...
    return () => {
      ...
      Array.from(registered.keys()).forEach(release);
      resize.disconnect();
    };
  }
  window.WorkbenchScrollShadows = Object.freeze({
    attach,
    update
  });
})();

(function () {
  ...
  const quantityReasons = {
    plan_target_not_recorded: '该计划未记录可核实的原目标量，未使用当前批次数量。',
  ...
  function tracks(items, conflicts) {
    ...
    for (const item of sorted) {
      if (furthest && item.start < furthest.end) {
        conflicts.add(item.task.task_ref);
        conflicts.add(furthest.task.task_ref);
      }
  ...
  function layout(data, mode, query, baseline, width = 1000, changedOnly = false) {
    ...
    const selected = data.tasks.filter(matches),
      groups = new Map(),
      conflicts = new Set(),
      locations = new Map();
    let start = instant(data.plan_span.start),
      end = instant(data.plan_span.end);
    const add = (task, before) => {
      const item = {
        task,
        start: instant(task.start),
        end: instant(task.end),
        baseline: before
      };
      if (before) {
      ...
      if (!groups.has(id)) groups.set(id, {
        id,
        label: resourceLabel(task, mode, labels),
        tasks: [],
        before: []
      });
    ...
    if (baseline && data.projections.baseline.state === 'available') data.projections.baseline.items.forEach(item => {
      if (item.before) add(item.before, true);
    });
    ...
    let top = 0;
    const rows = [];
    const occupancy = new Map(data.projections.occupancy.resources.filter(row => row.kind === mode).map(row => [row.resource_ref, row.segments.filter(item => item.concurrent_operations > 1).map(item => ({
      start: instant(item.start),
      end: instant(item.end)
    }))]));
    for (const group of groups.values()) {
      const overlap = new Set();
      const split = (items, conflicts) => [...tracks(items.filter(i => !P.isPoint(i.task)), conflicts), ...G.tracks(items.filter(i => P.isPoint(i.task)), start, end, width)];
      const current = split(group.tasks, overlap),
        old = split(group.before, new Set());
      ...
      [...current, ...old].forEach((all, index) => {
        ...
        const before = index >= current.length,
          height = before ? 26 : 56;
        const row = {
          ...group,
          items,
          ...
          height,
          before,
          point: P.isPoint(items[0].task),
          track: before ? index - current.length : index,
          trackCount: before ? old.length : current.length,
          overlap: P.isPoint(items[0].task) ? 0 : overlap.size,
          key: group.id + ':' + index
        };
        items.forEach(item => locations.set(item.task.task_ref, {
          row: rows.length,
          top,
          item
        }));
    ...
    return {
      ...
      locations,
      tasks: selected,
      groupCount: new Set(rows.map(row => row.id)).size
    };
  }
  function firstIntersecting(items, start) {
    ...
    while (low < high) {
      ...
      if (items[mid].end <= start) low = mid + 1;else high = mid;
    }
    return low;
  }
  ...
  function visibleRows(rows, start, end) {
    ...
    while (low < high) {
      const mid = low + high >> 1;
      if (rows[mid].top + rows[mid].height < start) low = mid + 1;else high = mid;
    }
    const result = [];
  ...
  function ticks(start, end, width, left, visibleWidth) {
    ...
    const low = start + Math.max(0, left - 150) / width * (end - start),
      high = Math.min(end, start + (left + visibleWidth + 150) / width * (end - start));
    const result = [];
    for (let at = Math.ceil(low / step) * step; at < high; at += step) result.push({
      at,
      x: (at - start) / (end - start) * width,
      label: wire(at)
    });
    return result;
  }
  function taskTitle(task, labels, conflict) {
    return [task.batch_id + ' · ' + task.sequence + ' ' + task.process_label, pieceLabel(task), '本工序目标量：' + quantityLabel(task.quantity) + ' · 计划来源整批量：' + quantityLabel(task.batch_quantity), task.quantity_reason ? quantityReasons[task.quantity_reason] : null, window.PointContract.isPoint(task) ? '时间点 · 0 h · 不占用资源' : null, timeLabel(task.start) + ' → ' + timeLabel(task.end), resourceLabel(task, 'machine', labels) + ' / ' + resourceLabel(task, 'operator', labels), conflict ? '资源时间冲突（安排重叠）' : null].filter(Boolean).join('\n');
  }

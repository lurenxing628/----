(function () {
  ...
  const headers = ['方案', '方案状态', '约束说明（非取舍评估）', '对比基准方案', '换型次数（次）', '批次', '产品', '数量（件）', '优先级', '交期（截至日）', '对比基准完工', '方案完工', '超期（小时）', '完工提前（小时，负数为延后）', '工序调整', '换设备', '交付风险', '草稿编号', '试调方案编号', '原来源编号', '试调方案保存时间', '核对提示'];
  const states = {
    editing: '可继续试调',
    saved: '已保存',
    discarded: '已放弃',
  ...
  function note(data) {
    return '完整' + data.comparison.batches.length + '批次、' + data.task_count + '道安排、' + data.unplanned_operations.length + '道未排；不受分页或显示筛选限制。' + '对比基准是原试调来源；交期截至日次日零点不含；时间与工厂现场一致；负数提前量表示延后。' + (data.scenario_ref ? '数据是试调方案保存时的内容。' : '数据是本次页面读到的内容，导出时不刷新。') + '未知不等于 0，空白表示不适用；数值保留原精度。' + data.comparison.changeover_reason + '文字统一加一个单引号，防止表格把它当公式或改写编号；按标准 CSV 解码后只去掉开头这一个引号。完整任务、资源变更、班表、报工和调整记录见“导出原始数据”（不含系统内部数据）。';
  }
  function cell(value) {
    if (value === undefined) return '""';
    if (typeof value === 'number') {
      C.check(Number.isFinite(value), '导出里有无效数值，没有拿 0 代替。');
      return String(value);
    }

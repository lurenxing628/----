(function () {
  ...
  const C = window.OutsourcingContract,
    KEY = 'aps_outsourcing_pending_v1',
    EVENT = KEY + '_changed';
  function originalInput(v) {
    const p = v.input,
      creating = Object.prototype.hasOwnProperty.call(p, 'target');
    const allowed = ['declared_operator', 'reason', creating ? 'target' : 'outsourcing_ref', ...C.fields];
    return Object.keys(p).every(k => allowed.includes(k)) && ['declared_operator', 'reason'].every(k => C.text(p[k]) && p[k] === p[k].trim() && !p[k].includes('\0')) && p.declared_operator.length <= 200 && p.reason.length <= 2000 && C.fields.every(k => creating || Object.prototype.hasOwnProperty.call(p, k) ? p[k] === v.after[k] : true) && (creating ? C.equal(p.target, C.target(v.target)) : C.ref(p.outsourcing_ref));
  }
  function valid(v) {
    try {
      return v && v.version === 1 && /^outsourcing-[a-f0-9]{48}$/.test(v.request_key) && ['pending', 'confirmed', 'rejected'].includes(v.phase) && C.object(v.input) && !!C.facts(v.after) && !!C.target(v.target, true) && originalInput(v) && !Object.prototype.hasOwnProperty.call(v, 'write_token') && (v.phase !== 'confirmed' || !!C.receipt(v.receipt, v));
    } catch (_) {
      return false;
    }
  }
  ...
  function useCommand(api) {
    ...
    const [saved, setSaved] = React.useState(initial.saved),
      [storageError, setStorageError] = React.useState(initial.error || null);
    const [error, setError] = React.useState(null),
      [notice, setNotice] = React.useState(''),
      [busy, setBusy] = React.useState(false);
    const active = React.useRef(saved),
      ...
      try {
        ...
        await navigator.locks.request(KEY, {
          ...
          original = {
            version: 1,
            phase: 'pending',
            request_key: 'outsourcing-' + Array.from(crypto.getRandomValues(new Uint8Array(24)), n => n.toString(16).padStart(2, '0')).join(''),
            input: d.input,
            after: d.after,
            target: d.target
          };
          save(original, null);
          try {
            accept(await api.command(original, d.write_context.write_token), original);
          } catch (e) {
  ...
  function input(draft, selected, item, now) {
    const values = {
      ...
      planned: C.stamp(draft.planned),
      returned: draft.returned ? C.stamp(draft.returned) : null,
      confirmedState: draft.confirmedState
    };
    C.facts(values, now);
    const p = {
    ...
    if (item) {
      ...
      p.target = C.target({
        ...
        supplier_ref: selected[0].supplier_ref,
        operation_refs: selected.map(r => r.operation_ref)
      });
      Object.assign(p, values);
    }
    return p;
  }
  window.OutsourcingSession = {
    KEY,
    valid,
    read,
    save,

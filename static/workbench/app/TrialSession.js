(function () {
  ...
  function useCommands(onReceipt) {
    ...
    const live = React.useRef(true),
      callback = React.useRef(onReceipt);
    callback.current = onReceipt;
    const patch = data => {
      if (live.current) set(s => ({
        ...s,
        ...data
      }));
    };
    ...
    React.useEffect(() => {
      ...
      restore();
      const listener = e => {
        if (e.key === A.PENDING_KEY || e.key === null) restore();
      };
    ...
    async function execute(intent, token) {
      ...
      try {
        ...
        const result = await A.command(intent, token, key);
        await confirmed(result, key);
        return true;
      } catch (error) {
    ...
    return {
      ...
      lookup,
      restore,
      blocked: state.busy || !!state.key || !!state.error
    };
  }
  window.TrialSession = {
    useRead,

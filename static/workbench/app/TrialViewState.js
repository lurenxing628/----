(function () {
  ...
  const C = window.TrialContract,
    PREFIX = 'aps_workbench_trial_view_v1:',
    EVENT = 'aps-workbench-trial-view-changed';
  ...
  function defaults(data) {
    ...
    return {
      mode: scope.resource_type || 'machine',
      baseline: true,
      only_changed: false,
      query: scope.query === undefined ? '' : scope.query,
      result_tab: 'delivery'
    };
  }
  function storage(value) {
    return value === undefined ? window.localStorage : value;
  }
  function read(target, store) {
    const name = key(target);
    let raw;
    try {
      raw = storage(store).getItem(name);
    } catch (_) {
      throw problem('read_unavailable', '本机试调查看偏好读取失败。');
    }
  ...
  function write(target, initial, patch, store) {
    ...
    const value = preferences({
      ...(read(target, store) || initial),
      ...patch
    });
    try {
      storage(store).setItem(name, JSON.stringify({
        schema_version: 1,
        kind: target.kind,
        ref: target.ref,
        preferences: value
      ...
      throw problem('write_unavailable', '本次查看偏好尚未保存，本页选择已保留。');
    }
  ...
  function clear(target, store) {
    ...
    try {
      ...
      throw problem('clear_unavailable', '查看偏好清除失败，请重试。');
    }
  }
  function useView(data) {
    const target = identity(data),
      name = key(target),
      initial = defaults(data),
      signature = JSON.stringify(initial);
    const pending = React.useRef({
        name,
        patch: {}
      }),
      active = React.useRef(name);
    active.current = name;
    if (pending.current.name !== name) pending.current = {
      name,
      patch: {}
    };
    function loaded() {
    ...
    function reload() {
      ...
      if (Object.keys(pending.current.patch).length) change({});else setState(loaded());
    }
    function reset() {
      if (active.current !== name) return;
      try {
    ...
    return {
      value: current.value,
      error: current.error,
      pending: Object.keys(pending.current.patch).length > 0,
      change,
      reload,
      reset
    };
  ...
  function Notice({
    ...
    return React.createElement('div', {
      ...
      role: 'region',
      'aria-label': label
    }, React.createElement(U.ErrorBox, {
      error: state.error

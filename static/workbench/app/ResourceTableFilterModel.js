(function () {
  ...
  function toggleKeys(value, changed, enabled) {
    if (!Array.isArray(changed) || changed.some(key => typeof key !== 'string' || !keyPattern.test(key))) throw C.failure('列值标识无效，未改变筛选。');
    const selected = rule(value),
  ...
  function envelope(raw, request) {
    ...
    if (request.snapshot_ref && meta.snapshot_ref !== request.snapshot_ref) throw C.failure('列值快照已经变化，未混用不同页的数据。请回到首页重新读取。');
    return data;
  }
  function facets(raw, request) {
    ...
    for (const option of data.options) {
      if (!C.object(option) || typeof option.key !== 'string' || !keyPattern.test(option.key) || keys.has(option.key) || typeof option.label !== 'string' || !count(option.count)) throw C.failure('列值标识、文字或数量不正确，请回到首页重新读取。');
      keys.add(option.key);
    }
    return raw;
  }
  function selection(raw, request) {
    const data = envelope(raw, request);
    if (!count(data.total) || !Array.isArray(data.keys) || data.total !== data.keys.length || data.total > 50000 || request.expected_total !== undefined && data.total !== request.expected_total || data.keys.some(key => typeof key !== 'string' || !keyPattern.test(key)) || new Set(data.keys).size !== data.total) throw C.failure('全部匹配值响应不完整或超过 50000 个值，未改变筛选。请回到首页重新读取。');
    return raw;
  }

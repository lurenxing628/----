...
(function (root) {
  ...
  const scenarios = [['auto', 'failed', '自动备份写入失败，本次未生成备份', 'sqlite3.OperationalError: database or disk is full\n临时副本未升为正式备份。\n样例目录: C:\\APS-SAMPLE\\backups\\2026\\09\\设备产能数据归档\\aps_20260907_auto.db\n下一步: 核查磁盘空间和本机写入条件，再由正式软件重试。'], ['cleanup', 'skipped', '自动备份失败，本轮清理已跳过', 'reason=auto_backup_failed_this_round\n旧备份未因本次清理被移除；该结论仅属于管理样例。'], ['restore', 'failed', '恢复后结构校验失败，已自动回滚', 'verify_failed_rolled_back\n样例恢复副本未通过数据表检查；已回到恢复前副本。\n这不是恢复成功记录。'], ['restore', 'blocked', '数据库正在维护，恢复未执行', 'busy\n维护窗口尚未释放，本次操作没有覆盖数据库。'], ['auto', 'pending', '到期检查待触发', '自动维护由访问请求触发，不是后台定时服务。\n此行是待执行情境样例，不是服务端任务队列。'], ['manual', 'verified', '手动备份完成完整性检查', '管理样例中的副本完整性检查通过。\n仅检查通过不等于已经演练过恢复。'], ['before_restore', 'verified', '恢复前保护副本已生成', 'before_restore\n正式恢复流程要求先保存恢复前副本。此处没有实际文件。'], ['manual', 'unverified', '历史副本只有文件元信息', '尚无完整性检查证据，不能由文件存在判断副本健康。'], ['restore', 'failed', '结构校验及自动回滚均失败', 'verify_failed_rollback_failed\n请停止继续使用数据库，保留现场并检查正式日志。\n管理样例，不是当前机器故障。'], ['cleanup', 'recorded', '过期副本因保底策略保留', '按保留天数清理时仍保留最新副本；本条没有执行删除。']];
  ...
  function inspectEnvironment(w, themeProps = {}) {
    ...
    check('model', () => w.APSSystemWorkbench === API && SAMPLES.logs.every(row => row.source === 'sample' && STATES[row.status]) && CURRENT.logs === null, '当前环境与固定管理样例分开；本机运行数据尚未读取。', '管理资源模型未注册或合同不匹配。');
    check('download', () => canDownload(w), '浏览器提供本地文件导出 API；是否保存以浏览器下载结果为准。', '浏览器缺少 Blob 下载能力。');
    check('theme', () => ['light', 'dark'].includes(themeProps.theme) && (typeof themeProps.onToggleTheme === 'function' || typeof themeProps.onSetTheme === 'function'), '主题值与宿主切换回调已接入。', '宿主尚未传入主题值与切换回调。');
    return {
      schemaVersion: 1,
...
function SMOverview({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "sm-meta"
  }, "\u5907\u4EFD\u3001\u65E5\u5FD7\u4E0E\u7EF4\u62A4\u7B56\u7565")), items.map(item => /*#__PURE__*/React.createElement("button", {
    key: item.tab,
    type: "button",
...
function SMFilters({
  ...
  }))), select('type', '类型', (kind === 'logs' ? ['runtime', 'operation'] : ['manual', 'auto', 'before_restore', 'restore', 'cleanup']).map(key => [key, model.TYPES[key]])), select('status', '状态', ['failed', 'blocked', 'pending', 'skipped', 'verified', 'unverified', 'recorded'].map(key => [key, model.STATES[key][0]])), kind === 'logs' && select('level', '级别', ['ERROR', 'WARNING', 'INFO'].map(key => [key, key])), kind === 'logs' && select('file', '记录集', ['aps_error.log', 'aps.log', 'launcher.log', 'OperationLogs'].map(key => [key, key])), /*#__PURE__*/React.createElement("label", {
    className: "sm-field"
...
function SMRecordDetail({
  ...
  return /*#__PURE__*/React.createElement("section", {
    ...
    state: row.status
  }), /*#__PURE__*/React.createElement("span", null, row.file || row.filename || '未生成文件')), /*#__PURE__*/React.createElement("h4", null, row.summary), /*#__PURE__*/React.createElement("pre", null, row.body), kind === 'backups' && /*#__PURE__*/React.createElement("details", {
    className: "sm-rules"
  }, /*#__PURE__*/React.createElement("summary", null, "\u6062\u590D\u6821\u9A8C\u8981\u6C42"), /*#__PURE__*/React.createElement("p", null, "\u76EE\u6807\u5907\u4EFD\u3001\u7EF4\u62A4\u7A97\u53E3\u3001\u6062\u590D\u524D\u526F\u672C\u3001\u6062\u590D\u540E\u7ED3\u6784\u6821\u9A8C\u4E0E\u7ED3\u679C\u7559\u75D5\u5747\u672A\u63A5\u5165\u3002\u6062\u590D\u548C\u56DE\u6EDA\u5931\u8D25\u5FC5\u987B\u5206\u522B\u8BB0\u5F55\u3002")));
}
function SMRecords({
  kind,
  source,
  ...
  const columns = [{
    ...
    key: 'summary',
    title: kind === 'logs' ? '摘要 / 记录集' : '记录 / 文件',
    render: row => /*#__PURE__*/React.createElement("div", {
      className: "sm-summary"
    }, /*#__PURE__*/React.createElement("strong", null, row.summary), /*#__PURE__*/React.createElement("small", null, kind === 'logs' ? row.file : row.filename || '未生成文件'))
  }, ...(kind === 'backups' ? [{
    key: 'sizeBytes',
    title: '大小',
    align: 'right',
  ...
  return /*#__PURE__*/React.createElement("section", {
    ...
    className: "sm-section-head"
  }, /*#__PURE__*/React.createElement("h3", null, kind === 'logs' ? '运行日志与操作记录' : '备份与恢复记录'), /*#__PURE__*/React.createElement("span", {
    className: "sm-meta"
  ...
  }, "\u672C\u539F\u578B\u5C1A\u672A\u63A5\u5165\u672C\u673A\u8BB0\u5F55\u3002\u603B\u6570\u672A\u77E5\uFF0C\u4E0D\u80FD\u5224\u65AD\u8BB0\u5F55\u4E3A\u7A7A\u6216\u8BFB\u53D6\u5931\u8D25\u3002") : results.rows.length === 0 ? /*#__PURE__*/React.createElement(SMUnavailable, {
...
function SMConfiguration({
  ...
  const changeTheme = next => {
    ...
    try {
      ...
      setThemeError('主题切换未完成，请检查宿主主题回调。');
    }
  };
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "sm-meta"
  }, "\u4E3B\u9898\u6CBF\u7528\u5DE5\u4F5C\u53F0\uFF1B\u5217\u8868\u8BBE\u7F6E\u4EC5\u5F53\u524D\u9875\u9762\u4F1A\u8BDD")), /*#__PURE__*/React.createElement("div", {
    ...
    className: "sm-note"
  }, "\u4E3B\u9898\u4E0D\u53EF\u7528\uFF1A\u5BBF\u4E3B\u9700\u8981\u4F20\u5165 theme \u4E0E onToggleTheme \u6216 onSetTheme\u3002"), themeError && /*#__PURE__*/React.createElement("p", {
    className: "sm-error",
    role: "alert"
    ...
    label: "\u4FDD\u5B58\u6B63\u5F0F\u914D\u7F6E",
    reason: "\u672A\u8BFB\u53D6\u672C\u673A\u6B63\u5F0F\u914D\u7F6E\u5FEB\u7167\uFF0C\u4E5F\u672A\u63A5\u5165\u914D\u7F6E\u4FDD\u5B58\u4E0E\u6821\u9A8C\u7ED3\u679C"
  })), /*#__PURE__*/React.createElement("div", {
    ...
    className: "sm-error",
    role: "alert"
  }, "\u6837\u4F8B\u914D\u7F6E\u6821\u9A8C\u672A\u901A\u8FC7\uFF0C\u8BF7\u4FEE\u6B63\u6807\u51FA\u7684\u5B57\u6BB5\u3002"), preview && /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement("summary", null, "\u751F\u6548\u8303\u56F4\u4E0E\u81EA\u52A8\u7EF4\u62A4\u89C4\u5219"), /*#__PURE__*/React.createElement("dl", null, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("dt", null, "\u5F53\u524D\u7F16\u8F91"), /*#__PURE__*/React.createElement("dd", null, source === 'sample' ? '独立管理样例' : '本机正式配置未读取')), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("dt", null, "\u89E6\u53D1\u65B9\u5F0F"), /*#__PURE__*/React.createElement("dd", null, "\u7531\u8BBF\u95EE\u8BF7\u6C42\u89E6\u53D1\u68C0\u67E5")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("dt", null, "\u65E5\u5FD7\u6E05\u7406\u5BF9\u8C61"), /*#__PURE__*/React.createElement("dd", null, "\u64CD\u4F5C\u8BB0\u5F55\uFF0C\u4E0D\u542B\u8FD0\u884C\u6587\u4EF6\u65E5\u5FD7"))), /*#__PURE__*/React.createElement("p", null, "\u5907\u4EFD\u4E0E\u6E05\u7406\u7531\u8BBF\u95EE\u8BF7\u6C42\u89E6\u53D1\uFF0C\u4E0D\u662F\u540E\u53F0\u5B9A\u65F6\u4EFB\u52A1\u3002\u95F4\u9694\u4E3A\u68C0\u67E5\u5468\u671F\uFF0C\u4E0D\u4FDD\u8BC1\u6307\u5B9A\u65F6\u523B\u6267\u884C\uFF1B\u6B63\u5E38\u9000\u51FA\u65F6\u7684\u5907\u4EFD\u4E5F\u53D7\u81EA\u52A8\u5907\u4EFD\u5F00\u5173\u63A7\u5236\u3002"), /*#__PURE__*/React.createElement("p", null, "\u65E5\u5FD7\u6E05\u7406\u53EA\u9488\u5BF9\u64CD\u4F5C\u8BB0\u5F55\uFF0C\u4E0D\u6E05\u7A7A\u8FD0\u884C\u6587\u4EF6\u65E5\u5FD7\u3002\u5907\u4EFD\u5931\u8D25\u65F6\u8DF3\u8FC7\u672C\u8F6E\u5907\u4EFD\u6E05\u7406\uFF0C\u4FDD\u7559\u7B56\u7565\u4E0D\u80FD\u5220\u9664\u5168\u90E8\u8FD1\u671F\u526F\u672C\u3002"))));
}
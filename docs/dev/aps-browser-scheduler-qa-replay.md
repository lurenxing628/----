...
## 2026-06-10 前台极限压测复跑入口

这次新增了可复跑脚本：

```bash
.venv/bin/python tests/_scripts_e2e/run_browser_extreme_stress_case.py --port 61662
```

脚本会创建隔离临时目录和临时数据库，不会写入仓库的 `db/aps.db`。它复用 `run_synthetic_case.py` 的合成造数底座，再叠加更复杂的前台压测因素：班组、人机关系、短班、停工、周末加班、人员请假、设备高密度停机、外协合并组、紧交期、大数量、短工序、长工序和隐藏失败边界批次。
...
| 步骤 | 结果 |
|---|---|
| 打开排产页 | `http://127.0.0.1:61662/scheduler/?status=pending&only_ready=yes&per_page=300` 可见 91 个主压测批次 |
| 执行排产 | 版本 1，成功 `819/819`，失败 `0`，超期 `82` 个 |
| 默认甘特范围 | 触发范围保护，提示“当前日期范围太宽，请缩小日期范围，或切换到更粗的时间粒度。” |
| 设备甘特首周 | `2026-06-11` 到 `2026-06-18` 显示 `208/208` 条任务 |
| 人员甘特首周 | 同范围显示 `208/208` 条任务 |
| 时间粒度切换 | 人员视图切到“小时”后仍显示 `208` 条任务，SVG 宽度扩大为小时级视图 |

## 只读甘特图缩放压测矩阵

这组步骤用于复验“只读甘特图结果查看 + 时间缩放”第一版。复验时不要拖动保存，也不要新增模拟调整接口；目标是确认查看模式可信、短工序可点、范围过大时能主动拦截，Win7 Chrome 109 或等价目标浏览器不会卡死。

### 必测数据量

| 任务数量 | 目的 |
|---:|---|
| 100 条 | 普通小场景，确认页面基本交互顺畅 |
| 500 条 | 第一版 1 分钟单日建议上限内的重点场景 |
| 1000 条 | 压力边界，允许提示筛选或变慢，但不能卡死 |
| 2000 条 | 大压力场景，超过硬限制时必须阻止渲染 |
...
| 视角 | 设备视图、人员视图 |
| 关键工序外框 | 开、关 |
| 工序关系线 | 只显示关键工序关系线、显示全部工艺关系线、不显示关系线 |
| 短工序 | 36 分钟、51 分钟、73 分钟、跨天短工序、结束在 00:00:00 的短工序 |

### 记录项

| 项目 | 怎么记 |
|---|---|
| 首次渲染时间 | 从页面开始加载到任务条可见 |
| 切换缩放时间 | 从选择粒度到任务条重画完成 |
| 切换筛选时间 | 从改变筛选到任务条稳定 |
| 横向滚动 | 是否卡顿、是否错位 |
| 弹窗 | 短工序是否可点，详情是否能打开 |
| 标记对齐 | 假期/停工背景、今天高亮、关键工序外框、超期红框、外协虚线、依赖线是否对齐 |

### 第一版通过口径

- 1分钟 / 单日 / 500 任务以内不应卡死。
- 1分钟 / 单日 / 1000 任务以上如果卡顿，可以通过范围保护或筛选提示接受。
- 超过 `max_range_days`、`hard_column_limit` 或 `hard_node_limit` 时必须阻止渲染并提示。
- 页面不引入外部 CDN；离线静态资源必须可用。

...
def test_reports_page_version_default_latest(tmp_path, monkeypatch) -> None:
    ...
    overdue_html = client.get("/reports/overdue?version=abc").get_data(as_text=True)
    _assert_selected_latest(overdue_html, 7)

    overdue_zero_html = client.get("/reports/overdue?version=0").get_data(as_text=True)
    _assert_selected_latest(overdue_zero_html, 7)

    utilization_html = client.get("/reports/utilization?version=-1").get_data(as_text=True)
    _assert_selected_latest(utilization_html, 7)

    downtime_html = client.get("/reports/downtime?version=abc").get_data(as_text=True)

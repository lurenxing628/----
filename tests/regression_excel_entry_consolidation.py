...
def main() -> None:
    ...
    with app.test_client() as client:
        dashboard_v2_html = _text(_get(client, "/", ui_mode="v2"))
        assert "Excel 演示" not in dashboard_v2_html
        demo_v2_html = _text(_get(client, "/excel-demo/", ui_mode="v2"))
        assert "Excel 导入演示" in demo_v2_html

        process_html = _assert_page_contains(
            client,
            "/process/",
            ["批量维护", "批量维护路线文字", "批量维护工序工时", "导出工序清单"],
        )
        _assert_subnav_clean(
            process_html,
            forbidden=["批量维护路线文字", "批量维护工序工时", "导出工序清单", "工种配置（Excel）", "供应商配置（Excel）"],
            active_label="零件工艺模板",
        )

        op_types_html = _assert_page_contains(client, "/process/op-types", ["批量维护", "工种配置"])
        assert "本页用于：" in op_types_html
        assert "维护自制/外协工种，供路线文字生成工序时使用。" in op_types_html
        assert "aps-scenario-strip" not in op_types_html
        ...
        assert "本页用于：" in suppliers_html
        assert "维护外协供应商、默认周期、状态和对应工种。" in suppliers_html
        assert "aps-scenario-strip" not in suppliers_html
        _assert_subnav_clean(suppliers_html, forbidden=["供应商配置（Excel）"], active_label="供应商配置")

        equipment_html = _assert_page_contains(client, "/equipment/", ["批量维护", "批量维护设备", "批量维护设备人员关系"])
        _assert_subnav_clean(
            equipment_html,
        ...
        personnel_html = _assert_page_contains(
            client,
            "/personnel/",
            ["批量维护", "批量维护人员", "批量维护人员设备关系", "批量维护个人日历"],
        )
        _assert_subnav_clean(
            personnel_html,
        ...
        batches_html = _assert_page_contains(client, "/scheduler/batches", ["批量维护", "批量维护批次"])
        _assert_subnav_clean(
            batches_html,
        ...
        excel_pages = [
            ("/process/excel/routes", "批量维护路线文字", "零件工艺模板", "返回零件工艺模板"),
            ("/process/excel/part-operation-hours", "批量维护工序工时", "零件工艺模板", "返回零件工艺模板"),
            ("/process/excel/part-operations", "导出工序清单", "零件工艺模板", "返回零件工艺模板"),
            ("/process/excel/op-types", "批量维护工种", "工种配置", "返回工种配置"),
            ("/process/excel/suppliers", "批量维护供应商", "供应商配置", "返回供应商配置"),
            ("/equipment/excel/machines", "批量维护设备", "设备管理", "返回设备管理"),
            ("/equipment/excel/links", "批量维护设备人员关系", "设备管理", "返回设备管理"),
            ("/personnel/excel/operators", "批量维护人员", "人员管理", "返回人员管理"),
            ("/personnel/excel/links", "批量维护人员设备关系", "人员管理", "返回人员管理"),
            ("/personnel/excel/operator_calendar", "批量维护个人日历", "人员管理", "返回人员管理"),
            ("/scheduler/excel/batches", "批量维护批次", "批次管理", "返回批次管理"),
        ...
        assert "导出（零件工序清单.xlsx）" in part_ops_html
        ...
        assert "导入后自动生成批次工序" in batch_excel_html
        assert "上传 Excel 并检查" in batch_excel_html
        assert "自动生成/重建工序" not in batch_excel_html
        assert "上传并预览" not in batch_excel_html

        part_hours_html = _text(_get(client, "/process/excel/part-operation-hours"))

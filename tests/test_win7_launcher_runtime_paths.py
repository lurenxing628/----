"""回归测试：Win7 启动器运行时路径/锁/契约/停止流程——共享数据根与各数据目录解析（含 frozen/注册表/环境变量）、运行时所有者与 Chrome 配置目录、运行时锁的失败即关闭（空/非法/不可读锁不被清理），以及 stop_runtime_from_dir 在端点存活、契约不可读/非法、端点文件不全或不可信、Chrome 清理未确认时保留产物并返回 busy，仅在确认停止后才清理（兜底 PID 强杀仅在 PID 确认匹配且契约有效时允许）；另守护 .bat 启动脚本/.iss 安装器/.ps1 打包脚本的文本契约（停止流程、Chrome --user-data-dir 配置目录参数精确匹配等）。"""

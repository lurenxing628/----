...
async function cycleSave(p, page, data) {
  ...
  await save(p, 'source_confirm', b(source, '保存归属并继续'));
  const checked = await (await previewResponse).json(); assert.deepEqual(checked.data.affected_groups, []);
  ...
  assert.equal(await hours.getByLabel('外协组 40 至 40 总周期', {exact: true}).inputValue(), '9.5');
  await p.click(b(hours, '保存工时')); await b(page, '按 0 保存').waitFor();
  await p.shot('hours-group-only-editor');
  const payload = await save(p, 'hours_confirm', b(page, '按 0 保存'));
  assert.equal(payload.confirm_zero_unit_hours, true);

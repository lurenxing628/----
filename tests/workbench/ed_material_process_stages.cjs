...
async function processStages(p, page, data) {
  ...
  assert((await source.getByRole('button', {name: /^检查归属/}).getAttribute('title')).includes('先确认路线'));
  ...
  await p.click(b(source, '检查归属')); await source.getByRole('alert').first().waitFor(); await p.shot('source-unconfirmed-rejected');
  ...
  assert.equal(checked.data.affected_groups.length, 0); await p.shot('source-real-preview');
  await saved(p, 'source_confirm', b(source, '完成归属 · 解锁工时'));
  ...
  await p.click(hours.getByRole('checkbox', {name: '确认本页已核对工时', exact: true}));
  await p.click(b(hours, '保存工时')); await hours.getByText('单件工时为 0，需要明确勾选复核。', {exact: true}).waitFor();
  await p.shot('hours-zero-needs-review');
  await p.click(hours.getByRole('checkbox', {name: '已复核单件工时为0', exact: true}));
  await p.shot('hours-real-confirm'); await saved(p, 'hours_confirm', b(hours, '保存工时'));

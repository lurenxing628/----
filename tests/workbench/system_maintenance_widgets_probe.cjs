...
async function createBackup(page) { await page.getByRole('button', { name: '新增备份', exact: true }).click(); await page.getByRole('dialog').getByRole('button', { name: '确认新增', exact: true }).click(); }
...
(async () => {
  ...
  try {
    ...
    for (const viewport of [{ width: 1920, height: 1080 }, { width: 1392, height: 924 }]) for (const theme of ['light', 'dark']) {
      ...
      assert.equal(await page.getByRole('button', { name: '新增备份', exact: true }).isEnabled(), true);
      ...
      mode = 'disabled'; await page.getByRole('button', { name: '刷新清单' }).click(); await page.getByText('文件动作禁用：' + fixture.disabled.data.capabilities.blocked_reason, { exact: true }).waitFor();
      assert(await page.getByRole('button', { name: /^新增备份/ }).isDisabled()); await shot(page, variant + '-journal-disabled'); passed('missing-journal-disables-file-actions', variant);
      ...
      assert((await page.getByLabel('日志读取范围').textContent()).includes('已截断'));
      assert((await page.getByLabel('日志读取范围').textContent()).includes('来源读取失败'));
      assert((await page.getByLabel('日志读取范围').textContent()).includes('来源不存在'));
      ...
      for (const bad of ['json', 'disguised-json', 'mime', 'empty', 'magic']) {
        ...
        await page.getByRole('button', { name: '导出这段日志 CSV' }).click(); await page.getByRole('alert').waitFor();
      ...
      assert(await page.getByRole('button', { name: '导出这段日志 CSV' }).isDisabled()); await page.getByRole('button', { name: '刷新清单' }).click();
      ...
      await page.getByRole('button', { name: '保存维护配置', exact: true }).click(); await page.getByText('八项维护配置已保存，操作记录已留存。').waitFor();
      ...
      await page.getByText('八项维护配置已保存，操作记录已留存。').waitFor(); assert.equal(report.requests.filter(row => row.method === 'POST').length, postCount);
    ...
    await test.page.evaluate(async () => {
      const timer = window.setTimeout, A = window.SystemMaintenanceAPI, intent = { action: 'create', summary: '新增备份', request_key: 'system-' + '6'.repeat(48) };
      ...
      try {
        ...
        let rejected = false; try { await api.command(intent, 'fixture-only-token', {}); } catch (error) { rejected = error.message === window.WorkbenchTerms.outcomes.pending('维护操作') && !error.rejected; }
    ...
    for (const scenario of ['normal', 'disconnect', 'not-recorded', 'not-found', 'wrong-kind', 'unknown', 'rejected', 'checking', 'rollback_failed', 'recovery_required', 'rolled_back', 'failed']) {
      ...
      await test.page.getByRole('region', { name: '上次维护操作的结果' }).waitFor();
      ...
      if (['normal', 'rolled_back', 'failed', 'rejected'].includes(scenario)) { await acknowledge(test.page); }
        ...
        else await test.page.getByRole('button', { name: '查询结果', exact: true }).click();
        await test.page.waitForFunction(() => !document.querySelector('button[aria-busy="true"]'));
        if (scenario === 'disconnect' || scenario === 'wrong-kind') await acknowledge(test.page);
          ...
          assert(await test.page.getByRole('button', { name: /^新增备份/ }).isDisabled());
          const count = report.requests.filter(row => row.method === 'POST').length; await test.page.reload();
          await test.page.getByRole('region', { name: '上次维护操作的结果' }).waitFor(); await test.page.waitForFunction(() => !document.querySelector('button[aria-busy="true"]'));
    ...
    await test.page.getByText('配置没有变化，没有写入新的操作记录。').waitFor(); await acknowledge(test.page); passed('ordinary-unchanged-receipt', 'actions'); await test.context.close();
    ...
    await test.page.getByRole('button', { name: '查询结果', exact: true }).click(); await test.page.waitForFunction(() => !document.querySelector('button[aria-busy="true"]'));
    ...
    await test.page.getByRole('button', { name: '查询结果', exact: true }).click(); await test.page.getByText('维护已结束，请重启整个软件',{exact:true}).waitFor();
    assert.equal(await test.page.getByRole('button', { name: '确认结果', exact: true }).count(),0);
    ...
    assert(report.requests.slice(corruptStart).every(row=>row.path.endsWith('/restore-host'))); assert.equal(await test.page.getByRole('button', { name: /^新增备份/ }).count(), 0);
    ...
    assert.equal(await test.page.getByRole('heading', {name:'系统已暂停，维护结果待确认',exact:true}).count(), 0);
    assert.equal(await test.page.evaluate(() => window.systemReadSuspended), true);
    assert.equal(await test.page.getByRole('link', {name:'返回工作台',exact:true}).getAttribute('href'), '/workbench?view=system');
    assert.equal(await test.page.getByRole('button', {name:/^新增备份/}).count(), 0);

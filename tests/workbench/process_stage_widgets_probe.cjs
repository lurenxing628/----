...
async function cases() {
  ...
  await caseOf('draft-tab-close-and-latest-context-review', async () => {
    ...
    await page.evaluate(()=>{fixtureState.part.operations[0].unit_hours=4;fixtureState.revision++;});await button('刷新详情并保留草稿').click();await page.getByRole('table',{name:'最新资料差异'}).waitFor();assert.equal(await unit.inputValue(),'8.5');assert(await page.getByRole('button',{name:/^保存工时/}).isDisabled());
    assert((await page.getByRole('table',{name:'最新资料差异'}).innerText()).includes('4'));await button('采用最新资料').click();assert.equal(await unit.inputValue(),'8.5');
  ...
  await caseOf('source-fieldwise-rebase-and-readable-review', async () => {
    ...
    await button('刷新详情并保留草稿').click();const review=page.getByRole('table',{name:'最新资料差异'});await review.waitFor();const text=await review.innerText();

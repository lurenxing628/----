...
async function detailsAndCatalogs(p) {
  ...
  await p.step('long-text-detail-revisions-keyboard-close', async () => {
    ...
    const detail = work.getByRole('region', { name: '工序报表详情' });
    ...
    assert((await detail.locator('.rw-pagination').textContent()).includes('第 2 / 2 页'));
  ...
  await p.step('review-navigation-restores-report-tab-sort-page', async () => {
    ...
    await p.read(() => work.locator('.rw-header').getByRole('button', { name: '执行复盘', exact: true }).click());

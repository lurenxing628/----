...
async function exercise(p, phase) {
  ...
  if (phase === 'restart') {
    await p.step(['WBP-CALIB-004', 'WBP-CALIB-006'], 'real-restart-retains-original-adoption-receipt', async () => {
      ...
      await page.getByRole('region', { name: '采用锁定回执', exact: true }).waitFor();
  ...
  await p.step(['WBP-CALIB-001', 'WBP-CALIB-002'], 'real-source-summary-typed-search-filters-sort-pagination', async () => {
    ...
    await p.read(() => table.getByRole('button', { name: '筛选原定额 h/件', exact: true }).click(), '/calibration/facets/old_unit_hours');
    await p.read(() => page.getByRole('dialog').getByRole('button', { name: '清除', exact: true }).click(), list);
    await p.read(() => table.getByRole('button', { name: '筛选有效样本', exact: true }).click(), '/calibration/facets/sample_count');
  ...
  await p.step(['WBP-CALIB-003', 'WBP-CALIB-006'], 'part-number-click-and-real-selected-sample-provenance', async () => {
    ...
    const previous = page.url(), scopeBefore = await page.getByLabel('排序字段', { exact: true }).inputValue();
    ...
    assert.equal(page.url(), previous); assert.equal(await page.getByLabel('排序字段', { exact: true }).inputValue(), scopeBefore);
  ...
  await p.step(['WBP-CALIB-004', 'WBP-CALIB-006'], 'preview-cancel-then-explicit-adopt-locked-original-receipt', async () => {
    ...
    await dialog.getByLabel('声明人', { exact: true }).fill('现场验收员');
    const preview = await p.read(() => dialog.getByRole('button', { name: '读取真实预览', exact: true }).click(), part + '/adopt-preview');
    ...
    await page.getByRole('button', { name: '预览采用', exact: true }).click();
    await p.read(() => dialog.getByRole('button', { name: '读取真实预览', exact: true }).click(), part + '/adopt-preview');
...
async function visuals(p) {
  await p.page.getByRole('button', { name: '查看采用回执', exact: true }).click();
  await p.page.getByRole('region', { name: '采用锁定回执', exact: true }).waitFor(); await p.shot('adoption-receipt-viewport');

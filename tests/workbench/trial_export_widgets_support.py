...
HEADERS = ['方案', '方案状态', '约束说明（非取舍评估）', '基准方案', '换型次数（次）',
           '批次', '产品', '数量（件）', '优先级', '交期（截至日）', '基准完工（工厂本地时间）', '方案完工（工厂本地时间）',
           '延期（h）', '完工提前（h，负数为延后）', '工序调整', '换设备', '交付风险',
           '草稿引用', '场景引用', '原来源引用', '场景保存时间（工厂本地时间）', '核对提示']
STATES = {'editing': '可继续试调', 'saved': '已保存', 'discarded': '已放弃', 'valid': '通过', 'warning': '有提示', 'blocked': '有阻断'}
PRIORITIES = {'normal': '普通', 'urgent': '急件', 'critical': '特急'}
RISKS = {'on_time': '可按期', 'overdue': '预计晚交', 'unavailable': '约束阻断，不可评估', 'invalid_data': '交期数据无效'}
MISSING = object()
...
def expected_note(data):
    return ('完整{}批次、{}道安排、{}道未排；不受分页或显示筛选限制。'.format(
        ...
        ('事实为场景保存时快照。' if data.get('scenario_ref') else '事实为本次页面读取内容，导出时不刷新。') +
...
def expected_rows(data):
    ...
    for batch in comparison['batches']:
        yield [data.get('name') or '未命名试调草稿', STATES[data['status']], constraints,
               data['base_identity'].get('display_name') or '原排产候选',

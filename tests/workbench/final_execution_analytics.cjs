...
async function exercise(p) {
  ...
  let result = await p.read(() => page.locator('.sidebar a[href$="?view=review"]').click(), suffix);
  ...
  await p.step(['WBP-REVIEW-003', 'WBP-REVIEW-004', 'WBP-REVIEW-005'], 'review-all-sorts-page-controls-and-complete-csv', async () => {
    ...
    result = await p.read(() => work.locator('#report-topic-panel > .rw-pagination').getByRole('button', { name: '下一页', exact: true }).click(), suffix);
    assert.equal(result.data.page.number, 2);
    result = await p.read(() => work.locator('#report-topic-panel > .rw-pagination').getByRole('button', { name: '上一页', exact: true }).click(), suffix);
    assert.equal(result.data.page.number, 1);
    result = await p.read(() => p.choose('每页数量', '50'), suffix);
    const first = result;
    result = await p.read(() => work.locator('#report-topic-panel > .rw-pagination').getByRole('button', { name: '下一页', exact: true }).click(), suffix);
    ...
    result = await p.read(() => p.choose('每页数量', '20'), suffix);
  });
  ...
  await p.step(['WBP-REPORT-001.A002', 'WBP-REPORT-010'], 'report-keyboard-topics-and-original-task-report-navigation-return', async () => {
    await p.read(() => work.locator('.rw-header').getByRole('button', { name: '报表中心', exact: true }).click(), suffix);
    let active = work.getByRole('tab', { selected: true });
    for (const [key, topic] of [['End', 'quality'], ['Home', 'delivery'], ['ArrowLeft', 'quality'], ['ArrowRight', 'delivery']]) {
      ...
      active = work.getByRole('tab', { selected: true }); assert(await active.evaluate(node => document.activeElement === node));
    }
    await work.getByRole('searchbox', { name: '搜索批次或工序', exact: true }).fill('OP1');
    result = await p.read(() => work.getByRole('button', { name: '查询范围', exact: true }).click(), suffix);
    assert.equal(result.data.page.total, 1);
    result = await p.read(() => p.choose('每页数量', '10'), suffix);
    ...
    const detail = work.getByRole('region', { name: '工序报表详情', exact: true }); await detail.waitFor();
    ...
    assert.equal(field.data.tasks.find(task => task.task_ref === operation.task_ref).operation_ref, operation.operation_ref);
    await page.locator('[data-field-task="' + operation.task_ref + '"] + tr .field-detail').waitFor();
    ...
    await detail.getByText('整道完成', { exact: true }).waitFor();
    assert((await detail.locator('.rw-pagination').innerText()).includes('第 2 / 2 页'));

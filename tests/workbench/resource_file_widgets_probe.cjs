...
async function resourceCases(resource){
  ...
  await run(name+'-ack-preview-reset-and-confirm',async()=>{
    ...
    assert(await page.getByRole('button',{name:/^确认导入/}).isDisabled());await select('预检明细筛选','涉及引用的更新 1');
  ...
  for(const mode of ['import','bulk'])await run(name+'-'+mode+'-unknown-reload-same-key',async()=>{
    await mount({...resource,mode,rows:2,pending:true});if(mode==='import')await file();await start();await page.getByRole('checkbox').check();await page.getByRole('button',{name:mode==='import'?'确认导入':'确认删除',exact:true}).click();await page.getByRole('button',{name:'查询原请求回执',exact:true}).waitFor();
    ...
    await page.evaluate(()=>mountFixture({...JSON.parse(sessionStorage.getItem('fixture-resume')),recovery:true},true));await page.getByRole('button',{name:'查询原请求回执',exact:true}).waitFor();assert.equal(await page.evaluate(()=>fixture.commands.length+fixture.previews.length),0);assert.equal(await page.evaluate(()=>sessionStorage.getItem('fixture-command-count')),'1');
    await page.reload();await page.getByRole('button',{name:'查询原请求回执',exact:true}).waitFor();assert.equal(await page.evaluate(()=>fixture.lookups[0]),intent.request_key);assert.equal(await page.evaluate(()=>fixture.commands.length+fixture.previews.length),0);assert.equal(await page.evaluate(()=>sessionStorage.getItem('fixture-command-count')),'1');
    assert((await page.locator('.modal-h2').innerText()).includes(resource.label));assert.equal(await page.locator('[role="dialog"] input').count(),0);assert.equal(await page.getByText(/当前为示例数据|尚未读取生产资料|本次明确选中/).count(),0);
    await shot(mode+'-recovery');await page.evaluate(()=>{fixture.ready=true;});await page.getByRole('button',{name:'查询原请求回执',exact:true}).click();await page.waitForFunction(()=>fixture.committed.length===1);assert.equal(await page.evaluate(()=>fixture.commands.length),0);await page.getByRole('button',{name:'完成',exact:true}).click();
  });
...
async function guardCases(){
  for(const [name,spec,message] of [
    ['wrong-category',{kind:'op_type',category:'internal',label:'自制工种',wrongCategory:true},'预检工种类别与当前自制或外协范围不一致，本批未提交。'],
    ['missing-columns',{missingColumns:true},'预检缺少完整业务列名，本批未提交。'],
    ['private-fields',{privateFacts:true},'预检明细包含未说明的字段或非公开关系内容，本批未提交。'],
    ['nested-internal-fields',{nestedFacts:true},'预检明细包含未说明的字段或非公开关系内容，本批未提交。'],
    ['bad-summary',{malformed:true},'预检统计与明细不一致，本批未提交。'],
    ['wrong-refs',{wrongRefs:true},'删除预览与明确选中的设备不一致，本批未提交。'],
  ...
  await run('guard-missing-category-template-and-file',async()=>{await mount({kind:'op_type',label:'工种',mode:'import'});await file();await page.getByRole('button',{name:'下载模板',exact:true}).click();await page.getByText(/工种类别缺失/).waitFor();await start(false);await page.getByText(/工种类别缺失/).waitFor();assert.equal(await page.evaluate(()=>fixture.previews.length+fixture.downloads.length),0);});
  ...
  await run('guard-category-export-mismatch',async()=>{await mount({...resources[1],mode:'export',wrongCategory:true});await page.getByRole('radio',{name:/全部外协工种/}).check();await start(false);await page.getByText('导出预检工种类别与当前范围不一致，未开始下载。',{exact:true}).waitFor();assert.equal(await page.evaluate(()=>fixture.downloads.length),0);});
  ...
  await run('guard-empty-and-mismatched-selection',async()=>{await mount({...resources[2],mode:'bulk',selectionCount:0});await start(false);await page.getByText('未选中设备，本次不会删除任何记录。',{exact:true}).waitFor();assert.equal(await page.evaluate(()=>fixture.previews.length),0);for(const wrongCount of [false,true]){await mount({...resources[2],mode:'export',selectionCount:0,wrongCount});await page.getByRole('radio',{name:/已选设备/}).check();await start(false);await page.getByText(wrongCount?'导出预览数量与明确选中的设备不一致，未开始下载。':'0',{exact:true}).waitFor();assert.deepEqual(await page.evaluate(()=>fixture.previews[0].body.refs),[]);}});
  await run('guard-malformed-receipt-locks',async()=>{await mount({...resources[2],mode:'bulk',rows:2,malformedReceipt:true});await start();await page.getByRole('checkbox').check();await page.getByRole('button',{name:'确认删除',exact:true}).click();await page.getByRole('button',{name:'查询原请求回执',exact:true}).waitFor();assert(await page.getByRole('button',{name:/^取消/}).isDisabled());assert.equal(await page.evaluate(()=>fixture.committed.length),0);});
  ...
  await run('guard-mime-filename-signature',async()=>{for(const spec of [{badDownload:true},{badFilename:true}]){await mount({...resources[3],mode:'import',...spec});await page.getByRole('button',{name:'下载模板',exact:true}).click();await page.getByText(spec.badDownload?'下载内容或文件类型不正确，未把错误响应保存为文件。':'下载文件名与所选格式不一致，未保存文件。',{exact:true}).waitFor();assert.equal(await page.getByText(/已交给浏览器下载/).count(),0);}await mount({...resources[3],mode:'import'});await page.getByLabel('选择人员导入文件').setInputFiles({name:'invalid.xlsx',mimeType:'application/octet-stream',buffer:Buffer.from('not a workbook')});await start(false);await page.getByText('文件内容不是 XLSX 工作簿，请勿只修改扩展名。',{exact:true}).waitFor();assert.equal(await page.evaluate(()=>fixture.previews.length),0);});
}
(async()=>{

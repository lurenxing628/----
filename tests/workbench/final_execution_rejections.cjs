...
async function exercise(p) {
  ...
  for (const [label, startValue, endValue, quantityValue, hoursValue, status] of [
    ['future', '2099-01-01T08:00', '2099-01-01T10:00', '2', '1', 422],
    ['reversed', '2026-09-02T08:00', '2026-09-02T07:00', '2', '0', 422],
    ['hours-exceed-span', '2026-09-02T08:00', '2026-09-02T10:00', '2', '3', 422],
  ...
  await p.step(['WBP-FIELD-010.A006', 'WBP-FIELD-018.A008'], 'actual-sql-abort-preserves-pending-original-and-no-duplicate-post', async () => {
    ...
    p.report.pending_request_key = JSON.parse(posted[4].body).request_key;
    ...
    assert.equal(p.report.requests.filter(request => request.method === 'POST').length, 5);

...
async function outsourcing(h) {
  ...
  await mark('WBP-DASH-009.open-registration', () => request('/outsourcing/targets', () => page.getByRole('button', { name: '新建外协登记', exact: true }).click()));
  await mark('WBP-DASH-010.cancel', () => dialog().getByRole('button', { name: '取消', exact: true }).click());
  await request('/outsourcing/targets', () => page.getByRole('button', { name: '新建外协登记', exact: true }).click());
  await dialog().getByLabel('选择工序 XB1-10', { exact: true }).check();
  await dialog().getByLabel('外协声明人', { exact: true }).fill('F shipping clerk');
  await dialog().getByLabel('外协核实原因', { exact: true }).fill('F dispatch handover verified');
  async function invalid(id) {
    await mark('WBP-DASH-010.' + id, async () => { await dialog().getByRole('button', { name: '预览核对', exact: true }).click(); await dialog().getByRole('alert').waitFor(); });
  }
  ...
  await mark('WBP-DASH-010.reject-future-sent', () => request('/outsourcing/receipts/preview', () => dialog().getByRole('button', { name: '预览核对', exact: true }).click(), 422, 'POST'));
  ...
  await mark('WBP-DASH-010.reject-future-return', () => request('/outsourcing/receipts/preview', () => dialog().getByRole('button', { name: '预览核对', exact: true }).click(), 422, 'POST'));
  await dialog().getByRole('button', { name: '清空实际回厂', exact: true }).click();
  await select('外协确认状态', '已回厂'); await invalid('reject-state-time'); await select('外协确认状态', '在途');
  async function commit(id) {
    const preview = await mark('WBP-DASH-010.preview', () => request('/outsourcing/receipts/preview', () => dialog().getByRole('button', { name: '预览核对', exact: true }).click(), 200, 'POST'));
    ...
    await dialog().getByText('已确认：外协登记已保存，回厂不等于工序完工。', { exact: true }).waitFor();
    await mark('WBP-DASH-010.original-receipt', () => request('/outsourcing/receipts', () => dialog().getByRole('button', { name: '完成核实并刷新', exact: true }).click()));
  ...
  await page.locator('[data-detail-ref]').getByRole('button', { name: '原外协物流登记', exact: true }).click();
  await page.locator('[data-dashboard-outsourcing]').waitFor(); assert.equal(await page.locator('.wb-current-plan').count(), 0);
  await mark('WBP-DASH-014.return-context', () => page.getByRole('button', { name: '返回原值班台条目', exact: true }).click());

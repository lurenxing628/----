...
assert.equal(P.stage({ stage: 'awaiting_reconciliation' }), '核对运行记录');
assert.equal(P.stage({ stage: 'unexpected' }), '运行阶段未知');
const run = { accepted_at: '2026-09-12T12:00:00', started_at: '2026-09-12T12:01:00', finished_at: '2026-09-12T13:02:03' };
assert.equal(P.elapsed(run), '1 小时 1 分 3 秒');
assert.equal(P.elapsed({ ...run, started_at: null }), '1 小时 2 分 3 秒');

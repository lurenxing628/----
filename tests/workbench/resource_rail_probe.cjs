...
(async()=>{
  ...
  try {
    ...
    for(const viewport of [{width:1920,height:1080},{width:1392,height:924}]) for(const theme of ['light','dark']) {
      ...
      assert.equal(await page.locator('.hb-rl2').innerText(),'未知');
      ...
      await mode(page,'false-ready');assert.equal(await page.locator('.hb-rl2').innerText(),'未知');
      ...
      assert.equal(await page.locator('.hb-rl2').innerText(),'无法核实');
      ...
      assert.equal(await page.locator('.hb-rl2').innerText(),'待读取');

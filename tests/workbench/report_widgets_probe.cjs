...
const names = ['WorkbenchCaption.jsx', 'WorkbenchPageContext.jsx', 'ResourceControls.jsx', 'ReportAPI.js', 'ReportControls.jsx', 'ReportTable.jsx', 'ReportDetail.jsx', 'ReviewChartViews.jsx', 'ReviewCharts.jsx', 'ReportCatalog.jsx', 'ReportWorkspace.jsx', 'ReviewWorkspace.jsx'];
...
const html = '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
  ...
  '</head><body class="aps-workbench"><div id="root"></div>' + manifest.scripts.filter(file => !file.endsWith('/main.js')).map(file => '<script src="/static/' + file + '"></script>').join('') +
...
(async () => {
  ...
  try {
    ...
    for (const viewport of [{ width: 1920, height: 1080 }, { width: 1392, height: 924 }]) for (const theme of ['light', 'dark']) {
      ...
      await work.getByLabel('每页数量', { exact: true }).selectOption('10');
      await page.waitForFunction(() => document.querySelector('.rw-pagination')?.textContent.includes('1 / 3'));
      await work.getByRole('button', { name: '下一页', exact: true }).click();
      await page.waitForFunction(() => document.querySelector('.rw-pagination')?.textContent.includes('2 / 3'));
      ...
      const detail = work.getByRole('region', { name: '工序报表详情' }); await detail.getByText('整道完成', { exact: true }).waitFor();
      ...
      const cells = await work.locator('[role="tabpanel"] > .rw-table-scroll table tbody tr').evaluateAll(rows => rows.map(row => Array.from(row.cells, cell => cell.textContent)));
      ...
      await page.waitForFunction(() => document.querySelector('.rw-pagination')?.textContent.includes('共 1 项'));
      await work.getByRole('button', { name: '清除筛选', exact: true }).click();
      await page.waitForFunction(() => document.querySelector('.rw-pagination')?.textContent.includes('共 23 项'));

"""回归测试：CalendarService 的 shift_hours 在全局 WorkCalendar 与个人 OperatorCalendar 上写入0小时能原样回读（get/policy_for_datetime/逐资源日历容量一致为0.0），且历史负工时在读侧不得静默归零、必须抛出指明 shift_hours 字段的 ValueError。"""
...
def test_calendar_shift_hours_roundtrip(db_path) -> None:
    ...
    try:
        ...
        projection = resource_calendars(conn, [{"source": "internal", "operator_id": "OP100"}], datetime(2026, 2, 10), datetime(2026, 2, 11))
        assert projection["operator", "OP100"]["available_hours"] == 0.0
        ...
        projection = resource_calendars(conn, [{"source": "internal", "operator_id": "OP100"}],
                                        datetime(2026, 2, 12), datetime(2026, 2, 13))
        assert projection["operator", "OP100"]["state"] == "unavailable"
        assert projection["operator", "OP100"]["available_hours"] is None
        assert projection["operator", "OP100"]["issues"][0]["code"] == "calendar_invalid"

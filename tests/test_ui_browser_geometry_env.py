...
from tests import regression_ui_browser_geometry_smoke as smoke
...
def test_find_chrome_explicit_bad_path_does_not_fallback(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(smoke, "_chrome_default_candidates", lambda: (("PATH:google-chrome", str(fallback)),))
...
def test_find_chrome_missing_local_can_skip(monkeypatch) -> None:
    ...
    monkeypatch.setattr(smoke, "_chrome_default_candidates", lambda: ())
...
def test_find_node_missing_local_can_skip(monkeypatch) -> None:
    ...
    with pytest.raises(pytest.skip.Exception) as exc_info:
        smoke._find_node_with_browser_runtime()
...
def test_find_node_capability_failure_reports_version(monkeypatch) -> None:
    monkeypatch.setenv("APS_BROWSER_SMOKE_REQUIRED", "1")
    monkeypatch.setattr(smoke.shutil, "which", lambda _name: "/tmp/node")
    monkeypatch.setattr(smoke.os.path, "realpath", lambda value: str(value))
    ...
    monkeypatch.setattr(smoke, "_run_probe_command", fake_run)

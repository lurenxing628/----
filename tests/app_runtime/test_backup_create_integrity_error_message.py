...
校验失败抛裸 RuntimeError（R32/O27 loud：坏库绝不升正式）时，web 入口必须转成具体中文提示
...
class _BackupIntegrityBoomManager:
    """backup() 抛裸 RuntimeError，模拟完整性校验执行失败/未通过。"""
...
def test_backup_create_integrity_runtime_error_shows_chinese_not_500(app_client, db_env) -> None:
    ...
    with mock.patch(
        ...
        assert resp.status_code in (302, 303), (
            f"完整性校验失败应重定向回备份页、不应 500，实际 {resp.status_code}"
        )

        html = client.post(
            "/system/backup/create", data={}, follow_redirects=True
        ).data.decode("utf-8", errors="ignore")
        assert "备份完整性校验未通过" in html, "应展示具体中文提示而非笼统 500"

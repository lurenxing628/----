...
def aggregate(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "worst_runtime_ms": max(runtimes) if runtimes else None,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    workers = require_serial_workers(args.workers)
    source_before = capture_source(REPO_ROOT)
    rows = list(map(run_seed, range(int(args.seeds))))
    source_after = capture_source(REPO_ROOT)
    binding = source_binding(source_before, source_after)
    payload = {
        "schema_version": COMPARE_SCHEMA_VERSION,
        "status": "passed" if rows and all(row.get("status") == "passed" for row in rows) and binding != "unbound_source_changed" else "failed",
        "tier": "long_run",

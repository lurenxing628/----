...
def test_gantt_page_version_default_latest(tmp_path, monkeypatch) -> None:
    ...
    page_resp = client.get("/scheduler/gantt?view=machine&week_start=2026-03-02&version=abc")
    assert page_resp.status_code == 200
    assert "第 7 版" in page_resp.get_data(as_text=True)

    page_zero_resp = client.get("/scheduler/gantt?view=machine&week_start=2026-03-02&version=0")
    assert page_zero_resp.status_code == 200
    assert "第 7 版" in page_zero_resp.get_data(as_text=True)

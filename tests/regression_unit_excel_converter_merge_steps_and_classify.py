"""回归测试：UnitExcelConverter.convert 把“单元产品信息统计”表的 XX-X 工步并入所属工序并判定内外协（有工步且有人员=自制，否则外协）、解析“工号 姓名 姓名”人员列、累计工步时间为工序工时（小时）、同名工种自制/外协并存时给外协侧加“（外协）”、并按主模板列补默认值（技能等级 normal/主操设备 no/供应商状态启用）输出 7 张模板且与导入预览链路兼容。"""

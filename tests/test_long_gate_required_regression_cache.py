...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda repo_root: _required_proof_path(repo_root).write_text(
...
def test_required_tracked_scope_changes_force_group_rerun(monkeypatch, tmp_path, changed_path):
    module = _import_run_quality_gate()
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    command_plan = _real_quality_gate_plan()
...
def test_required_environment_changes_force_group_rerun(monkeypatch, tmp_path, env_key):
    module = _import_run_quality_gate()
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    command_plan = _real_quality_gate_plan()

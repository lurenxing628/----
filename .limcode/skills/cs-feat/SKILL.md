...
需求清楚 + 范围小时走完整四阶段太啰嗦。fastforward 不写 design / checklist / acceptance，直接实现，验证完成后只回写一份 `{slug}-ff-note.md`。触发："快速模式"、"fastforward"、"直接开干"、"别那么多步骤"，去 `cs-feat-ff`。

**别走** fastforward：跨多个子系统、有术语冲突风险、需要分多步验收——这些情况跳过 design 意味着 AI 和用户没共同确认过同一份方案，实现完容易发现彼此理解不一样。
...
| fastforward 代码已写完但还没 `{slug}-ff-note.md` | `cs-feat-ff` 收尾 |

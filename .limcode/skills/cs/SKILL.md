...
CodeStable 把开发活动组织成**项目内产物 + 3 条主流程**，所有产物聚在 `.codestable/`。下面是日常速读图，完整目录以 `.codestable/reference/shared-conventions.md` 为准：

```
.codestable/
├── requirements/    需求实体（"为什么要有这个能力"，覆盖过去 / 现在 / 未来）
...
└── brainstorms/     开放脑暴材料（大需求还没拆 roadmap 前的发散记录）
```

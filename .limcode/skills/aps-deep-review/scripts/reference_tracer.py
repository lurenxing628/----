...
def _classify_layer(filepath: str) -> str:
    if filepath.startswith("web/routes/"):
        return "Route"
    if filepath.startswith("web/bootstrap/"):
        return "Bootstrap"
    if filepath.startswith("web/viewmodels/"):
        return "ViewModel"
...
def generate_reference_report(repo_root: str, changed_files: List[str]) -> str:
    ...
    for changed_file in changed_files:
        ...
        skip = any(norm.startswith(p) for p in ["tests/", ".limcode/", "evidence/", "backups/"])
        if skip:
            continue
...
def main():
    ...
    if not changed_files:
        _safe_print("未找到变更的 Python 文件。")
        return 0

    _safe_print(f"分析 {len(changed_files)} 个变更文件的引用链...")

...
3. **生成 `manifest.yaml`**——所有条目初始 `status: pending`；落盘后 `python .codestable/tools/validate-yaml.py --file docs/api/manifest.yaml --yaml-only` 校验；展示给用户 review
...
选定条目 → 读 source_files → 按模板生成 → 用户 review → 落盘 → `python .codestable/tools/validate-yaml.py --file {路径} --require doc_type --require entry --require status` → manifest 对应条目 `status: current`
...
3. **批量生成**——剩余 `pending` 逐条走"读源码 → 提取 → 生成"，可按清单分批处理；每条 `status: draft`
4. **整体 review**——批量完成展示概况（条目数 / 跳过数 / 待确认数）；review 前先 `python .codestable/tools/validate-yaml.py --dir docs/api --markdown-only --require doc_type --require entry --require status` 批量校验条目文档；`manifest.yaml` 仍用 `python .codestable/tools/validate-yaml.py --file docs/api/manifest.yaml --yaml-only` 单独校验
...
- `python .codestable/tools/search-yaml.py --dir docs/api --filter status=outdated`——架构 check 或上次更新已标记的
- 对比 `manifest.yaml` 里 `last_scanned` 之后变更的源码文件
- `python .codestable/tools/search-yaml.py --dir docs/api --sort-by last_reviewed --order asc` 按最久没复核的排在前主动复核

重新读源码 → 对比已有文档 → 增量更新变化部分 → `python .codestable/tools/validate-yaml.py --file {路径} --require doc_type --require entry --require status` 校验 → `status: current` + `last_reviewed` 当天。

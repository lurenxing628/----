...
| **case 2：小需求** | 单 feature | 知道要解决什么问题，对解法 / 边界还摇摆 | `.codestable/features/YYYY-MM-DD-{slug}/{slug}-brainstorm.md` → `cs-feat-design` |
...
**spike 落地约定**：
- case 2：实验代码扔 `.codestable/features/YYYY-MM-DD-{slug}/` 下（和 brainstorm note 同目录），文件随便起名（`spike.py` / `try-{topic}.ts`）
...
**落盘**：收敛完成后写 `.codestable/features/YYYY-MM-DD-{slug}/{slug}-brainstorm.md`。

...
**backfill / update 额外**：目标模块代码入口和核心文件 + 用户素材 + 相关 compound 沉淀（decision / explore / learning）+ 相关已有 feature 方案。compound 检索命令见 `reference.md` 第 5 节，产出自查见 `reference.md` 第 2 节。**update 专项**：当前 doc 全文 + `last_reviewed` 之后的代码变更（`git log` 粗扫）。

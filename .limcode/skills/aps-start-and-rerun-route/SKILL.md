...
description: 启动 APS 项目、在真实数据库上重跑复杂排产样例、校验 /scheduler/gantt/data，并打开最新版本的 /scheduler/gantt 页面。适用于用户提到启动项目、再跑一遍、重跑排产、打开或查看 scheduler gantt、修改系统后再验证一次等场景。
---
...
- 重跑排产（默认）：`python .limcode/skills/aps-start-and-rerun-route/scripts/run_start_and_rerun_route.py`
- 仅启动项目：`python .limcode/skills/aps-start-and-rerun-route/scripts/run_start_and_rerun_route.py start-only`
- 默认行为：

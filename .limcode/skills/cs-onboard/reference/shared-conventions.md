...
│       ├── {slug}-intent.md      （可选，用户自写的 design 前置草稿）
│       ├── {slug}-design.md      （标准流程）
...
├── brainstorms/           开放 brainstorm 创意空间（cs-brainstorm case 4 产出）
│   └── {slug}/            一次开放脑暴一个子目录，固定 brainstorm.md
│                          spike / 实验文件可放同目录，结论回写到 brainstorm.md
...
- audit 目录：带日期前缀 `audits/YYYY-MM-DD-{slug}/`，目录内固定 `index.md` + `finding-NN.md`
...
- index 文档 `doc_type: audit-index`
- 单条发现 `doc_type: audit-finding`
- 审计只负责列发现；用户明确要求修复时，修复动作仍按 issue / refactor 的边界处理，小 bug 也走 `cs-issue` 快速通道，不由 audit 直接改
...
5. **起草前先查重叠**——动手写前用 `python .codestable/tools/search-yaml.py --dir .codestable/compound --query "{关键词}"` 查语义相近的旧文档。命中就把候选列给用户在三条路径里选：

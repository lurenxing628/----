...
3. **写一份空的 `{slug}-intent.md`** 作为草稿骨架，模板见同目录 `reference.md` 的"0. `{slug}-intent.md` 空模板"。
...
5. **新逻辑按职责归属放置**——已有职责的延伸优先留在现有模块，独立职责或明确复用需求再新建文件，不为压低行数机械拆分
...
涉及目标、业务取舍、安全或明显成本差异且无法从现有约束判断时，向用户澄清。可逆的内部细节采用项目现有做法并说明必要假设，不逐处打断。具体：

- **声明假设**：非用户原话的判断写成"假设：……"，让用户能精确反驳
- **重要取舍给选项**：确有多种实质不同做法时比较并给出倾向，不为凑数量制造伪选项
...
2. **扫 .codestable/ 全局输入**——Glob `.codestable/` 发现可用目录和文档类型，按类取用：
   ...
   - `compound/` → 用 `python .codestable/tools/search-yaml.py --dir .codestable/compound` 搜相关 decision / explore / trick / learning；命中冲突 decision 必须正面回应
...
按 `reference.md` 模板写第 2 节各子节（2.1 名词层 / 2.2 编排层 / 2.3 挂载点 / 2.4 推进策略 / 2.5 结构健康度与微重构）。重点提示：
...
- **第 2.5 节"结构健康度与微重构"是固定步骤**——详细写法见 `reference.md` 的"2.5 结构健康度与微重构"；`SKILL.md` 只强调硬边界：先查 compound 已有 convention，评估文件级 + 目录级，只允许"只搬不改行为"的微重构。
- 2.5 结论只能是：不做 / 微重构（拆文件） / 微重构（重组目录）。选择微重构时，把"搬什么 → 搬到哪 → 怎么验证行为不变"落进 checklist 作为第 1 步且独立验证退出。
- 超出"只搬不改行为"的观察（改签名、改返回结构、改调用语义、模块拆合）不阻塞本 feature，写进 2.5 的"超出范围的观察"，提示后续走 `cs-refactor`。

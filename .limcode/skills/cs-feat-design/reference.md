...
本文件提供 `cs-feat-design` 使用的 `{slug}-intent.md` / `{slug}-design.md` / `{slug}-checklist.yaml` 参考格式。

## 0. `{slug}-intent.md` 空模板
...
- `## 2. 名词与编排` ← design 的灵魂，是 implement 的主输入
  ...
  - `### 2.3 挂载点清单`
  - `### 2.4 推进策略`
  - `### 2.5 结构健康度与微重构` ← 固定节，结论从"不做 / 微重构（拆文件） / 微重构（重组目录）"中选一个 + 可选"超出范围的观察"
- `## 3. 验收契约`
- `## 4. 与项目级架构文档的关系`
...
checks:
  - item: "{检查项描述}"
    source: 名词契约  # 名词契约 | 编排骨架 | 流程级约束 | 挂载点 | 范围守护 | 验收场景
    status: pending
```

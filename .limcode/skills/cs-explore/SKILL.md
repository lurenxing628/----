...
- 旧探索过期：只读问答中说明过期；获准更新记录时才修改状态并保存当前版本

---

## 只读问答与归档

用户只是问实现方式、定位问题或要求只读研究时，查现行代码并直接回答，附证据与限制；不创建文件、不改旧报告状态，不先要求补齐流程目录。命中旧记录时核对关键事实再复用，不为“复用还是重探”重复询问。

以下归档阶段仅用于用户明确要求保存或更新探索记录。
...
- 证据数量按问题需要确定，每条都标注 `文件:行号`，不为了凑数量添加无关证据

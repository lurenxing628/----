...
> 重要提示：本文是 2026-05-23 的前端布局调研记录，只能作为视觉结构和外部资料参考。后续实现、分步顺序、接口契约、按钮放开条件、字段命名、测试要求，全部以 `.codestable/roadmap/aps-three-gap-directions/aps-three-gap-directions-roadmap.md` 和 `.codestable/roadmap/aps-three-gap-directions/aps-three-gap-directions-items.yaml` 为准。本文早期写到的“细明细”和复杂现场动作，已经被 roadmap 拆成更小的独立条目，不能直接照本文开工。
...
- **异常必须从日志里拿出来**：晚交、物料问题、资源冲突、产能不足、数据缺失等，应有“异常总览”和“为什么晚了”详情层。
...
3. 方案对比页在现有表格上方先补“推荐方案卡”和“好处/代价摘要”，批次和资源的细明细以后按 roadmap 单独评估。
4. 排产分析页把诊断卡升级成“异常总览”和“为什么晚了”侧栏。
5. 资源派工页作为车间反馈入口，但第一阶段先按 roadmap 做身份护栏、执行事件基础、开工/完工受控写入和最小重排护栏；暂停、继续、报异常放到后续独立条目。
...
| 12 | 多方案对比 | 最适合“先给推荐结论，再给总指标差异，再给后续可选明细” |
...
2. 当前首页已有四个状态卡，以及“最近排产”和“排产作业 / 基础资料 / 结果查看”一类工作区分组：`templates/dashboard.html:4-21`、`templates/dashboard.html:23-86`。
...
1. APS 排产工作台怎么排版。
2. MES / 车间端怎么做开工、完工，以及后续再补暂停和异常反馈。
...
| [Oracle MES Workstation](https://docs.oracle.com/cd/E18727_01/doc.121/e13057/T469821T469943.htm) | 派工列表、Job On、Clock In、Clock Out、异常反馈 | 说明现场动作必须记录实际时间和人员；当前 roadmap 先做开工/完工，暂停和异常后续单独做 |
| [Oracle MES Supervisor Workbench](https://docs.oracle.com/cd/E26401_01/doc.122/e48906/T469821T469939.htm) | 主管看板、工单状态、甘特、异常解决、重排工单 | 支持计划员端要看到计划和现场实际和异常影响 |
...
- 已有“正式采用方案、原算法代表方案、重点工序优先代表方案”三类代表方案。
...
- 把三套方案做成并排小卡：
  - 每张卡只放 4-6 个指标。
  - 与正式采用方案或原算法相比的差值要直接写出来。
  - 不要让评分成为主角。
- 第一版不承诺批次或资源的细明细；只保留推荐卡、三方案摘要、总指标差值和跳转入口。
- 批次、设备、人员的变化明细后续再按 roadmap 单独做。
...
- 操作按钮大而清楚。
- 后续遇到异常时再选原因、补备注。
- 计划员页面后续再看计划和现场反馈差距、异常记录、暂停时长和严重程度。
...
- 但当前还没有真正的开工、完工；暂停和异常反馈也还没有进入可用闭环。

按当前 roadmap 收窄后的第一阶段做法：
...
- 每个任务卡显示：
  ...
  - 是否超期。
- 按状态显示按钮的最终目标：
  - 未开工：开工，但按钮放开要等最小重排护栏通过。
  - 生产中：完工；暂停和报异常由后续异常反馈条目放开。
  - 暂停中：继续生产、完工；这一类由后续异常反馈条目放开。
  - 已完工：只看记录，不允许误操作。
- 按 roadmap 当前拆法，第一阶段只允许开工/完工先做受控写入；普通用户按钮要等最小重排护栏通过后再放开。
- 完工弹窗只收：
  - 完成数量。
  - 报废数量。
  - 备注。
- 后续异常/暂停原因下拉不属于当前开工/完工第一阶段，等 `shop-exception-feedback` 条目启动后再做：
  - 设备问题。
  - 物料问题。
  - 质量问题。
  - 工艺问题。
...
- 新增“今日待处理”列表：
  ...
  - 是否有方案对比结果可看。
  - 是否有停机影响。
- 首页入口继续保留“排产作业 / 基础资料 / 结果查看”这类简单分组，不要改成复杂角色门户。
...
- 车间反馈入口。
- 任务卡片视图。
- 第一阶段先预备开工、完工按钮；暂停、继续生产、报异常按钮按后续异常反馈条目做。
- 完工弹窗；异常弹窗按后续异常反馈条目做。
- 计划和现场实际对比列。
- 状态颜色：
  - 灰色未开工。
...
5. 用户要下发执行，进入资源派工。
6. 车间先完成开工、完工反馈；暂停和异常反馈后续完成后，计划员再回来复盘计划和实际差距。
...
| 第四层 | 工作区入口 | 排产作业、基础资料、结果查看、车间反馈 | 下一步能做哪些事情 |
...
- “可能线索：M-03 在同一天还有 4 道工序，当前利用率接近满负荷。”
- “后续还有 2 道工序会被一起推迟。”
...
- 用户能知道推荐方案的好处和代价。
- 第一版先让用户看到总指标差值；哪些批次提前/延后、哪些设备或人员变忙，按 roadmap 后续条目再决定是否做细表。
...
| material_shortage | 物料线索待核对 | 物料问题或物料时间晚；当前只作为待核对线索，不直接下结论 |
...
| 建议先复核的工序 | 工序、设备/人员、计划开始、计划结束、等待时长 |
| 可能线索 | 资源过载、停机、前序晚、物料问题、排不进去、数据缺失 |
...
| 计划开始 | 05-24 08:00 |
| 计划结束 | 05-24 18:00 |
| 当前状态 | 未开工 / 生产中 / 已暂停 / 异常中 / 已完工 |
| 计划偏差 | 未到时间 / 已晚 2 小时 / 提前 |
| 后续异常摘要 | 当前阶段不展示；异常反馈条目完成后再显示 |

卡片按钮按状态变化：

| 状态 | 主按钮 | 次按钮 | 禁用或隐藏 |
| --- | --- | --- | --- |
| 未开工 | 开工 | 暂不提供报异常 | 完工、继续 |
| 生产中 | 完工 | 暂停、报异常后续单独做 | 开工 |
| 暂停中 | 继续生产 | 完工、报异常，后续单独做 | 开工 |
| 异常中 | 继续生产 | 完工、查看异常，后续单独做 | 开工 |
...
- 任务状态变成“已完工”。
- 记录实际完成时间。
- 保留实际完成时间；计划和现场反馈差距按后续复盘条目集中计算。
- 如果晚完工，回写给排产分析或资源派工。

#### 5.5 后续暂停和异常反馈

> 当前 roadmap 第一阶段不按本小节直接开工。本小节只是历史调研留下的后续能力素材；真正实施时，要等 `shop-exception-feedback` 条目启动，并按 roadmap/items 里的字段、路由、中文文案和测试要求执行。
...
| --- | --- |
| 暂停开始时间 | 当前时间 |
| 原因 | 设备问题、物料问题、质量问题、工艺问题、人员问题、其他 |
| 预计恢复时间 | 可选 |
| 备注 | 可选 |
...
> 本小节属于后续 `plan-vs-actual-review` 素材，不属于当前开工/完工第一阶段。当前第一阶段只保留开工和完工受控写入；计划和现场反馈差距、异常时长、异常原因都等复盘和异常反馈条目完成后再集中展示。
...
- 车间人员能看到今天自己的任务。
- 未开工任务能点“开工”。
- 第一阶段生产中任务先能在受控条件下“完工”；“暂停、报异常”按 roadmap 后续条目单独做。
- 暂停任务的“继续生产”按 roadmap 后续条目单独做。
- 完工后能看到实际完成时间。
- 计划员能看到计划开始/结束和实际开始/结束。
- 后续异常反馈上线时，异常原因再用固定选项，不让用户每次从零输入；这不属于当前开工/完工第一阶段验收。
- 任何操作都有成功提示和失败提示。

### 6. 报表中心详细蓝图

报表中心现在已经有超期、资源负荷、停机影响统计。下一步要让它从“报表入口”变成“风险查看入口”。
...
- 在现有候选方案表上方加推荐方案卡。
- 加三方案摘要卡。
- 第一版不加批次级变化明细；只预留“后续版本再补明细”的空状态。
...
- 资源派工页新增车间反馈标签。
- 任务卡片展示。
- 第一阶段只做开工、完工受控写入；暂停、继续、报异常按 roadmap 后续条目单独做。
- 记录计划和现场实际。
...
- 未开工任务能在受控条件下开工。
- 生产中任务能在受控条件下完工。
- 暂停、继续生产、报异常不属于本条验收，按后续 `shop-exception-feedback` 验收。
- 计划和现场实际偏差按后续复盘条目验收。
...
1. 资源派工页加任务卡片。
2. 第一阶段只做开工、完工受控写入。
3. 暂停、继续、报异常按后续异常反馈条目做。
4. 计划员页面显示计划和现场实际按后续复盘条目做。
5. 后续重排尊重已开工/已完工事实。
...
2. `方案对比业务化`：推荐卡、三方案摘要、总指标差值。
3. `异常解释层`：异常总览、为什么晚了、影响链路、处理入口。
4. `车间反馈开工/完工预备`：资源派工页先增加开工、完工受控写入；暂停、继续生产、报异常后续单独做。

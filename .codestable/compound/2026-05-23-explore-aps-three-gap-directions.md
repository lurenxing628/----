...
| 多方案对比 | Deltek Acumen、Asta Powerproject、Microsoft Project 的基线对比 | 用“当前值、基线值、差值”帮助用户看变化；甘特上可显示 baseline vs current | 本项目可以先做“相对最终采用”的差值文案，后续再做批次级和甘特级对照 |
...
| 车间反馈 | Tulip work instructions、OpenMES、Oracle workstation queue | 工位页面围绕当前任务、操作指导、状态按钮和异常提交组织 | 本项目第一版可先做“任务卡 + 状态按钮 + 异常原因 + 备注”，不做完整作业指导 |
| 车间反馈 | Sepasoft / inmation / Symestic 的 OEE 和 downtime reason | 停机和异常要有原因目录，且原因目录要能后续统计 | 本项目第一版异常原因先分设备、人员、物料、质量、工艺、其他，不做完整 OEE |
| 国内中文口径 | 华为云 MBM、金蝶工序派工/报工、安达发 C1、数夫、MES 安灯资料 | 国内业务用户常见词是派工看板、工序派工、工序报工、异常上报、工单开工、暂停原因、处理完成 | 页面命名尽量用“方案为什么被采用、为什么会晚、开工/完工/报异常”，少用 What-if、candidate、pegging |
...
| 3 | 三方案摘要卡 | 最终采用、原算法最好、重点工序优先最好并排看 |
...
| 测试 | 验什么 |
| --- | --- |
| 有三类代表方案时显示推荐卡 | 页面能展示最终采用、原算法最好、重点工序优先最好 |
...
> 用户点开一个超期批次，系统能用中文说明：这批晚了多久，第一卡点是哪道工序，主要原因是什么，建议先看哪里。
...
第一卡点是第 30 工序“精加工”，计划使用设备 M-03。

系统判断主要原因是 M-03 接近满负荷：本周已安排 46.5 小时，可用 48 小时，利用率 96.9%。
...
- 所以本项目第一版不能说“根因一定是某某”，只能说“按当前排程数据看，最明显卡点是某某”。
...
| 第一卡点 | 最晚、最紧、最可能影响交期的工序或资源 | “最先需要复核第 30 工序精加工” |
| 可能原因 | 根据规则判断出的原因，不装成绝对真相 | “可能是 M-03 设备排满” |
...
| 证据不足 | 没有足够数据判断 | “暂时不能判断主要原因，请先查看甘特和工序明细” |
...
| `material_not_ready` | 物料未齐 | 批次齐套状态为 no 或 partial | 查看物料齐套 |
...
- planned_finish_time
- delay_hours
- first_blocking_op_id
- first_blocking_op_label
- primary_reason_code
...
- 没有物料数据时，不假装是缺料，只显示“物料状态缺失”或“不判断物料原因”。
- 没有车间反馈前，不判断“现场做慢了”。
...
> 让车间能把实际开工、完工、暂停、异常反馈回 APS，让计划员能看到计划和实际差距。
...
| 视角 | 用户 | 重点 | 页面样子 |
| --- | --- | --- | --- |
| 车间视角 | 班组长、操作员 | 今天要做什么、现在开工/暂停/完工/异常 | 任务卡、大按钮、少字段 |
| 计划视角 | 计划员、调度员 | 哪些任务没开工、哪些异常、计划和实际偏差 | 表格、筛选、异常汇总、跳转 |
...
| 当前状态 | 未开工 / 生产中 / 暂停 / 异常 / 已完工 | 现场状态 |
| 实际开始 | 05-24 08:32 | 开工后显示 |
| 实际结束 | 05-24 19:10 | 完工后显示 |
| 偏差 | 晚开工 32 分钟 / 晚完工 70 分钟 | 计划 vs 实际 |
...
如果选择“已停工”或“是否暂停生产 = 是”，系统可以同时生成 pause 状态。
...
| 当前状态 | 允许动作 |
| --- | --- |
| pending/scheduled | start, exception |
...
POST /scheduler/resource-dispatch/execution/<op_id>/exception
...
| pause/resume 能累计暂停时长 | 暂停时长计算正确 |
| exception 能记录原因和严重程度 | 计划员能查到异常 |

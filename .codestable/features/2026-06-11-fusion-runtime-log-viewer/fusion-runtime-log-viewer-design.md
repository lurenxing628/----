...
7. **报错出口接门双路径**（侦察纠偏：主战场是 templates/error.html 非 error_boundary）：① templates/error.html :4/:23 文案补「打开运行日志」链接，并新增**发生时刻**一行（render_error_template context 注入，服务端本地时间，格式与日志时间戳同 `YYYY-MM-DD HH:MM:SS`）——用户带着这个时刻去运行日志页按时间对位，把"看到错就去翻日志"变成"按时刻定位条目"；**链接用裸路径 `/system/runtime-logs` 不用 url_for**（实现期修订：错误页渲染于各种降级场景——含未挂 system bp 的最小 app，error_boundary 契约测试实证 url_for 会 BuildError 把富错误页打跌进兜底页；/system 前缀在 factory 注册时钉死，裸路径零路由依赖）；② error_boundary.py:344 minimal 兜底页是纯字符串拼接且渲染于"模板系统已坏"的场景——**只改文案加裸路径文本提示与同格式时刻**。errorhandler(500) 的 JSON payload 文案不动（API 消费方不需要页面链接）。

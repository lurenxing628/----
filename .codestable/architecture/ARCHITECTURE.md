...
- APS 工作台上下文链接合同：`web/viewmodels/scheduler_workbench_links.py` 统一封装工作台计划上下文、跨页链接、中文标签映射和现场写入地址护栏；`web/viewmodels/scheduler_workbench_link_query.py` 集中维护各目标页要带的版本、方案、日期、批次和资源参数矩阵（2026-06-13 把内部专用常量 `_TARGET_QUERY_SPECS`/`_CONTEXT_FREE_TARGETS`/两套禁键集「只搬不改」抽到同包 `web/viewmodels/scheduler_workbench_link_specs.py`，原文件由 500 行降到门禁内，对外签名零 diff）；`core/services/scheduler/resource_dispatch_page_context.py` 装配资源派工页面的版本、方案身份、筛选条件和可查询状态等只读查询上下文；`web/routes/domains/scheduler/scheduler_resource_dispatch.py` 接线资源派工只读复盘入口和写入入口状态。第 1 阶段已用于排产分析候选方案跳转、周计划入口、资源派工现场记录写入口控制和现场记录二级接口公开身份脱敏，避免这些页面各自拼 URL 时丢版本、方案、日期、批次或资源对象，也避免把内部计划身份交给前端当作写入凭证。壳层另有计划上下文胶囊（fusion-plan-context-capsule，契约 4.2）：`templates/base.html` top-header 常驻一行「版本 · 方案身份 · 生成时间 · 策略 · 数据范围」只读回显，`web/viewmodels/plan_context_capsule.py` 装配、模板全局 `workbench_plan_capsule` 注入；`build_workbench_plan_context` 扩 `generated_at_label/strategy_label` 两公开字段（_UNSET sentinel 区分「未喂参显示 -」与「喂了缺失值走词表缺失态」），6 个发布点（首页/甘特/分析/周计划/报表/资源派工）喂 raw 值、label 转换只在合同内一处；无 version 页面零渲染，URL 直入页面两项显示「-」不查库补。
- 公开上下文 token：页面需要保留模拟方案或工序保存身份时，不把 `scenario_id/op_id` 编进可解码的签名 payload；`web/public_token_registry.py` 在 Flask app 内存中登记随机公开 token，`scheduler_plan_context_token.py` 和 `scheduler_ops.operation_update_token()` 只把随机 token 放进 URL/HTML。token 默认 12 小时失效、重启后失效，解析失败走用户可见 ValidationError“请刷新页面后重试”。`dashboard`、报表入口、排产分析、甘特、周计划、资源排班和说明书回跳都能解析 `plan_context_token`；`scheduler_workbench_link_query.PLAN_CONTEXT_TOKEN_TARGETS` 决定哪些目标页输出 token。共享链接层如果只有服务端内部 `scenario_id`，会在 Flask app 上下文内派生 `plan_context_token`；没有 token 生成环境时禁用链接并提示刷新，不退回裸 `scenario_id`。公共回跳地址由 `current_public_return_url` 统一清洗，会丢弃 `scenario_id/op_id/schedule_id/candidate_id/source_table` 以及候选和工序保存相关内部字段；需要保留模拟方案上下文时只留下随机 `plan_context_token`。
...
- 首页计划员驾驶舱（fusion-dashboard-cockpit，2026-06-13，模块 C）：首页 `templates/dashboard.html` 单栏四段 = ①上下文胶囊（壳层 top-header `workbench_plan_capsule` 承接，本页不另渲）②hero 指令卡 ③6 格体检表 ④其余待处理清单。`web/routes/dashboard.py` 读取最新排产、正式采用方案范围、今日计划任务和现场事实，交给 `web/viewmodels/dashboard_workbench.py`（体检表六格：超期/待排/方案待确认/现场待确认/资源压力/基础数据，每格即入口可点跳、正常项灰显——替代旧 stat-grid，「待排数」承接被裁的「最近一次排产」卡唯一增量信息）与 `web/viewmodels/dashboard_workbench_cards.py`（待处理线索 + 阈值单源）装配，再由 `web/viewmodels/dashboard_cockpit_hero.py` 的 `build_cockpit_hero` 三态收口 hero：把待办队列第 1 条置顶为 hero 指令卡、其余转 `rest_todos` 给第④段（正文唯一性——hero 那条不在下方清单重复）；排产失败时 `_failed_run_applies_to_current_view` 独立闸门（不复用 summary_matches_identity，failed→is_current_executable_official_version 恒 False 会让分支不可达）配合 `resolve_result_status(...) == "failed"` 让失败指令顶格；全健康时 hero 转绿色平静态「当前没有必须马上处理的风险」。已删：stat-grid（正则锚先迁）、「当前查看排产」卡、「下一步」链接卡（侧栏做事路线承接）、死字段 recent_metrics/overdue_count/latest_summary 整链白算。`build_cockpit_hero` 全程整串解析时间（`_parse_dt` 无截断）、计数走 `isascii()&isdigit()` 拒 unicode 数字/bool、坏数据不伪装（计数算不出走「可用清单里逾期最久」诚实限定语而非挑首条冒充）。首页值班台只展示实时生成的待处理，不保存已处理状态，不直接写现场记录，不新增数据库表，不改排产算法；所有跨页动作继续使用 `WorkbenchLink`（含 context-free 'batches' 第 14 目标），页面正文只显示中文计划身份和中文业务文案，模拟方案跨页上下文用 `plan_context_token`，不把 `scenario_id` 放进公开链接或隐藏字段。现场事实读取失败时显示「现场情况暂时读不到」数据缺口提醒，不把读取失败误判成「现场情况待确认」；坏版本号/坏日期参数回最新视角且不回显原始串（`_requested_version`/`_valid_date_arg`），摘要超期数与明细不一致时拒绝展示准确数量（`_summary_overdue_count` 一致性闸）。首页顶部另有备份健康提示：`web/viewmodels/dashboard_backup_health.py` 纯只读扫描备份目录（不实例化 BackupManager，避开其建目录副作用）取 `aps_backup_*.db` 最大 mtime，超 `BACKUP_STALE_DAYS=7` 个日历日或从未备份时渲染琥珀 `ui.notice`，备份目录读取失败时明示「备份状态读取失败」而非静默装健康；健康时零渲染（fusion-backup-health-hint）。**「删常用工作区按钮墙 + 首页带上下文跳转」移交 #10 fusion-nav-specs-unify**（侧栏做事路线就位再删，避免导航真空）。
...
- 甘特任务详情区：`/scheduler/gantt/data` 由 `GanttService` 读取排程明细，并通过 `ExecutionFactProvider` 按 `op_id` 聚合现场执行事实；`core/services/scheduler/gantt_tasks.py` 输出公开任务标题、计划时间、现场实际小结、超期提示和资源/工序/图号字段；`web/viewmodels/scheduler_gantt_task_detail.py` 追加资源派工、计划和现场实际、超期清单链接；`static/js/gantt_render.js` 点击任务后刷新 `#ganttTaskDetail`，`static/js/gantt_popup.js` 继续维护旧弹窗和新详情区。详情区和旧弹窗只展示公开字段，关键链 edge 保留内部 `from/to` 做连线，同时用 `from_label/to_label` 给用户看，避免缺 `op_code` 时把 `op_<数字>` 露出来。现场事实可视化（fusion-gantt-execution-visuals，契约 4.10）：`gantt_tasks._execution_visuals` 以 `has_execution_record` 为门槛产出条形视觉——progress 只允许 completed→100（红线禁部分进度估算伪装精度），`custom_class` 按四态白名单 frozenset{processing/paused/exception/completed} 追加 `execution-<status>`（词表 not_started 与未知码零类，DOM 不收垃圾串；raw 状态码只在服务端消费一次，meta 仍只出中文公开标签）；无事实=计划行 payload 原样（preview/候选身份永远无事实自动满足）。视觉通道分离：fill 归四种配色模式（--aps-bar-color inline），完工=半透明绿罩接管 `.bar-progress`（普通/hover/active 三态选择器一组写齐，否则 frappe 默认紫会在悬停时闪回），processing/paused/exception=描边 accent 且选择器自带 `:not(.overdue)` 守卫（超期红边优先由选择器语义保证、不依赖书写顺序），暗色主题块内重申描边规则（dark 基础 .bar stroke 特异性更高会盖掉）；零新增裸 hex 全走 --ui-* token。侧栏详情补优先级/加工方式/时长三行、popup 补现场摘要一行（双向互缺收口），图例标记行含四执行状态样例。CSS 写法由 tests/gantt/test_gantt_execution_visuals_contract.py 正则锁选择器+token 值。沿链巡检（fusion-chain-walk-navigation）：详情面板「上一道/下一道」沿工艺依赖边走（gantt_chain_walk.js 索引建在 allTasks 原始 dependencies 上——currentTasks 的被 depsMode 重写）、←/→ 沿关键链 ids 正序（范围外 id 跳过明示、输入框聚焦跳过）；选中语义统一 selectTaskById（focusBatch 幂等赋值+「清除聚焦」按钮补窄入口），视图外目标只渲详情+提示不装饰。资源负荷条带（fusion-gantt-load-strip，契约 4.6 第二个落地实例）：契约 v3 `resource_load` 字段——`core/services/scheduler/gantt_resource_load.py` 资源×日桶聚合（容量分母 `capacity_hours_at_noon` 单源 helper、外协不计、ratio 算不出 None），severity/links 由 `web/viewmodels/scheduler_gantt_load_strip.py` 装饰（阈值 import dashboard_workbench_cards 唯一字源），前端 `gantt_load_strip.js` Top 5 降序条带与假期层同像素公式、点击格弹当天任务+去派工/报表链接；详见 ui-gantt.md。控件重排（fusion-gantt-controls-rework）：解码条 `#ganttLegend` 迁出折叠面板常显且批次 chips 点击即筛选、控件三层化（版本摘要单行+zoom ± 步进）、宽屏（≥1180px）点击任务条单一详情反应禁浮层；详见 ui-gantt.md。
- 现场事实公开标签单源（契约 4.10）：`core/services/scheduler/execution_fact_presentation.py` 的 `execution_detail_meta(fact)` 是「ExecutionFact→前端公开显示标签」的全站唯一实现（2026-06-13 从 `gantt_tasks._execution_detail_meta` 抽出去前缀公开），产出 `execution_status_label/actual_*_time_label/actual_summary_label/has_execution_record`（无记录诚实显示「暂未记录现场实际」，时间为 ISO `YYYY-MM-DD HH:MM:SS`，status 文案走 `operation_execution_labels.execution_status_label`）；甘特任务详情区与批次详情排程去向卡共消费，新消费方禁内联状态文案/重复实现该映射。
- 批次详情排程去向卡（fusion-batch-detail-schedule-card，契约 4.10/4.2，模块 W）：`/scheduler/batches/<batch_id>` 在既有「基础信息卡」与「可编辑工序卡」之间新增一张只读卡，回答「本批次排进哪个最新正式方案、几道工序、什么跨度、现场干到哪、怎么去甘特看」。取数在路由 `web/routes/domains/scheduler/scheduler_batch_detail.py::_resolve_schedule_placement`，整段（含 `get_latest_version` 与 `g.db`/`ExecutionFactProvider` 构造）包在 try/except 内：版本取 `get_latest_version()`（≤0→`no_official_plan`）、角色硬钉 `ROLE_ADOPTED`，`resolve_plan` 解析一次后 `list_plan_detail_rows_all_for_resolution`（全 kwarg、复用同一 resolution）取本批次行，本批次空时再查版本级 `get_plan_time_span_for_resolution` 区分 `plan_empty`/`not_placed`，现场事实走 4.10 唯一入口 `facts_by_op_id_for_plan_rows(rows, plan_fields)`（不传 include_op_ids，缺事实工序自然走「暂未记录现场实际」）；任何异常 `current_app.logger.exception` 记栈+`error` 态、只降级本卡不让整页 500。计划设备/人员走单源 `display_machine/display_operator`、工序名走 `gantt_task_labels.public_task_label`、现场列走 `execution_detail_meta`、span 日期由路由 `parse_dt` 解析跳坏值各自独立求 min/max。纯视图模型 `web/viewmodels/scheduler_batch_schedule_placement.py` 装配摘要 *_label（经 `build_workbench_plan_context`）+ op_rows（仅 8 公开 label 键）+「在甘特中定位本批次」`build_workbench_link`（gantt 目标带 version/批次跨度日期窗口），禁 import core.services、不碰 rows/ExecutionFact。诚实五态 ok/no_official_plan/plan_empty/not_placed/error；卡内零内部计划身份（op_id/schedule_id/scenario_id/source_table/candidate_id），span 日期算不出时定位按钮不渲染。门禁 `tests/web_pages/test_scheduler_batch_schedule_placement.py`、`tests/schedule/route_view/test_scheduler_batch_detail_route_contract.py`（含 ok 态契约）、`tests/schedule/route_view/test_batch_detail_linkage.py`（既有可编辑卡锚点靠 `{% if schedule_placement %}` 守卫不破）。
...
- 报表工作台回跳：`/reports/`、超期、资源负荷、计划和现场实际、停机影响由 `web/viewmodels/scheduler_reports_workbench.py` 统一装配入口卡、页面级链接、行级动作和保守空状态，并继续复用 `WorkbenchLink` 保留版本、方案、计划身份、返回地址、日期、批次和资源上下文；排产主导航、报表顶部导航、报表筛选隐藏字段和全局计划工作台导航由 `web/viewmodels/scheduler_navigation_links.py` 统一从当前请求生成，模板不再手拼 URL，带上下文进入“首页值班台”也不会丢 plan_id 或 back_to。`ReportEngine`、`execution_review`、`schedule_plan_query_service` 和 `schedule_plan_query_repo` 承接底层过滤，页面和 Excel 导出共用同一批 `batch_id`、`resource_type`、`resource_id` 条件，并把资源负荷、停机影响和计划实际复盘的批次/资源条件下推到计划明细 SQL，避免只把 URL 做好看而数据没过滤。用户显式填写的资源负荷、停机影响、计划实际复盘日期范围最多 62 天，复用资源派工口径；按版本自动回填出来的跨度不套这个“手填日期”限制。当前 `resource_type/resource_id` 主资源筛选只支持设备和人员，班组上下文进入首页值班台、排产分析、周计划和报表类目标时会禁用链接，不生成会跳 400 的 URL；资源派工继续使用 `scope_type/scope_id/team_id` 保留班组上下文；甘特图不携带班组筛选。排产分析、甘特和周计划通过 `scheduler_navigation_publish.py` 发布导航上下文时，会复制服务端方案身份护栏字段，并把报表行跳转带来的批次和资源范围继续发布给顶部导航，避免旧正式版本、对比方案、模拟预览误启用计划和现场实际入口，也避免用户从具体批次/资源跳转后再导航回全量范围。资源派工现场实际写入地址由 `scheduler_resource_dispatch_query.py` 按服务端归一化 filters 生成，前端不再用裸路径叠加地址栏原始查询串来补 `query_date` 等关键上下文。报表展示、执行复盘、延期诊断遇到坏数字时抛 `ValidationError`，导出行数和阈值额外拒绝负数和小数，不静默按 0 处理；计划和现场实际仍只复盘正式采用方案；停机影响第一版只做设备级说明和设备级回跳，不做任务级明细；内部追踪字段只留在请求解析和服务端内部，不进入页面正文、普通 HTML 属性、导出表头或公开 payload。排产历史页行级 5 链接与排产分析版本选择器两条甘特链也已收编入 `WorkbenchLink`（fusion-handrolled-links-adoption）：历史行由路由层逐版本读 adopted 计划日期跨度（先分页后装配、同版本缓存去重、单行坏历史只禁用该行并明示原因），`web/viewmodels/system_history_links.py` 纯数据变换装配；分析选择器用 `resolve_navigation_plan_context` 全量方案身份装配，场景预览跳甘特用 `plan_context_token` 保持模拟方案，不再掉回正式视角，裸模拟预览禁用明示。`TARGET_PAGE_PATHS` 扩至 13 目标：`history`（version 可选筛选）与 `batch_detail`（路径参数型目标，`{batch_id}` 占位 quote 替换、缺参 fail-loud）；两目标的 query 合同（version 可选+back_to / 仅 back_to）由 extra_params 禁键集守护，不可被调用方绕过。
- 报表过滤与诚实展示：超期清单和延期诊断按排产版本复盘延期风险，不再从页面、隐藏字段或导出 URL 传播 `date_from/date_to/query_date`；`resource_type/resource_id/batch_id` 仍会传到 `ScheduleDelayDiagnosisService` 和计划查询 repo，避免整版诊断后再丢弃。资源负荷、停机影响、派工和甘特明细查询遇到坏时间行时不再在 SQL 层提前丢掉，会把坏行带到服务层记录 degradation 计数和样例；超期清单对“有排程但计划完成时间写法不对”的批次单列“排程时间异常”，不误报成“未排程逾期”，页面和 Excel 摘要都会说明数据不完整。批次详情的排程时间跨度也会区分“已排工序全量数量”和“可解析时间记录算出的跨度”，坏时间数量通过页面提示说明。
...
- 固定名运行文件安全：启动契约文件（`aps_port.txt`、`aps_host.txt`、`aps_db_path.txt`、`aps_runtime.json`、`aps_launch_error.txt`、`aps_runtime.lock`、`launcher.log`）以及备份/清理中的固定名文件读写统一走 `core/infrastructure/safe_files.py`。该 helper 先用 `lstat` 拒绝软链接、硬链接和非普通文件，读取时再打开文件描述符复核普通文件，写入和删除时默认拒绝软链接，不会先删链接再伪装成正常写入；清理备份时只处理普通备份文件；数据库恢复会先校验当前数据库目标也是普通单链接文件，启动契约清理只清理当前状态目录和带匹配契约的镜像日志目录；启动失败路径仍通过 stderr/logger/launcher.log 可观测，写日志失败也不能静默写穿软链接。
...
- 页面正文、普通 HTML 属性、导出、文件名、提示语、帮助文档和公开 JSON 不能直接展示 `scenario_id`、`source_table`、`candidate_id` 这类程序内部字段；服务端内部可以继续用这些字段对齐同一套计划。模拟方案跨页传播不再把 `scenario_id` 放进公开链接或隐藏字段，必须用 `plan_context_token`；`plan_role`/`version` 等既有路由参数仍按各页面契约使用。

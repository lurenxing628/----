...
- 首页计划员值班台：经典首页 `templates/dashboard.html` 和现代镜像 `web_new_test/templates/dashboard.html` 由 `web/routes/dashboard.py` 读取最新排产、正式采用方案范围、今日计划任务和现场事实，再交给 `web/viewmodels/dashboard_workbench.py` 与 `web/viewmodels/dashboard_workbench_cards.py` 生成风险卡、今日待处理和快捷入口。首页值班台只展示实时生成的待处理，不保存已处理状态，不直接写现场记录，不新增数据库表，不改排产算法；所有跨页动作继续使用 `WorkbenchLink`，页面只显示中文计划身份和中文业务文案，内部字段只留在 URL、隐藏参数或服务端日志里。

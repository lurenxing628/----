...
10. **fusion-nav-specs-unify** — 导航 specs 化：侧栏五段「做事路线」分组 + module_subnav 统一五个手写宏 + aria-current 全站 + 焦点环上岗。**承接 #19 移交（2026-06-13）**：「删 dashboard 常用工作区按钮墙 + 首页带上下文跳转」并入本条——侧栏「做事路线」就位后才删按钮墙（避免导航真空），test_manual_entry_scope/test_context_propagation 在此条统一裁决。依赖：2、9（胶囊与侧栏同壳层，先胶囊后重排）。
...
13. **fusion-chain-walk-navigation** ✅ done（2026-06-13，feature 2026-06-13-fusion-chain-walk-navigation，Codex 设计两轮（5 阻塞：范围外目标/生命周期两分/script 顺序/滚动导出/清聚焦入口）+实现两轮（1 阻塞：视图外目标误装饰）审核收口，复审 CLEAN；真浏览器 CDP 实测）— 沿链巡检：详情面板上一道/下一道（前端 dependencies 反查建 Map，零后端改动）+ 关键链 ←/→（normalizeCriticalChain ids 正序）。依赖：12（软依赖：同在详情面板区施工避免冲突，非产物依赖）。
14. **fusion-gantt-fix-pack** ✅ done（2026-06-13，feature 2026-06-13-fusion-gantt-fix-pack，Codex 设计四轮（首轮 2 阻塞+二轮 2 残留+三轮 frontmatter 残留，四轮 CLEAN；prototype.call 零实例化方案为复审建议采纳）+实现三轮（首轮 1 建议文档反向守卫不全、二轮 BLOCK 补「不能点击/灰色入口」短语，三轮 CLEAN）审核收口；浏览器 CDP 实测+node 直跑 vendor 9 级全配）— 甘特小修包（纯 bugfix/守卫，跑测试即可验收）：select 精确匹配（修 B1 带出 B12）+ 暗色 cc-outline 补丁 + 删"模拟调整"死按钮 + ZOOM_SPECS 与 min.js 双份真相 boot 断言。依赖：12（软依赖：同在甘特 JS 施工避免冲突，非产物依赖）。（UX 重排部分已按拍板拆为第 27 条。）
...
19. **fusion-dashboard-cockpit** ✅ done（2026-06-13，feature 2026-06-13-fusion-dashboard-cockpit，实现期 9 轮 Codex + 3 轮 UltraCode 对抗硬化、双引擎在最终状态一致 0 blocker）— 首页驾驶舱（构图 2026-06-11 定稿，样张 drafts/dashboard-restyle-proto.html）：单栏四段 = 上下文胶囊 → hero 指令卡（待办队列第 1 条置顶展示，下方明细清单**不重复**它；排产失败时 _failed_run_todo 顶格，result_status 已在手）→ 6 格体检表（超期/待排/方案待确认/现场待确认/资源压力/基础数据，每格即入口可点跳，正常项灰显——替代旧 stat-grid，"待排数"承接被裁的"最近一次排产"卡唯一增量信息）→ 其余待处理清单（3px 左语义条，不嵌灰盒）。删 stat-grid（先迁正则锚）+ 删按钮墙 + 删"下一步"链接卡（侧栏做事路线承接）+ 风险卡 severity 按 result_status 分级 + 死字段裁决（overdue_count/latest_summary 零消费且每请求白算一遍）+ 超期 todo 直挂主导线索摘要（性能门槛：仅 overdue_count 小时启用）+ 全健康空态定义（hero 转绿色平静卡"当前没有必须马上处理的风险"）。依赖：2、3（爆点清单先行）。**验收 Path C 回写（2026-06-13）**：本 feature 完成 ②③④ 重组 + 删 stat-grid/当前查看排产卡/下一步面板 + 正文唯一性 + 死字段清理（含 scheduled_count 整链）+ 文档同步 + 新建 dashboard_cockpit_hero.py + context-free 'batches' 目标；**「删常用工作区按钮墙 + 首页带上下文跳转承接」移交 #10 fusion-nav-specs-unify**（侧栏做事路线就位再删），test_manual_entry_scope/context_propagation 本 feature 不动。
...
30. **fusion-due-soon-alert** ✅ done（2026-06-15，feature 2026-06-14-fusion-due-soon-alert）—（模块 N）临期预警："3 天内到期且未完工"新待办类别 + 体检表格位 + 临期窗口常量单点定义；数据 = 既有批次交期 vs 正式方案排程结束时间，与超期判定共用同一口径（4.11），零新数据链路。依赖：19（驾驶舱构图先落定，类别与格位才有挂点）。**验收回写（2026-06-15）**：两项用户裁决落地——①格位=**独立第 7 格**「临期批次」（居末、复用 warning/notice/ok 不新增 severity 档）；②口径=**finish-基准**（临期 = 排程完工 finish ∈ [due_exclusive−3天, due_exclusive)，与超期 finish≥due_exclusive 严格互斥零重叠双算，now 无关可冻结）。summary 新增 `near_due_batches{count,items,window_days}` 键与 overdue 并列同源切分（同一全批次循环两段切分、零新遍历、schema 仍 1.2）；`NEAR_DUE_WINDOW_DAYS=3` 装配侧单点；缺键诚实降级（第 7 格"数据不足"、不放大成全摘要降级）。微重构（只搬不改行为）：assembly 拆 `due_risk_items.py`、workbench 拆 `dashboard_workbench_todos.py` + 中立 `dashboard_workbench_shared.py`（单向 import 无环）。

...
6. 设计确认后实现包 A，并跑 `tests/regression_scheduler_workbench_links_contract.py`。
7. 包 A 验收后再做包 B；包 B 验收后再做包 C。
...
建议按“阶段 0 + 13 个实施阶段”推进。阶段 0 是本文档前置，已经在本轮文档修正中完成，不进入 `items.yaml`。表里的“第一轮”才是马上开工范围，“第一版”是后续第一批可上线范围，“第二阶段”是增强项。
...
| 12 | 停机影响任务级明细 | 把停机影响从设备级说明补到受影响任务 | 阶段 1、7 | 第二阶段 |
| 13 | 牵连批次和订单影响面 | 延期和方案解释能说明可能牵连哪些批次或订单 | 阶段 1、4、10 | 第二阶段 |
...
内部值到中文展示也要在 design 里写成表，不能让每个模板自己翻译：

| 内部值 | 中文展示值 | 说明 |
|---|---|---|
| `plan_role=adopted` | 正式采用方案 | 只有它能进入现场事实写入 |
| `plan_role=baseline_best` | 原算法代表方案 | 只能看，不能写现场事实 |
| `plan_role=critical_best` | 重点工序优先代表方案 | 只能看，不能写现场事实 |
| 有 `scenario_id` 但没有可读名称 | 模拟预览（未命名） | 不把 `scenario_id` 显示给用户 |
| `guardrail_reason_type=plan_not_writable` | 当前方案不可写 | 常见于候选/模拟/历史方案 |
| `guardrail_reason_type=task_state_blocked` | 当前任务状态不可写 | 任务状态不允许开工/完工 |
| `guardrail_reason_type=action_unavailable` | 后端暂未开放这个动作 | 前端不能自己补写入地址 |
| `guardrail_reason_type=data_gap` | 数据不足，暂时不能判断 | 要说缺什么，不能只说失败 |
| `resource_type=operator` / `scope_type=operator` | 人员视角 | 资源派工和资源负荷都用这个中文 |
| `resource_type=machine` / `scope_type=machine` | 设备视角 | 资源派工和甘特都用这个中文 |
| `resource_type=team` / `scope_type=team` | 班组视角 | 第一版能带参就带，页面不足时降级总览 |
| `period_preset=week` | 按周 | 和 `query_date` 一起保留 |
| `period_preset=month` | 按月 | 和 `query_date` 一起保留 |
| `period_preset=custom` | 自定义 | 必须同时保留日期范围 |
| `view=machine` | 设备甘特 | 甘特 URL 仍用 `view=machine` |
| `view=operator` | 人员甘特 | 甘特 URL 仍用 `view=operator` |

`required_params` 落地规则：

- feature design 必须以页面设计稿第 3.4 节的跨页链接参数矩阵作为逐路线基线；下面这张表是开工摘要，如果设计稿矩阵比这里更细，按更细的写。
- 只要当前上下文有 `plan_role` 和 `scenario_id`，跨到首页、分析、甘特、资源派工、超期清单、资源负荷报表和报表中心时都要带上；`execution_review` 是例外，它只看正式采用方案，不带候选方案身份。
- 跳资源派工或报表类页面时，能拿到就同时带 `date_from/date_to`、`query_date`、`period_preset`；跳资源派工时还要带 `scope_type`，不要只带一个 `resource_id`。
- `date_from/date_to` 和 `start_date/end_date` 是同一个业务意思，ViewModel 接收统一上下文，按目标页真实路由翻译。

关键跳转清单必须写进测试：

| 来源 | 目标 | 必带参数 | 禁用规则 |
|---|---|---|---|
| 首页超期待办 | 超期清单 / 延期解释 | `version`、`plan_role`、`scenario_id`、`date_from`、`date_to`、`batch_id` 如果有 | 无版本时禁用 |
| 首页方案待确认 | 排产分析 | `version`、`plan_role`、`scenario_id` 如果有 | 无候选方案时不生成 |
| 首页资源高负荷 | 资源派工，备用资源负荷报表 | `version`、`plan_role`、`scenario_id`、`date_from`、`date_to`、`query_date`、`period_preset`、`scope_type` 如果有、`resource_id` 如果有 | 资源对象缺失时跳总览 |
| 首页现场情况待确认 | 资源派工现场事实区 | 正式采用 `version`、`plan_role=adopted`、`date_from`、`date_to`、`query_date`、`period_preset`、`batch_id` 如果有、`scope_type/resource_id` 如果有 | 非正式方案不生成写入入口 |
| 首页风险汇总 / 常用工作区 | 报表中心 | `version`、`plan_role`、`scenario_id`、`date_from`、`date_to`、`query_date`、`period_preset` | 无版本时禁用 |
| 分析推荐卡 | 甘特 | `version`、`plan_role`、`scenario_id`、`start_date`、`end_date`、`view` | 无可查看排程时禁用 |
| 分析诊断卡 | 超期清单 | `version`、`plan_role`、`scenario_id`、`batch_id` 如果有 | 无诊断数据时禁用 |
| 甘特任务详情 | 资源派工 | `version`、`plan_role`、`scenario_id`、`date_from`、`date_to`、`query_date`、`period_preset`、`scope_type`、`resource_id`、`batch_id` 如果有 | 任务没有资源对象时显示原因 |
| 甘特任务详情 | 计划和现场实际 | `version`、`date_from`、`date_to`、`batch_id` | 非正式方案下入口禁用 |
| 资源派工任务明细 | 报表中心 | `version`、`plan_role`、`scenario_id`、`date_from`、`date_to`、`query_date`、`period_preset`、`scope_type`、`resource_id` 如果有 | 没有查询结果时显示空状态 |
| 资源派工现场记录 | 计划和现场实际 | `version`、`date_from`、`date_to`、`query_date`、`period_preset`、`batch_id`、`resource_id` 如果有 | 非正式方案下入口禁用 |
| 报表中心资源负荷 | 资源派工 | `version`、`plan_role`、`scenario_id`、`date_from`、`date_to`、`query_date`、`period_preset`、`scope_type`、`resource_id` | 无资源行时显示原因 |
| 报表中心超期清单 | 甘特图 | `version`、`plan_role`、`scenario_id`、`start_date`、`end_date`、`batch_id`、`view` | 无甘特数据时显示原因 |
| 计划和现场实际 | 资源派工 | `version`、`date_from`、`date_to`、`batch_id`、`scope_type`、`resource_id` 如果有 | 无现场记录时显示空状态 |
...
- 任务卡能显示图号或物料信息；拿不到就显示中文缺口，不显示内部字段。
- 任务卡能显示计划开始/结束、实际开始/结束和偏差；没有实际记录时显示“暂未记录现场实际”。
- 执行流水能说明来源和记录时间；如果后端暂时没有记录时间，就在 design 里把缺口写成后续接线项，不在页面硬编时间。
...
第一轮 A/B/C 的小烟测已经随第 7 节完成；本阶段做的是第一版完整主流程测试，依赖阶段 1-7，不依赖第二阶段的延期解释接现场事实、甘特资源负荷摘要、停机影响任务级明细和牵连批次/订单影响面。
...
- 如果有实际开始/结束，用它解释偏差。
- 如果没有，才显示“暂未记录现场实际”。
- 本阶段只做现场事实接线；牵连批次/订单精确影响面由阶段 13 承接。如果暂时不能算，就明确说“当前不能判断牵连批次/订单”。
...
## 16. 阶段 12（第二阶段）：停机影响任务级明细

这一节只属于第二阶段，不属于第一轮，也不属于第一版验收。第一版停机影响只做设备级说明、数据缺口和回跳，不假装能列出全部受影响任务。

### 16.1 目标

计划员看停机影响时，不只知道“某台设备有停机”，还要尽量知道哪些排程任务可能受影响。

### 16.2 要做什么

- 把停机记录按设备和时间窗口关联到排程任务。
- 能匹配到任务时，显示受影响任务、批次、资源、计划时间和影响说明。
- 不能精确匹配时，显示“当前只能看到设备级停机影响，暂时不能判断具体任务”，不能硬编任务。
- 明细行要能用 `WorkbenchLink` 回甘特或资源派工。

这阶段已在 items.yaml 里新增 roadmap item：`downtime-task-impact-detail`。

## 17. 阶段 13（第二阶段）：牵连批次和订单影响面

这一节只属于第二阶段，不属于第一轮，也不属于第一版验收。第一版只要保守说明“当前不能判断牵连批次/订单”，不能为了看起来完整而编影响面。

### 17.1 目标

延期解释和方案对比不只告诉计划员“这个任务晚了”，还要在数据足够时说明可能牵连哪些后续批次或订单。

### 17.2 要做什么

- 基于批次、工序顺序、资源占用和订单交期，计算可解释的牵连范围。
- 只展示能被当前数据证明的批次、订单、交期和影响说明。
- 数据不够时，明确写“当前不能判断牵连批次/订单”，不要显示 0 或空白误导用户。
- 牵连明细行要能回甘特、超期清单或排产分析。

这阶段已在 items.yaml 里新增 roadmap item：`downstream-batch-order-impact`。

## 18. 每个 feature 开工前都要检查
...
- 如果涉及写入，是否只对正式采用方案开放。
- 如果没有数据，页面怎么说。
- 页面、导出列和公开 payload 是否都没有暴露内部字段。
- 每个新增区域的空状态、加载失败、数据不足文案是否都写清楚。
...
- 第 5.2 节字段表、目标页字典、内部值到中文映射、逐路线 `required_params` 矩阵。
...
- 首页、分析、甘特、资源派工、报表之间跳转不丢 `version`、方案、日期范围、查询日期、区间类型和资源视角。
...
- 第一版不要求停机影响任务级明细；这是第二阶段增强。做完第二阶段后，停机影响才需要尽量列出受影响任务。
- 第一版不要求牵连批次和订单影响面；这是第二阶段增强。做完第二阶段后，数据足够时才展示牵连范围，数据不足时必须明说。
- 非正式方案不能写现场实际。
- 普通用户看不到内部字段。
- 关键页面有回归测试和浏览器几何覆盖。
- 用户指南和真实页面一致。

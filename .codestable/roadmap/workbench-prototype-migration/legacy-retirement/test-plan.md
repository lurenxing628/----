...
- 本计划不是完整执行结果。V4 私有源码真实应用 patch 后，已通过46项正常pytest、两种循环扫描、两侧12组导入、51页与39非页面GET/HEAD、113空表单POST、报表/打印/手册和三条真实消息链。完整源绑定与限制见 `factory-acceptance.md`、`factory-verification.json`。浏览器 B/K/V/P、candidate 全站、所有交易/mode组合及回退演练仍未通过本轮替代。
- Main 已报告正式容量窗口通过：5000×4=20000 rows，admission_to_terminal=122.728285375s，runtime=121.342296s，1472 inputs 前后 hash 一致，重启 refs/payload/业务保留。G没有重跑或独立替代该证据；Main正式应用/挂载仍须17授权，本轮仅在已批准的私有候选应用验证。
...
- `candidate-review-index.json` 对 14 个 Python renderer 文件验证了“恢复模板字符串后 AST 完全相等”；这只证明 patch 不改其他解析/事务/参数代码。后续 V4 已真实应用并跑 factory，旧样稿不能取代其源绑定。
...
- V4实际专项为 `test_final_navigation_boot.py`、`test_final_legacy_navigation.py`、`test_final_legacy_navigation_layering.py`、`test_final_legacy_report_dates.py` 及两份support。Main当前已将四项列入supplemental组，不等于必跑门禁或运行回执；G未改共享registry。后续导航overlay单独记证据，不回填V4通过。

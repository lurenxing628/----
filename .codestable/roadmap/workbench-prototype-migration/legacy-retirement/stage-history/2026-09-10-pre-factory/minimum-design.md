...
- 状态：退役仍未挂载或改交付。后续明确授权的规范 nav helper、旧 GET SELECT 转换模块及专项测试已实际实施；dispatcher/结果/打印/手册/错误模板均已完成可应用 patch，未获17通过与挂载许可，不宣布17或18完成。
...
实际文件/待应用文件区分及SHA见handoff。G不向共享schema/DTO、build-order或现有入口写入；已经使用私有CHECKUP_CALLGRAPH定位parser与影响面。模板和dispatcher仅在draft-payload，产品导航辅助代码已经存在但旧适配器未挂载。

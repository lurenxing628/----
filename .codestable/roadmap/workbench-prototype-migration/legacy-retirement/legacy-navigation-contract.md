...
- Main未挂载源码：`web/routes/workbench/legacy_navigation.py`、`legacy_navigation_plan.py`、`legacy_page_contract.py`。调用方提供既有connection；查询下沉到 `core/services/workbench/legacy_navigation_queries.py`，web不直接SQL/Repository、不补建身份。私有V4已真实安装并验证，见 `factory-acceptance.md`。
...
- 旧报表显式日期校验共用 `core/services/report/date_input.py`；原web公开函数兼容导出，去除workbench到旧report模块的目录回边。日期算法/默认值/文案/62天上限不变，两种正常scanner基线不变且exit0。
- 合法人员日历详情先核实人员，再明确 `operator_calendar_scope_retired`；缺人员404，不改选其他日历。
...
4. Main当前registry已列nav、legacy、layering、report_dates四项专项；它们位于supplemental组，仍由Main选择最终门禁并绑定完整source/support。私有factory harness提供真实调用范例，不进入生产交付。

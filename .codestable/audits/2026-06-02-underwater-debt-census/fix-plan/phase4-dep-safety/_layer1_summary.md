...
R45 | 真实行号: CriticalConfigReadResult→config_adapter.py:10, read_schedule_config_value→:16, read_critical_schedule_config→:26 | 漂移: 三符号无漂移(老:10-27=实测27行);但registry的"schedule_params.py:98在core/models/"失准,真实路径在core/algorithms/greedy/ | 修法类: 直删整文件+同提交退sp06:15路径(非收口/非parity;吞异常壳:22-23随删消除,无需改raise) | 同文件兄弟需协调: R48=同一物理文件另一叙述,必须合并单提交整删,无先后无撞行号 | 最大爆炸风险: 仅"删文件忘退sp06:15"→read_text抛FileNotFoundError令sp06测试红(响亮非静默);本体收口点+10余活跃消费方不动,无静默炸点 | 与既有分析冲突: 有(registry误把schedule_params.py标core/models/且与R45 same_file;实为core/algorithms/greedy/同目录他文件,LB07/R33/R51对R45零碰撞) | 前置: 无硬前置,最早可落;须与R48合并+同提交退sp06 | owner_pending: n
- VERIFY: R45 | 核验: PASS | 行号复核: 一致(三符号 :10/:16/:26、吞异常 :22-23、import :6、文件 27 行、sp06 路径 :15 全对齐零漂移) | 争议点: 无(registry 把 schedule_params.py 误标 core/models/，档案 §1 已自行抓到并不影响删壳结论) | 承重误删风险: 无(load_bearing=false，修法纯直删+退 sp06:15 同提交，未混入统一/透传/加形参/兜底；吞异常随整文件删除消失) | 需升级第三方裁: n
...
### R28
- DOSSIER: R28 | 真实行号: _safe_float→batch_service.py:55-64(@staticmethod在:55,def在:56);消费点 batch_template_ops.py:172、batch_copy.py:72;收口点 parse_finite_float→core/shared/number_utils.py:23-26(已存在,scheduler/number_utils.py:6 已re-export);fitness白名单 test_architecture_fitness.py:77 | 漂移: 无(全部与evidence一致,≈0) | 修法类: 收口到parse_finite_float(allow_none=True),改raise方向(P4灵魂线)+同步处理fitness:77条目;非承重可实改;推荐方案b函数体改`return parse_finite_float(value,field='ext_days',allow_none=True)` | 同文件兄弟需协调: 无(same_file_siblings=[],batch_service孤儿债);interference R04/R29/R33均住number_utils.py、same_symbol=false、无代码耦合 | 最大爆炸风险: allow_none用错=False会炸所有空ext_days正常批次;越界统一setup/unit_hours的`or 0.0`兜底会静默炸建批次主流程;上游模型层已parse_optional_float校验故实际爆炸半径极小 | 与既有分析冲突: 无 | 前置: 仅B05的F门,无硬债前置,完全独立叶子可任意排期 | owner_pending: n
- VERIFY: R28 | 核验: PASS | 行号复核: 一致(_safe_float @staticmethod:55/def:56~64、消费点 batch_template_ops:172、batch_copy:72、收口点 number_utils.py:23-26、fitness :64/:77 全准；registry old_location :56-65 偏1行已被档案正确标注) | 争议点: 无(字段11的fitness探测器看名/看体待定是诚实开放项,要求动手前跑-k allowlist据实定夺,处置正确) | 承重误删风险: 无(lb=false、未对承重债下删/统一/透传;灵魂线走loud raise无新增兜底) | 需升级第三方裁: n
...
### R11
- DOSSIER: R11 | 真实行号: 第一份 _normalize=gantt_service_support.py:32-51; 第二份(staticmethod)=gantt_critical_chain_provider.py:113-137; provider自调:180; service分叉:384(support)/:386(provider); 白名单 gantt_contract.py:19/89 | 漂移: 有 — support:32-51准；provider自调 registry写171→实际180(+9)；service registry写375/377→实际384/386；blast区间104-128含邻函数应为113-137 | 修法类: 收口到X(抽单份到已存在 gantt_critical_chain.py，禁return raw)，owner_pending待裁暂不给终态 | 同文件兄弟需协调: R63≡R11同一动作必须合并单work item(否则撞改两文件)；R55(加scope)/R12(加dropped_count)必须晚于本债否则复活P5 | 最大爆炸风险: 误把 critical_chain_for_plan_detail_filter 改 return raw → support路径丢available/edge_type_stats默认，降级事件静默漏报+前端契约两路分叉(快照测试:122 subset检查抓不到) | 与既有分析冲突: 无 | 前置: R11≡R63合并先于R12+R55；先建 normalize parity 测试(钉死 available 的 bool()差异) | owner_pending: y
...
### R63
- DOSSIER: R63 | 真实行号: support `_normalize`→gantt_service_support.py:32-51(调用点:58)、provider `_normalize`→gantt_critical_chain_provider.py:113-137(调用点:180) | 漂移: 有,provider 整体 +9 行(old_location 还误把相邻 `_copy_critical_chain_result` 首行并入);support 尾行 +1 | 修法类: 收口到 X(已存在落点 gantt_critical_chain.py,support:7/provider:19 均已 import;抽单份 helper,两路改调它,逐字保留 7 项归一不变量;≡R11 同一动作合并) | 同文件兄弟需协调: R11=同一物理动作须合并;R55(scope,:71-75)与 R12(dropped_count)必须晚于本条,否则两副本各改=复活债;跨桶 R10 同 gantt_service.py 须串行勿并行 | 最大爆炸风险: 只改一路→filtered/unfiltered 两 payload 字段集静默分叉(被 gantt_contract.py:28-36 二次归一吸收难察觉),及误删相邻 `_copy_critical_chain_result` | 与既有分析冲突: 无 | 前置: 先补 parity 黄金基线测试作收口安全门;本条无债级前置(自身是 R12/R55 硬前置) | owner_pending: n
...
### R34
- DOSSIER: R34 | 真实行号: get_version_time_span→schedule_repo.py:36-59 / list_overlapping_with_details→:114-126 / list_dispatch_rows_with_resource_context→:128-158 | 漂移: 无(三 def 行号与盘上逐行一致) | 修法类: 直删+退测试(非收口;承重false,灵魂线不触) — 删3死方法+退facade断言:31/36/39-41+删无用import:12/:14+退detail_queries 10用例+删gantt_critical:59无效monkeypatch+repoint benchmark:503 | 同文件兄弟需协调: R35(list_between:61-69,夹在中间,全仓零引用)必须同提交、自下而上删,避免删后行号二次漂移撞facade断言 | 最大爆炸风险: 生产零引用,所有遗漏皆响亮AttributeError无静默炸点;唯一真消费者benchmark:503真读span["start_time"],漏repoint则基准脚本崩 | 与既有分析冲突: 有(registry planned_deps_hint旧措辞称R34"收敛到column_name/活孪生",实为纯删死方法,与R05改的是不同符号不同文件→R05非硬依赖仅软约束;fix_hint写的repoint目标get_plan_time_span_for_resolution在当前代码不存在,实际为get_plan_time_span:187) | 前置: R35硬同批;R05软在前(仅当detail_queries选迁移路径时变硬);F门=全测试+benchmark可跑+AST 0违规 | owner_pending: y(待裁:detail_queries整删vs迁移、benchmark repoint层级)

档案: /Users/lurenxing/Documents/GitHub/----/docs/_panorama_data/phase4_dep_safety/dossiers/R34.md
- VERIFY: R34 | 核验: PASS | 行号复核: 一致(36/61/114/128逐行命中,零漂移) | 争议点: 档案多处(§7/§8/§10/待裁点)断言收口符号get_plan_time_span_for_resolution"当前代码不存在"系误判——它实存于schedule_plan_query_service.py:210且:187内部就调它,fix_hint符号名其实正确;另registry顶层adv_refuted=false与verdict.refuted=true/正文打架(registry自矛盾非档案错)。生产零引用/SQL逐字符等价/承重false 全证实 | 承重误删风险: 无 | 需升级第三方裁: n(owner_pending待裁点性质不变)

...
### R59
- DOSSIER: R59 | 状态: 2026-06-08 G20 已 fixed | 终态行号: `parse_report_nonnegative_int`→report_number_parsing.py:51-64;旧 `_INT_TEXT_PATTERN` / `_parse_plain_report_int` / `import re` 残留应为 0 | 修法类: P5收口到已存在点 parse_required_int(min_value=0,reject_integer_float=True)+保留blank短路+删私有正则 | 同文件兄弟需协调: 无(siblings=[]);已守住禁区 parse_report_int(:33,'2000.0'仍是别的field语义) 与 `__all__` | 最大爆炸风险已处理: GF1 已先落, R59 显式传 reject_integer_float=True;测试已钉 '1.0'/1.0/2.000 raise 与 blank→blank_default | 与既有分析冲突: 无 | 前置: GF1 已满足;Batch-B/G20 已执行 | owner_pending: n
- VERIFY: R59 | 核验: PASS | 行号复核: 当前主文件已删除旧私有正则;`parse_report_nonnegative_int` 在 :51-64 委派 strict_parse;测试消费者在 `tests/web_pages/test_web_silent_fallback_contract.py` | 争议点: 无(保留 blank 短路、拒整值 float、友好错误文案、兄弟 parse_report_int 语义均有守卫) | 承重误删风险: 无(非承重,未越界改strict_parse/未动parse_report_int/未动__all__) | 需升级第三方裁: n
...
LB08 | 状态: fixed(2026-06-08) | 真实行号: LEGACY_PUBLIC_PATTERNS→core/models/scheduler_public_errors.py:63, _LEGACY_CODE_PREFIXES:95, make_public_error:171, legacy_public_error_message:214, infer_legacy_public_code:280 | 执行: 已在 LEGACY_PUBLIC_PATTERNS 上方补承重注释,按 O35 指向 internal_operation/resource_validation 中文模板,并绑定既有 regression_scheduler_user_visible_messages 契约;零删/统一/透传 | 同文件兄弟: R46 已在 LB08 护栏存在后删除 _safe_identifier 死别名;两者禁区零重叠 | 最大爆炸风险仍为禁区: 删错正则桥→legacy中文串 fullmatch 失配→静默降级通用文案+code归类丢失(P4灵魂线红) | 与既有分析冲突已消解: planned 草稿误指 auto_assign 消费方,实际注释已按真实产出点改写 | 前置: 已消费,LB08 先落后 R46 删 | owner_pending: n(O35 已裁;历史曾为 y)
- VERIFY: LB08 | 执行后核验: PASS | 现盘行号复核: LEGACY_PUBLIC_PATTERNS:63/_LEGACY_CODE_PREFIXES:95/make_public_error:171/legacy_public_error_message:214/infer_legacy_public_code:280；护栏注释在 :62 | 争议点: O35 已裁,注释指向 greedy/internal_operation + dispatch/resource_validation 真实产出点,未贴 auto_assign 消费方旧草稿 | 承重误删风险: 无(仅补注释+绑现有契约,零删/统一/透传,与R46删除段零重叠) | 需升级第三方裁: n
...
R46 | 状态: fixed(2026-06-08) | 执行: 已直删 scheduler_public_errors.py 中误导注释 + def _safe_identifier + return public_safe_identifier 三行死别名 | 修法类: 直删(非收口/非parity,承重=false) | 同文件兄弟: LB08 已先落护栏;R09 后续动 _positive_int 时按新行号/符号定位,且 clamp-to-0 语义勿误并 | 最大爆炸风险已避开: v4_sanitizers.py:37 _safe_identifier 活函数保留;public_safe_identifier/LEGACY正则桥/_positive_int/make_public_error/__all__ 未动;R46自身爆炸半径=0 | 与既有分析冲突: 无 | 前置: LB08 已满足;无F门;先于R09 | owner_pending: n
- VERIFY: R46 | 执行后核验: PASS | 残留复核: models 层 `_safe_identifier` 无命中；v4_sanitizers.py:37 同名活函数及 :121/:122 调用保留；_positive_int 现盘 :163 | 争议点: 无 | 承重误删风险: 无(直删未混入统一/透传/加形参,LB08 禁区全保留,三锁定隔离) | 需升级第三方裁: n
...
### R61
- DOSSIER: R61 | 状态: 2026-06-08 已 fixed | 终态: 旧 `filter_plan_rows_for_report_context` + `_plan_row_matches_batch` + `_plan_row_matches_resource` 死簇已删除；测试负向断言已重定向到 `normalize_report_resource_filter`；档案以 `.codestable/.../dossiers/R61.md` 为权威副本。

R61 | 执行前真实行号: filter_plan_rows_for_report_context→report_context_filters.py:172-187, _plan_row_matches_batch:160, _plan_row_matches_resource:164 | 执行后: 三死符号在 core/web/data/tests Python 代码零命中；当前 live `filter_downtime_rows_for_report_context:244`、`_row_text:156`、`normalize_report_resource_filter:119` 均保留 | 修法类: 已直删死簇 + 重定向两条负向测试到 kept 符号 `normalize_report_resource_filter`，零覆盖损失；非承重非收口 | 同文件兄弟需协调: 无(same_file_siblings=[];LB02/LB05/R14 都在 report_engine.py 不在本文件) | 最大爆炸风险: 后续重复清理时误删 live 孪生 filter_downtime_rows_for_report_context(report_engine:406)或共享 _row_text/normalize_report_resource_filter→大面积 ImportError | 与既有分析冲突: 无(「退测试 vs 重定向」已按更优重定向执行,与 registry 方向一致) | 前置: 已完成,后续不要重复执行 | owner_pending: n
- VERIFY: R61 | 核验: PASS | 终态复核: 三死符号零命中；live 孪生/共享符号现盘行号为 :244/:156/:119 且 report_engine/web 消费仍在 | 争议点: 无 | 承重误删风险: 无(lb=false,纯删死码,保留清单_row_text/normalize/filter_downtime孪生正确列禁删;零生产引用grep证实;重定向同源零覆盖损失成立) | 需升级第三方裁: n

...
| R28 | PARSE-INT | 🟢🟢·🟢— | 🟢 | _safe_float 静默吞错→loud raise 正方向，上游已 parse_optional_float 严校 except 不可达 | 必 allow_none=True（否则炸空 ext_days 批次）；先跑 `pytest -k allowlist` 定 fitness 白名单退留；禁动消费点工时兜底。 |
...
| R11 / R63 | GANTT | 🟢🟡🟡·🟡🟡 | 🟡 | 两份 _normalize 收口去重；support:58 改 return raw → raw 缺 available 默认 → collect_gantt_degradation_events 判 available 失效降级该报不报；收口手滑写 bool(available) → available=0 由 True 翻 False 静默放宽 | 先建 normalize parity 黄金基线（钉 available=0→True + bool() 包裹差异，5 边界）无 parity 不得收口；收口取 support 的 bool() 写法；禁 support:58 改 return raw；禁误删 _copy:104；A1 早于 A2/A3 禁拆两人各改一份。 |
...
- 原子性最终判定：A1{R11≡R63}必先于 A2{R12}/A3{R55}（倒序改两副本复活 P5）；R12 与 R55 同批一次穿白名单（同改单份 _normalize + contract unavailable 分支）；R10 同 gantt_service.py 任意序。
...
- 必须先落 parity/注释：normalize parity 黄金基线（钉 available=0→True + bool() 包裹差异，5 边界）是本簇唯一会让灵魂线静默失效的缺失前置，A1 动手前先建；R12 dropped_count 双补 :54 与 :328-334 且穿 _normalize 白名单 + contract:22-39；R55 scope 调用点显式赋值禁反推。

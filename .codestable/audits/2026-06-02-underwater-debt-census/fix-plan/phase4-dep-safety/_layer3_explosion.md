...
| R17 | EXEC-FACT | 🟢🟢🟢·🟢🟢·🟢🟢 | 🟢 fixed | 2026-06-08 已 fixed：service 死 import + support 推导式死键已删除；活键 `EXECUTION_ACTION_REPORT_EXCEPTION` 保留并加测试锁住。未触碰 `_build_event_payload` 的写死消毒层，也未触碰 R15 support raise | 执行终态：只删 R17 死物，未重构承重函数；后续禁按旧行号重复施工。R15 provider 坏时间解析仍留 G09。 |
...
| R20 | EXEC-FACT | 🟢🟢🟢·🟢🟢·🟢🟢 | 🟢 fixed | 2026-06-08 已 fixed：service labels 纯转出垫片已删除，5 个消费方已直连 `core.models.operation_execution_labels`，门禁测试与开发者指南已摘旧路径；漏改旧路会 loud ImportError | 执行终态：只改 import/删壳/摘门禁记账，未碰 LB01 禁区；后续禁按旧 facade 清单重复删除。 |
...
| R70 | LEAF-DUP-P4 | 🔴🟢🟢·🟢🟢 | 🟢 | **2026-06-08 已 fixed**；旧 schedule_service.py:46-50 死副本已纯删，live 在 run/input_collector.py:79 | 保 ValidationError import（并发拒绝路径仍用）。r1-LB 误判「:46 是承重正式函数 _raise_schedule_empty_result」，r2 逐符号 rg 推翻（:46 就是死副本本体本文件零调用），见回炉。 |
...
| R52 | GRAPH-ERR-DIAG | ✅ fixed | O07 已裁 KEEP；旧方向 A 的 31 用例蒸发/异常类不等价风险已通过“不删 impl、不迁测试、补认账注释”规避。 |
...
| 17 | **R52 dossier「~23 测试」严重失真** | GRAPH-ERR-DIAG/R52 | 历史方向 A 风险已由 KEEP 规避 | 实测 test_ready_queue.py 共 31 用例，约 18 个走 `_ready()`→全量版断 ReadyQueueContractError；当前保留全量版与测试，不迁移。若未来重启删除路线，必须显式裁定这 18 个全量版合同用例归属。 |

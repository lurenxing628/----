...
| **G10** | EXEC-FACT | {R18, R19 repo私有版} A4(repo) ✅ fixed | R18 独立处置 repo stub 护栏;R19 repo 私有版只补“顺序无关”注释/parity,二者仅同文件保守串行 | R19 repo 处已保私有版并补 parity；R18 已独立补 stub 组护栏注释（注释后现盘 6 格 = :356/:358/:402-:408）；R13 不碰 repo stub |
...
| **G23**🟢fixed(2026-06-10) | COMPAT-DISPATCH | {R33, R30} A1 | 同改 config_service_component_contract 测试+R30 删 shared 实现/R33 删壳 re-export,删序错即 ImportError | 硬序 R33 步1(迁两测试 import)→R30(删 shared 实现/三 FieldPolicy/三常量)→R33 步2/3(删壳+:411 断言) |
...
- 原 owner_pending ⏸ 裁后分流：G09/G10 已 fixed；G22/G27/G29/G33/G34/G40/G41 按 OWNER-DECISIONS 执行；G13/R55 本轮暂停，G15/R71 仅 parity，G26/R29、G30/R34、G39/R52、G42/R24 已裁后去挂起。
...
ROOT（承重注释 + 共享前置门，纯增量零结构，最先落）
  ...
  ├─ G40a  LB08 承重注释 + 绑 regression 契约                  [门控 G40 删 R46]🟢 fixed(O35 已裁 internal_operation 产出点)
...
Batch-C（身份族收敛 / 收口委托，依赖承重族 + parity）
  ...
  G17(R31)🟢fixed(2026-06-10) ← E05/E06 与 G23 同窗口
  G23(R30+R33)🟢fixed(2026-06-10) ← R33 步1 先（簇内硬序）

Batch-D（facade 删除最晚 / 跨 owner-pending 收口）
  G18(R26)🟢fixed(2026-06-10) ← 三桶收敛(E07/E08/E09)已全闭合 + E10 软自 G15
...
| 🟢 fixed | `core/models/scheduler_public_errors.py` | **LB08**(承重) + R46 | LB08 注释已在 LEGACY_PUBLIC_PATTERNS 上方；R46 死别名已删；禁区:62/:94/:142/:175/:218/:284/:340 未动；**含 R09 _positive_int 毗邻后续按符号重定位** | G40 已 fixed：LB08 注释先落→R46 删 `_safe_identifier`；R46 删段与禁区零重叠 |
...
| 🟡 中 | `tests/gate_meta/test_sp05_path_topology_contract.py` | R06 + R27 + gantt 空包(+R01/R26/R43 改别段) | R06/R27/gantt 同改点已 fixed；R01 改 schedule_persistence 键块:53-56；R26 改 SERVICE_BEHAVIOR_*:20-82；R43 改 ROUTE_* 段 | G38 已完成(现盘 :312 三元组+旧 delayed 循环已删);后续 R01/R26/R43 串行各改各段重 rg,禁碰现盘 :175 def/:317 起断言 |
...
| 🟢 低-中 | `core/services/scheduler/execution_snapshot.py` | R19 + R01 + R46(__all__ 伪串行登记) | R19 收口点 positive_op_ids:29-42+导出:123-128；`:42 return sorted` 事实承重(sha256 指纹)；R01/R46 的 `__all__` 不在本文件 | E16 已降登记备查；R19 强制保 sorted |
...
| **R13 解耦 R18** | R13 连带删 repo:254 方法,与 R18 同原子提交 | 死字段曾被测试读活；repo:400/402 已 stub raise（契约护栏）,R13 **不碰 repo**,R18 独立补注释 | **拆分原 R13+R18 同原子批**。R13(G09 provider 链) 与 R18(G10 repo 链) 解耦各自独立；O06 已裁先迁测试后删，2026-06-10 owner 已确认并完成 R13；R18 已补护栏注释 fixed |

...
> 铁律遵守：承重只补注释/收口+parity、不删/统一/透传；灵魂线不新增兜底；P5 收口到已存在点；分层 0 违规；owner_pending 只标不给终态。
...
> 编号 G## 为全局唯一。owner_pending 标 ⏸（只标不给终态）。承重门标 🔒。
...
| **G09** | EXEC-FACT | {R15, R19, R13} A3(provider) ⏸ | 同文件 provider.py 删改互移行号;先收口语义(R15/R19)后删死物(R13) | 强序 R15 先(SCC 最前置,收口解析)→R19 改:70-82→R13 末(删字段);R15 provider 处+R13 升 owner |
...
| **G13** | GANTT | {R55} A3 ⏸ | 加 scope=filtered/full,**必须加在 G11 统一后单份** helper,default full,filtered 调用点显式覆写 | G11 先于 G13(强偏序);禁 helper 内靠 filters 空否反推;G12⟂G13 同改 _normalize 宜同批穿三道白名单 |
...
| **G15** | CONFIG-DUAL | {LB07, R71, R47} ASC-1 ⏸ | 🔒 config 双栈 helper 锁步收敛;LB07 承重安全网前置 R71/R47;R71/R47 同两栈文件对称删动作 | 硬序 LB07 先(注释+扩 parity,Batch-1 ROOT)→R47+R71 同批(parity 绿后;先删 R47 死参再 R71 收敛);R71 物理收敛动作待 owner 裁 |
...
| **G22** | PARSE-INT | {R08, R09} A4 ⏸ | 同文件 viewmodel 串行避免行号互撞;R09 跨子簇(B 副本归此,A 副本归 G19 邻域) | R08 先(B01)→R09 后(B05)串行;owner 未裁前整体不进批;R09 双副本分两路 parity(C 严格 5.9→None vs A/B 宽松 5.9→5) |
...
| **G25** | COMPAT-DISPATCH | {R49 之 evaluation/ortools 4 行} A3 | R49 本体跨 3 文件,evaluation:40-41/ortools:24-25 与 dispatch_rules 不同文件互不影响 | 可独立删,无行号互撞无序约束 |
| **G26** | COMPAT-DISPATCH | {R29} A4 ⏸ | 孤立,误标→planned(owner-pending);授权 CSV ABSENT+全量 delegation-facade 半截在场 | 不进任何原子删除批;KEEP(补注释) vs B 收敛(先重写 monkeypatch 为身份测试);只标不给终态 |
...
| **G29** | PLAN-IDENTITY | {R72} ③ ⏸ | 独立叶子,收口落点 web scheduler_utils,与 R44 协调非排序 | 单债;与 R21/R55/R44 同文件行号联动(谁后做谁重 rg);owner 裁后落 |
| **G30** | RESOURCE-REPO | {R34, R35} AC-1 ⏸ | 同物理文件 schedule_repo.py 硬同批,R35:61-69 夹在 R34 删段:59↘:114 之间 | 无功能先后作一原子 diff,按符号名自下而上删;R34 owner_pending 整体挂起(detail_queries 整删 vs 迁移待裁) |
...
| **G33** | RESOURCE-REPO | {R05} AC-3 🔒⏸ | 🔒 收口点扩容三原子小步,步内不可换序;本簇唯一承重(verdict=LB) | 强序 步1 扩 collar(加 team 谓词+放开空 id 全量)→步2 补 5 条 parity→步3 收敛 repo:461-463/:454-460+_normalize_scope_type;owner 裁 collar 形态/(team,"")语义 |
| **G34** | RESOURCE-REPO | {R67} AC-4 ⏸ | 抽单一常量,4 处手抄须全收或①②必收③④酌情;半截去重比现状更危险 | 自身不可拆;常量须 tuple 保序;owner 裁是否收编第 4 处 superset |
...
| **G39** | GRAPH-ERR-DIAG | {R52, R25} A3 ⏸ | 同符号 get_ready_operation_ids 的 shell+body,impl 删则垫片:9 ImportError,必同提交 | R52 决策门先裁(A 删 vs B 保留作差分 oracle);走 A 则先迁 23 测试+3 ValidationError 契约到新建 test_sgs_graph_ready.py→再删 impl+R25 垫片 |
| **G40** | GRAPH-ERR-DIAG | {LB08, R46} A4 🔒⏸ | 🔒 同文件 scheduler_public_errors.py,LB08 注释钉死承重边界后 R46 才能保护下删:162-164 | LB08 注释先落(或同提交先于 R46)→R46 删:162-164(按符号 grep 重定位,不照搬);LB08 注释逐字文案产出点待裁 |
...
| **G42** | GRAPH-ERR-DIAG | {R24} A6 ⏸ | 独立单债,82 行零消费孪生副本(ISOLATED) | 路 A:先调和 networkx roadmap→删 core 整文件→测试逐条剪(混合用例:82 仅删首行:83 保:84+ web 断言);路 B 违铁律 5 须另立 P5 |
...
- owner_pending ⏸（只标不给终态）：G09,G10*,G13,G15,G18,G22,G26,G27,G29,G30,G33,G34,G39,G40,G41,G42（部分单元内含 owner 裁断点；G09/G15/G27/G30/G33 整单元受 owner 门挂起或部分挂起）

> 注：73 条债（80 原始 − 已 fixed/误标净化）全部归入上述单元。已 fixed 6 债（LB03/LB06/R07/R16/R56/R57，corrections E）不占调度单元，仅作前置已完成态 + 认账残留（见 §5）。新债 N1/N2（corrections C）不单独成原子单元，挂在毗邻单元同期处置（N1↔G04/G22、N2↔G04/G22 邻接，补注释+绑契约，owner_pending）。
...
| E07 | G18(R26) → G26(R29) | facade 删晚于收敛 | **H** | PHASE0§10.2;R26 顶层 shim 2 离线消费者经老路径,先删 shim 致其测试红 |
| E08 | G18(R26) → G23(R33) | facade 删晚于收敛 | **H** | 同上(B06 桶) |
| E09 | G18(R26) → G39(R52) | facade 删晚于收敛 | **H** | 同上(B09 桶) |
| E10 | G18(R26) → G15(R71) | 转出边 | **S** | R26 顶层 config_snapshot.py 转出 R71 编辑的深文件;R71(B03)先 R26(B14)后天然满足,不同文件不撞行号 |
| E11 | G18(R26) ↔ G19(R01) / R43 | SP05 同文件串行 | **S** | 各改 SP05 不同模块键段,串行编辑避 git 行号漂移误合,无逻辑依赖 |
| E12 | G27(R22) → LB03(已 fixed? 否,B01 承重) | parity 先于收敛 + same_symbol build_plan_identity | **H** | LB03(B01 承重注释+guard 收口)须全局先落于 B02 身份族;R22 先落 24 键 exact parity 钉键集再收敛 |
| E13 | G41(R14) → G07(LB01) | 承重让位 + same_symbol _resolve_strict_plan | **H** | R14 删:134-139 前 LB01 承重裁断必先行(同符号,删除让位) |
...
| E16 | G09/G10(R19) ↔ G19(R01) / G40(R46) | __all__ 同符号块 | **S→部分 H** | execution_snapshot.py:118 导出块共享,高顺序敏感;R19 调整导出须与 R01/R46 串行对账最终导出表(EXEC-FACT 标真边) |
...
| E22 | G13(R55) → G27(R21)/G04(R44)/G29(R72) | 消费者只读触发条件勿砍 | **S** | R55 只读 scheduler_gantt:345-351,他债改该段勿砍 resource_type/resource_id 触发条件 |
...
| E25 | G22(R09) → R07(已 fixed) | 前置已完成 | **P** | R07 改 raise 已落(偏离用 ValidationError 待 owner 认账);R09 收 A 副本(service:24)在 R07 稳定行号重盘——A 副本未被 R07 触碰 |
...
> 全局承重根（B01/B03 承重注释门）必须最先。owner_pending 单元标 ⏸ 落在「裁断后批」。
...
ROOT（承重注释 + 共享前置门，纯增量零结构，最先落）
  ...
  ├─ G40a  LB08 承重注释 + 绑 regression 契约                  [门控 G40 删 R46]🔒⏸(文案产出点待裁)
...
Batch-B（依赖 ROOT 承重门 / 单门控前置）
  ...
  G12(R12)         ← G11
  G13(R55)⏸        ← G11（owner 裁本轮是否做）
  G19(R01+R04)     ← GF1（+ E24 与 G40a 认账协同）
  G20(R59)         ← GF1
  G24(R49+R50+R51) ← schedule_params/optimizer_config 收口点只读在位（已满足）
  G40(R46)         ← G40a 🔒⏸
  G30(R34+R35)⏸    ← owner 裁 detail_queries（E18 软自 G33）
  G36(R37)
  G39(R52+R25)⏸    ← R52 决策门 + 迁 23 测试到新文件
...
Batch-C（身份族收敛 / 收口委托，依赖承重族 + parity）
  ...
  G09(R15→R19→R13)⏸ ← provider 链 owner 裁 R15 收法/R13 删留（E16 与 G19/G40 __all__ 串行对账）
  G10(R18+R19+R13)⏸ ← repo 链 owner 二次确认 R13 删留
  G29(R72)⏸        ← owner 裁 web/core 落点（E21 软自 G04）
  G15(R47+R71)⏸    ← G15a 🔒（R71 物理收敛待 owner 裁）
  G22(R08+R09)⏸    ← N1 注释(E26) + R07 前置(E25,已满足)；owner 裁
  G33(R05 步3)🔒⏸  ← G33a 步1/步2（owner 裁 collar 形态）
  G34(R67)⏸        ← owner 裁第 4 处 superset 收编
...
Batch-D（facade 删除最晚 / 跨 owner-pending 收口）
  ...
  G26(R29)⏸        ← owner 裁 KEEP vs 薄壳化（KEEP 则不阻塞，B 收敛则前置 G18）
...
- 唯一双向标记的边均为 **S（同文件行号联动 / 同符号 __all__ 对账 / 认账协同）**，不构成 H 环：
  - E16（R19↔R01/R46 __all__）：同符号块共居，按「最终导出表对账」串行化即可定向（R19 调整导出 → R01/R46 跟随对账），不形成回边。
  - E24（R04↔LB08）：认账协同非删除依赖，LB08 注释先落（ROOT）→ R04（Batch-B），单向。
  - E11（R26↔R01/R43 SP05）：同文件串行编辑各改各段，无逻辑依赖，可任意定序，非环。
- 跨簇 H 链全部单向收敛：`GF1→{G19,G20}`、`G04→G01`、`G23→G17`、`G18→{G26,G23,G39}`、`G41→G07`、`G27→LB03`、`G18→G15(S)`——无任一汇点回指其源。
...
| 🟠 高 | `core/data/repositories/schedule_repo.py` | R34 + R35 | R34 删:36-59/:114-126/:128-158；R35 删:61-69(**夹在 R34 删段:59↘:114 之间**) | G30 一原子 diff 按符号名自下而上删(:128-158→:114-126→:61-69→:36-59);R34 owner 门挂起 |
...
| 🟡 中 | `web/viewmodels/scheduler_resource_dispatch_execution.py` | R08 + R09(B 副本) | R08 改死常量:25/死分支:227-228/:367-368；R09 改 B 副本 def:33+调用点:257/:258/:353；同文件删行位移彼此锚点 | G22 串行 R08 先(B01)→R09 后(B05);双 owner_pending 整体挂起;R09 B 副本宽松 5.9→5 须分路 parity |
...
| 🟡 中 | `tests/test_sp05_path_topology_contract.py` | R06 + R27 + gantt 空包(+R01/R26/R43 改别段) | R06/R27/gantt **同改:310 存在循环+:315 delayed 循环**(逐增量摘致 old_string 失配)；R01 改 schedule_persistence 键块:53-56；R26 改 SERVICE_BEHAVIOR_*:20-82；R43 改 ROUTE_* 段 | G38 四包一次性同提交(改:310 成三元组+删:315-316);禁碰:173 def/:318 断言;R01/R26/R43 串行各改各段重 rg |
| 🟡 中 | `tests/regression_config_service_component_contract.py` | R33 + R30(交界) | R33 删:14/:16/:19 元组+:393-411 身份断言；交界:411 `parse_compat_date is` 在 R30 删实现后失效由 R33 步2 删 | G23 内 R33 步2/3 在 R30 之后 |
| 🟢 低-中 | `core/services/scheduler/execution_snapshot.py` | R19 + R01 + R46(__all__ 块) | R19 收口点 positive_op_ids:28-40+导出:118-123；R01/R46 共享 __all__ 块；`:40 return sorted` 事实承重(sha256 指纹) | E16 串行对账最终导出表;R19 强制保 sorted |
...
| | | `core/services/scheduler/run/schedule_persistence_errors.py:13` | 〔R1-P2 新认〕**第 3 份裸内联未收口**：`int(value or 0)` **不** wrap 收口点，调用点 :24/:56(`or 0`)/:77。**owner 复核**：若 R09 同族第 3 副本则 G22 收编面 2→3 份、parity 分路口径扩；若独立 run/ 包解析器须在 registry 标「同符号非 R09 范围」防误并。**只标不给终态** |

> **R1-P2 收编面订正**：corrections A/§5.1 原称「剩 2 旧内联副本（viewmodel:33 + service:24）」。实盘 Optional-family 未收口副本**至少 2 份已认（viewmodel:33 / service:24）+ 1 份待 owner 认（schedule_persistence_errors:13）**。service A 副本不止 def:24 一处，含调用点 :169/:180/:181，parity 须钉调用点比较语义（5.9→5 vs 5.9→None 放宽点在 :180-181），不是改 def 一处即可。
...
| 关系类型 | 条数 | 占比 | 说明 |
|---|---|---|---|
| **H 硬序**（违序即破坏） | 13 | 45% | E01,E02,E03,E05,E06,E07,E08,E09,E12,E13,E16(部分),E26(注释面),GF1 两门 |
| **S 软序**（零成本先后/重 rg 纪律） | 14 | 48% | E04,E10,E11,E14,E15,E17,E18,E19,E20,E21,E22,E23,E24,E27,**E28(R1-P1)**,**E29(R1-P3)** |
...
| A07 | R19→R01/R46（__all__ 同符号坐实） | EXEC-FACT/GRAPH | H | execution_snapshot:118 导出块共享 |
| A08 | R11/R63→R12（收口前置硬边 A1→A2） | GANTT | H | R12 加键必须在统一后单份 helper |
| A09 | R11/R63→R55（收口前置硬边 A1→A3） | GANTT | H | R55 加 scope 同理 |
| A10 | R12⟂R55（同批穿三道白名单） | GANTT | S | 新键落同 _normalize |
| A11 | R26→R29（facade 删晚于收敛） | CONFIG-DUAL | H | 普查"0 消费者"被推翻,2 离线消费者 |
| A12 | R26→R33（facade 删晚于收敛） | CONFIG-DUAL | H | 同上 |
| A13 | R26→R52（facade 删晚于收敛） | CONFIG-DUAL | H | 同上 |
...
| A17 | R04→6 处 except 同改 + R09 双副本分裂 + R22→LB03 + R67→R42 第二文件 + R06+R27+gantt SP05 两行 + R24 测试:83 | 多簇 | H/S | 见各簇 §C（合并计为 1 行多项,实条数另见下注） |

> A17 实含 6 个独立新增点（R04→6except 内部、R09 双副本分裂、R22→LB03、R67→R42 第二文件覆盖、R06+R27+gantt SP05 同两行原子边、R24 测试:83 单行剪）。**新增总计按独立边/原子点核：17 条**（A01–A16 + A17 折算 R22→LB03 + R06+R27+gantt SP05 + R67→R42 第二文件三条跨簇真边；其余 R04→6except / R09 双副本 / R24:83 属单元内覆盖扩展不计跨簇）。
...
|---|---|---|---|
| **R09 收口点已存在** | 「唯一批准新建 parse_optional_positive_int」 | 收口点 `operation_execution_scope.py:9 parse_positive_execution_int` **已存在**（执行重构新建,bool/非数字/<=0 loud raise）；3 新文件已正确收口 | **作废「新建模块」批次步骤**。〔红队第1轮修订 R1-P2 收编面订正〕R09 已认 2 旧内联副本（viewmodel:33 B副本 + service:24 A副本，A 副本含调用点 :169/:180/:181，放宽点在 :180-181 比较）**+ 待 owner 认第 3 份**（schedule_persistence_errors.py:13 裸 `int(value or 0)` 不 wrap 收口点——若同族则收编面 2→3、parity 分路口径扩，若独立 run/ 包须 registry 标「同符号非 R09 范围」）；须**分两路 parity**（C 路严格 5.9→None，实证锚点 context.py:28-30 见 E29 vs A/B 宽松 5.9→5），owner 裁语义。收口点同住一文件的 LB01 最终底:36-50 禁碰（E28）。同名异义 family 对照见 §3.1（STRICT 4 处禁删 vs Optional 5 处）。G22 不再含「建模块」步，改「收编+分路 parity」。R46→R09 由「同文件硬边」降软位移（E15） |
| **R13 解耦 R18** | R13 连带删 repo:254 方法,与 R18 同原子提交 | 死字段被 3 测试读活（reschedule:196、scope_read_contract:108/190/220）；repo:399/401 已 stub raise（契约护栏）,R13 **不碰 repo**,R18 独立补注释 | **拆分原 R13+R18 同原子批**。R13(G09 provider 链) 与 R18(G10 repo 链) 解耦各自独立；planned_fix 步 3/4 作废；R13 直删→**升 owner 二次确认删/留**（先改 3 测试） |
...
### 5.2 执行重构新引入债（纳入清单，均 owner_pending）
...
- **已结构性消除 6 债**：LB03 / LB06 / R07 / R16 / R56 / R57 → **DAG 前置已完成,从批次中移除占位**，仅留认账残留（见各簇 §E）：
  ...
  - R07：用 ValidationError 非 AppError/NOT_FOUND,跨文件错误类不对称,owner 裁是否统一 + 缺 schedule=None→raise 回归
  - R56：偏离铁律 3,owner 认账（见 5.1）
  - R16：已修,R15 state_builder 处已落地,原「R15 避让 R16 删表」前置消解
- **R29 误标纠回 planned(owner-pending)**：权威 CSV 整目录 ABSENT,common/number_utils.py 仍全量 delegation-facade、4 兄弟全薄壳,半截迁移不对称客观在场。**重新入排**（G26,KEEP vs 薄壳化,owner 裁）；R26→R29 facade 删序前置（E07）因此**复活**为真前置。
...
1. **批次数收缩**：旧 16 批因含「新建模块（R09）」「R13+R18 同原子」「R34 收敛重构」「R56 承重待做」等已被推翻/已完成的占位批。重建后有效分层为 **ROOT + 4 大批（A/B/C/D）**（§2.2），其中 A/B 多为零前置可早落，C/D 集中 owner_pending 与承重收敛。
2. **承重根前置统一上提**：GF1 默认值门 + LB01/LB02/LB05/LB07/LB08/LB03/R05-step1/R22-parity 全部上提为 ROOT（纯增量零结构，先落），是 13 条 H 边的共同 source。
3. **owner_pending 单元后置**：G09/G10/G13/G15/G18/G22/G26/G27/G29/G30/G33/G34/G39/G40/G41/G42 含 owner 裁断点，**只标不给终态**，集中在 Batch-C/D，裁断前不锁 patch。
...
> 裁定原则：承重(load_bearing/N1/N2)只补「我是故意的」注释+绑契约/parity，绝不删/统一/透传；灵魂线不新增兜底；P5 收口到已存在点；分层 0 违规；owner_pending 只标不给终态。
...
原子簇 42 个（G01–G42）+1 共享前置门 GF1 = 43 调度单元（多债强原子 20 / 单债 22 / 承重门 5🔒 / owner_pending 16⏸）。
验环：**无环（DAG 成立，环成员=空）**——13 条 H 硬边全单向收敛，唯一双向标记的 E16/E24/E11 均为 S 软序可定向。
批次草案（序）：ROOT（GF1 默认 False + LB01/LB02/LB05/LB07/LB08/LB03/R05-step1/R22-parity 承重注释+parity，纯增量零结构）→ Batch-A（零前置死叶子 G03*/G14/G16/G21/G25/G28/G31/G32/G35/G37/G11/G38）→ Batch-B（单门控前置 G06/G07/G08/G12/G13/G19/G20/G24/G40/G30/G36/G39）→ Batch-C（身份族收敛 G04→G01/G27/G09/G10/G29/G15/G22/G33/G34/G17/G23）→ Batch-D（facade 最晚 G18←三桶收敛 + G26/G41/G42）。
...
与旧 DAG 主要差异：R09 收口点已存在→作废建模块改双路 parity；R13 解耦 R18 各自独立+升 owner；R34 纯删（repoint 目标存在）+R05→R34 降软；R54 升 5 套手维面（gantt_task_detail 别名元组+**三基数 16/15/12 禁统一键名**）；R56 入 fixed（偏离铁律 3 待认账）；新债 N1（can_write_feedback 失忆债门控 R08）/N2（return 0 sentinel）补注释+绑契约；R03 四态 parity（missing 态:267 生产可达，非全死分支）；R29 误标纠回 planned 复活 R26 facade 删序前置；批次由 16 收缩为 ROOT+4 大批，owner_pending 后置 Batch-C/D 只标不给终态。
红队第1轮：**采纳 11 / 驳回 0 / 降级保留 3（按提示采纳精度）**。关键修订=补 E28(R09 收口家↔LB01 最终底同文件)/E29(N1↔R09 C 路同文件)两 S 边；§3 补登 5 文件(承重 5→6、重灾区 19→24)；新增 §3.1 `_positive_int` family 对照表(STRICT 4 禁删 vs Optional 5 收候选,认 schedule_persistence_errors:13 第 3 副本 owner 复核)+§3.2 fail-CLOSED 方向硬门(防认账注释粘反向 fail-OPEN)；R54 三基数(16/15/12)分组 parity 禁向 16 键看齐；facade 删序方向修正 A14→R33→R31、E06/A15→R33→{R30,R31}；E03 补 collar 第 6 调用方 analysis_links:24(未传 plan_id 安全)。全图仍无环。

...
| R34 | `get_version_time_span` | schedule_repo.py:36-59 | 活孪生 `schedule_plan_query_repo.get_plan_time_span`:257 | repoint 目标 `get_plan_time_span_for_resolution` **存在=schedule_plan_query_service.py:210**（dossier 正文误判“不存在”，verify 已纠正） |
...
| R36 | `get_by_op_code` / `list_by_status` | batch_operation_repo.py:25-35 / 50-61 | 无 | ISOLATED |
| R37 | `list_links_with_machine_names` | operator_machine_repo.py:82-90 | 无 | 活近亲 `list_links_with_operator_info`:92 **勿误删** |
| R38 | `list_as_dicts`×3 | op_type_repo.py:73 / operator_repo.py:85 / part_repo.py:71 | 无 | 结构同形 SQL 各异，**非字面同体**，记三笔独立删，禁抽 helper |
| R39 | `list_unparsed` | part_repo.py:32（体:33 转调活方法 list:20） | 无 | 删它使下方 R38(:71) 上移 2 行 |
...
- **内部顺序**：**无功能先后**，作一个原子 diff。执行纪律：同提交内 **按符号名（非行号）定位、自下而上删**（先 :128-158 → :114-126 → :61-69 → :36-59，边删边算偏移最稳）。R35 owner_pending=false 给终态；R34 owner_pending=true（detail_queries 整删 vs 迁移、benchmark repoint 层级待裁），故 AC-1 整体受 R34 的 owner 门挂起。
- 同批连带退场（R34 名下，R35 不碰）：facade_delegation.py:31/36/39-41 三断言 + :12/:14 两 import；detail_queries.py:168+9 处（共 10）；gantt_critical_chain_unavailable.py:59 无效 monkeypatch；benchmark_fjsp.py:503 repoint。删错全是**响亮 AttributeError，无静默炸点**。

### 子簇 AC-2 = {R38(part 份), R39}（part_repo.py，硬同批，行号坐标耦合）
...
### 子簇 AC-2′ = R38 的 op_type / operator 两份（可独立，仅随 R38 同 PR）
- op_type_repo.py:73-74、operator_repo.py:85-86 各自零引用零碰撞，**可独立删**，无同文件兄弟。与 part 份同属 R38 一笔账但物理隔离。
...
### 独立单点
- **R36**（batch_operation_repo.py:25-35 / 50-61，ISOLATED）：零碰撞零跨边，直删两方法（从后往前），owner_pending=false，本桶最早可落之一。
- **R37**（operator_machine_repo.py:82-90，直删）：同文件零碰撞、owner_pending=false。唯一执行纪律=diff 锁 :82-90 + 删后 grep 活近亲 `list_links_with_operator_info`:92 仍在（命名一字之差，误删静默炸设备页人机联动分组）。
...
1. **R34 ↔ {R10,R11,R12,R21,R55,R63}**（gantt_service.py 同文件）+ **R34↔R23**（schedule_plan_query_service.py 同文件）：经 rg 确认这 7 个邻居**全在 C01**——属**簇内弱耦合**（same_file=true, same_symbol=false），非跨簇。R34 本体动 schedule_repo.py，**不碰 gantt_service.py/schedule_plan_query_service.py 的方法体**，仅 benchmark repoint 若选服务层会读 schedule_plan_query_service.py:210（不改其体）。对 AC-1 排序无硬约束。
...
4. **R37 ↔ R41**（equipment_pages.py 同文件）：R41 在 C01。**伪干扰边**——R37 直删动 operator_machine_repo.py:82-90，**根本不碰 equipment_pages.py**，零改动行。对 R37 无排序约束，应删此边（见 C 节）。
...
- **删 R34↔{R10,R11,R12,R21,R55,R63,R23}**（作为“跨簇硬边”理解时）：这些是 C01 簇内 same_file 弱耦合，R34 本体不动 gantt_service.py/schedule_plan_query_service.py，**不构成排序边**；保留为“同簇知情”但**降为零权重**（不门控批次）。
...
### 降级
- **R05→R34 硬依赖 → 软约束**（corrections 明列）：旧叙事假设“R34 收敛到 column_name / 收敛到活孪生”，本轮回盘证 R34=**纯删死方法**，其 `schedule_repo.list_dispatch_rows_with_resource_context` 与 R05 改的 `schedule_plan_query_repo.list_dispatch_rows` 是**不同符号、不同文件**，无硬数据依赖。降为软约束（保 R05 Batch-8 先于 R34 Batch-13 的零成本先后；仅 R34 选“迁用例到活孪生”时才回升为硬前置）。
- **R13↔R18 解耦**：不涉本簇（登记备查）。

### 本簇无关 fixed 列表（corrections E）
- LB06/R56/R57/R07/R16/LB03 已 fixed——**均非本簇成员**，对本簇无前置门控（本簇成员 R05/R67/R34/R35/R36/R37/R38/R39 无一在 fixed 集）。
...
### 其余成员承重=无禁区
- R34/R35/R36/R37/R38/R39 全 load_bearing=false、lb_no_touch=null、文件非承重，**纯直删无承重禁区**。R34/R35 唯一“禁区”是相邻**活方法保护**（非承重）：list_version_rows_by_op_ids_start_range:71 + list_by_version_with_details:160 + 其 facade 断言 :32-34/:37 必须保留。R37 同理保 list_links_with_operator_info:92。

...
| P1-18 | PR-7 | complexity, ui_contract, test_coverage | `开发文档/技术债务治理台账.md` 的 `complexity:web-routes-domains-scheduler-scheduler_run-run_schedule` 登记仍在；当前 route 入口是 `web/routes/domains/scheduler/scheduler_run.py` 的 `run_schedule`。 | open | 是 | 否 | PR-7 处理 `/scheduler/run` view result。 | 旧编号来源证据不足；当前事实源成立。 |
| P1-19 | PR-8 | complexity, ui_contract, test_coverage | `开发文档/技术债务治理台账.md` 的 `complexity:web-routes-domains-scheduler-scheduler_batches-batches_page` 登记仍在；当前页面入口是 `web/routes/domains/scheduler/scheduler_batches.py` 的 `batches_page`。 | open | 是 | 否 | PR-8 处理 batches_page viewmodel。 | 旧编号来源证据不足；当前事实源成立。 |
| P1-20 | PR-9 | oversize, startup_regression | `开发文档/技术债务治理台账.md` 的 `oversize:web-bootstrap-launcher` 登记仍在；`.limcode/plans/20260405_技术债务最终合并修复plan/subplans/SP03_启动链静默回退专项.md` 把 launcher 归为 C 类台账化热点。 | open | 是 | 否 | PR-9 先作为支线，不插入排产主链。 | 旧编号来源证据不足；当前事实源成立。 |
| P1-21 | PR-9 | complexity, startup_regression | `开发文档/技术债务治理台账.md` 的 `complexity:web-bootstrap-launcher-_classify_runtime_state` 登记仍在；当前入口是 `web/bootstrap/launcher.py` 的 `_classify_runtime_state`。 | open | 是 | 否 | PR-9 处理 runtime state 分类，必须有启动链专项测试。 | 旧编号来源证据不足；当前事实源成立。 |
| P1-22 | PR-9 | complexity, startup_regression | `开发文档/技术债务治理台账.md` 的 `complexity:web-bootstrap-launcher-_list_aps_chrome_pids` 和 `complexity:web-bootstrap-launcher-stop_runtime_from_dir` 登记仍在。 | open | 是 | 否 | PR-9 处理 Chrome/stop runtime，不能模糊写成 plugin。 | 旧编号来源证据不足；当前事实源成立。 |
| P1-23 | PR-9 | complexity, startup_regression | `开发文档/技术债务治理台账.md` 的 `complexity:web-bootstrap-plugins-_apply_enabled_sources` 登记仍在；当前入口是 `web/bootstrap/plugins.py` 的 `_apply_enabled_sources`。 | open | 是 | 否 | PR-9 处理 plugin enabled source 合同。 | 旧编号来源证据不足；当前事实源成立。 |
| P1-24 | PR-9 | silent_fallback, startup_regression | `开发文档/技术债务治理台账.md` 的 `silent_fallback.entries` 中仍保留 `path=web/bootstrap/plugins.py` 相关 plugin fallback 历史记录，多数状态为 `fixed`。 | mostly-fixed | 否 | 否 | PR-9 执行前只能复核是否有仍 open 的 plugin fallback，不能默认当作未修债务。 | 旧编号来源证据不足；当前事实源多为已修记录。 |

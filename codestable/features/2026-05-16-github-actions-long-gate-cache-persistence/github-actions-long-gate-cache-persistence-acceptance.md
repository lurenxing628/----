...
- PR 不保存缓存：当前 save 条件只允许 `push` 和 `workflow_dispatch`，所以 fork PR 和同仓 PR 都不会把运行产物写回 cache。
...
- 缓存路径只包含已忽略的 `evidence/QualityGate/` long gate 运行产物，其中包括 `evidence/QualityGate/quickref_vs_routes.md`。
- 缓存路径没有包含已跟踪的历史文件 `evidence/Conformance/quickref_vs_routes.md`。
...
- 没有改变 CI 质量门禁命令。
- 没有缓存历史 tracked `evidence/Conformance/quickref_vs_routes.md`。
...
- PR 不 save，可以避免 PR 运行产物写回主仓库缓存；只有 main push 和手动 workflow 在门禁成功后保存。
...
- 建议在 GitHub Actions 上观察一次 main push 或手动 workflow：确认 restore 在门禁前执行、save 在门禁成功后执行。
- 建议再观察一次 pull request：确认 workflow 可以跑门禁，但 save step 被跳过。

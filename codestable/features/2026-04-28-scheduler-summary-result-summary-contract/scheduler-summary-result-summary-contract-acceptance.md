...
PR-5 已完成 summary/result_summary 合同补证。本次没有改运行代码，只补测试和 CodeStable 追踪文档，用真实落库、真实读回和真实页面 HTML 证明公开摘要和内部诊断没有混在一起。
...
- `diagnostics.optimizer.attempts` 在正常大小下随历史 JSON 落库并读回，rejected 诊断保留 `origin.type`、`origin.field`、`origin.message`。
...
- 无新增兜底对抗复审：无阻塞；确认未提交 diff 没有运行代码改动，也没有在模板/route/viewmodel 里补过滤。

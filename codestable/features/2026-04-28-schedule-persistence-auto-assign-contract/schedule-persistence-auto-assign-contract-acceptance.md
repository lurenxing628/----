...
- 新增 `tests/test_schedule_persistence_auto_assign_contract.py`，覆盖 `out_of_scope_schedule_rows > invalid_schedule_rows > no_actionable_schedule_rows`、全部非法无可落库行、`simulate=True`、`auto_assign_persist=no`、外协隔离、已有资源不覆盖、只补空字段和 missing set 门控。
...
- `tests/test_schedule_persistence_auto_assign_contract.py`：新增 PR-6 专项合同测试。
...
- `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m pytest -q -p no:cacheprovider tests/test_schedule_persistence_auto_assign_contract.py`：4 passed。
- `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m pytest --collect-only -q tests | rg "tests/test_schedule_persistence_auto_assign_contract.py|tests collected"`：默认收集已包含 4 个 PR-6 专项测试，`748 tests collected`。
...
- `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m ruff check core/services/scheduler/run/schedule_persistence.py tests/test_schedule_persistence_auto_assign_contract.py`：通过。
...
- `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python tools/check_full_test_debt.py`：通过，active xfail 仍为 5，`collected_count=748`。
...
- `git diff --check`：通过。
- `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python scripts/run_quality_gate.py --require-clean-worktree`：复审修正提交后在干净工作区复跑，通过。

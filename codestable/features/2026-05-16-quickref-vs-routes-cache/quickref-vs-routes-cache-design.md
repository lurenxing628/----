...
- quickref report：`evidence/Conformance/quickref_vs_routes.md`。它是这条命令运行后生成的对账报告，只证明系统速查表和当前路由是否一致，不证明整个仓库通过 final gate。
...
- `evidence/Conformance/quickref_vs_routes.md` 当前已有历史跟踪文件；本 feature 的提交边界不新增或更新这份运行报告内容。

变化：

- 将 `ENTRY_QUICKREF_VS_ROUTES` 加入 enabled 集合。
- 为 quickref entry 增加 input/config/tool/dependency/env scopes。
- 保持输出文件声明为 `evidence/Conformance/quickref_vs_routes.md`，让通用 cache 层校验它存在且 hash 匹配。
...
- `scripts/run_quality_gate.py`：clean-worktree generated path 说明包含 quickref report。
...
- S5：`evidence/Conformance/quickref_vs_routes.md` 缺失或 hash mismatch 时不复用。

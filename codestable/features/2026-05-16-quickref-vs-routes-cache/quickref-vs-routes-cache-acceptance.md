...
- 已为 `quickref_vs_routes` 声明 `output_result_files`：`evidence/QualityGate/quickref_vs_routes.md`；历史 tracked `evidence/Conformance/quickref_vs_routes.md` 不再作为 long gate 运行输出。
...
- 没有把 cache hit 或 `evidence/QualityGate/quickref_vs_routes.md` 当 clean-worktree final proof。
- 没有提交 `evidence/QualityGate/quickref_vs_routes.md` 的本次生成内容。
...
- `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python tests/check_quickref_vs_routes.py`：后续修复后 stdout 应为 `evidence/QualityGate/quickref_vs_routes.md` 和 `OK`。
...
- `evidence/QualityGate/quickref_vs_routes.md` 只证明 quickref 对账报告内容，不证明整个仓库 clean-worktree final gate 通过。

    ...
    *,
    ...
    for bid, b in batches.items():
        ...
        if due_d:
            ...
            due_min_by_batch[bid] = max(0, int(due_min))
        else:
    ...
    for idx, (bid, dur, w) in enumerate(jobs):
        ...
        t = model.NewIntVar(0, horizon * 10, f"tardy_{idx}")
    ...
    if rest:
        # 交期早/高优先级的优先（简单排序）
        rest.sort(key=lambda bid: (due_min_by_batch.get(bid, 10**9), -_priority_weight(getattr(batches.get(bid), "priority", None)), bid))

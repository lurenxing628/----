    ...
    def restore(self, backup_path: str) -> RestoreResult:
        ...
        try:
            with maintenance_window(self.db_path, logger=self.logger, action="restore"):
                # 恢复前自动备份：失败必须与「恢复本身失败」显式区分。
                # 此处 _copy_db_file 尚未执行、原库一字未动；用户要知道是「恢复前保护快照
                # 没通过」而非笼统「恢复失败」，且原库安全。MaintenanceWindowError 须继续上抛
                # 交外层处理（busy/锁语义），不得误并入快照失败分支。
                try:
                    ...
                    fallback_log(
                        self.logger,
                        "error",
                        f"恢复前保护快照创建/校验失败，已中止恢复（原库未改动）\n{traceback.format_exc()}",
                    )
                    return RestoreResult(
                        ok=False,
                        code="pre_restore_snapshot_failed",
                        message="恢复前保护快照创建或完整性校验未通过，已中止恢复（原数据库未改动），请查看日志。",
                        before_restore_path=None,
                    )

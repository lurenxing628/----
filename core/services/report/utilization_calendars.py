...
def resource_calendars(conn, rows, start, end):
    ids = {kind: sorted({str(row[kind + "_id"]) for row in rows if row.get(kind + "_id") not in (None, "")
    ...
    if dates > MAX_CALENDAR_DATES or dates * (1 + sum(len(keys) for keys in ids.values())) > MAX_POLICY_CELLS:
        return _unavailable(ids, "calendar_range_limit" if dates > MAX_CALENDAR_DATES else "calendar_cell_limit")
    ...
    for row in reader.rows["downtimes"]:
        downtime[row["machine_id"]].append(row)
    result = {}
    for kind, keys in ids.items():
        records = {row[kind + "_id"]: row for row in reader.rows[kind + "s"]}
        for key in keys:

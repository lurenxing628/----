...
class IteratedGreedyLimits:
    ...
    # Simulated annealing temperatures relative to the first usable primary-objective delta.
    temperature_ratio_start: float = 0.1
    temperature_ratio_end: float = 0.001
...
def resolve_iterated_greedy_limits(construction: Optional[Dict[str, Any]], *, enabled: bool) -> IteratedGreedyLimits:
    ...
    start_ratio = _unit_fraction(raw.get("temperature_ratio_start", 0.1), "temperature_ratio_start", low=0.0, high=1.0, low_open=True)
    end_ratio = _unit_fraction(raw.get("temperature_ratio_end", 0.001), "temperature_ratio_end", low=0.0, high=start_ratio, low_open=True)
...
def new_iterated_greedy_report(limits: IteratedGreedyLimits, *, objective_name: str) -> Dict[str, Any]:
    return {
        ...
        "iterations": 0, "interrupted_iterations": 0, "incumbent_adoptions": 0, "decodes": 0, "duplicate_decision_pruned": 0,
        "budget_pruned_before_decode": 0,
        ...
        "improvements": 0, "accepted": False, "parent_origin": None, "parent_score": None,

    ...
    *,
    ...
    for completed_op_id in sorted(set(execution_completed_op_ids or set())):
        ...
        for op in list(operations or []):
            ...
            if op_id == int(completed_op_id):
                continue
            batch_id = str(getattr(op, "batch_id", "") or "").strip()
            seq = int(getattr(op, "seq", 0) or 0)
            if batch_id != completed_batch_id or seq <= completed_seq:
                continue
            seed = seed_by_op_id.get(int(op_id))
            if seed is not None and isinstance(_seed_start(seed), datetime) and _seed_start(seed) < _seed_end(completed_seed):
                raise AppError(

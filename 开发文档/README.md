...
用途：统一串联测试收集、full-test-debt proof、`ruff`、架构门禁、启动链专项回归与系统速查表一致性检查。本地与托管只认这一条入口。full-test-debt proof 由 `python tools/check_full_test_debt.py` 执行，它证明当前没有未登记的 full pytest 失败，并且已登记测试债务仍受台账约束；它不代表历史测试债务已经全部修完。
...
python scripts/sync_debt_ledger.py mark-test-debt-fixed --debt-id <debt-id>
```

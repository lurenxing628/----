...
REQUEST_SERVICES_PUBLIC_ATTRS = (
    ...
    "gantt_adjustment_scenario_service",
...
class RequestServices:
    ...
    @cached_property
    def gantt_adjustment_draft_service(self) -> GanttAdjustmentDraftService:
        return self._construct(
            "gantt_adjustment_draft_service",
            lambda: GanttAdjustmentDraftService(self._db, logger=self._app_logger),
        )
    ...
    @cached_property
    def gantt_adjustment_scenario_service(self) -> GanttAdjustmentScenarioService:
        return self._construct(
            "gantt_adjustment_scenario_service",
            lambda: GanttAdjustmentScenarioService(self._db, logger=self._app_logger),
        )

    @cached_property
    def gantt_adjustment_publish_service(self) -> GanttAdjustmentPublishService:
        return self._construct(
            "gantt_adjustment_publish_service",
            lambda: GanttAdjustmentPublishService(self._db, logger=self._app_logger, op_logger=self._op_logger),
        )
    ...
    @cached_property
    def resource_dispatch_execution_service(self) -> ResourceDispatchExecutionService:
        return self._construct(
            "resource_dispatch_execution_service",
    ...
    @cached_property
    def operation_execution_feedback_service(self) -> OperationExecutionFeedbackService:
        return self._construct(
            "operation_execution_feedback_service",
            lambda: OperationExecutionFeedbackService(self._db, logger=self._app_logger, op_logger=self._op_logger),
        )

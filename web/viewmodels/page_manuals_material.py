...
MATERIAL_TOPICS: Dict[str, Dict[str, Any]] = {
    ...
    "material_batch": _topic(
        title="批次物料需求（齐套判定）",
        summary="这里维护批次对应的物料需求，并由系统更新齐套显示。",
        full_manual_anchor="#批次物料需求齐套判定",
        help_card=_card(
            "齐套判定页这样用最稳",
            "先选批次，再新增物料需求；系统会自动更新齐套显示。",
            "需求数量必须大于 0；到料数量不填会按已到齐处理，明确填 0 或不足数量才会显示未齐套。",
            "如果批次没有任何物料需求行，系统会按默认齐套、齐套日期为空显示。",
            "这里按需求数量和到料数量判断齐套；只有启用齐套检查时，齐套状态和齐套日期才会影响排产。",
            "物料必须先在“物料主数据”里存在。",
        ),
        sections=[
            _section(
                ...
                "\n".join(
                    [
                        "- 给批次挂接具体物料需求。",
                        "- 根据需求数量和到料数量，辅助判断批次是齐套、部分齐套还是未齐套。",
            ...
            _section(
                ...
                "\n".join(
                    [
                        ...
                        "- 录入需求时，需求数量必须大于 0；到料数量不填会按已到齐处理，明确填 0 或不足数量才会显示未齐套。",
                    ]
                ),
            ),
            _section(
                ...
                "\n".join(
                    [
                        "- 如果该批次根本没有需求行，系统会按默认齐套、齐套日期为空显示。",

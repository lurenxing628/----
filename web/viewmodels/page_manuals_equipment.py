...
EQUIPMENT_TOPICS: Dict[str, Dict[str, Any]] = {
    ...
    "excel_equipment": _topic(
        ...
        help_card=_card(
            ...
            "状态可填：可用 / 启用 / 正常，停用 / 禁用 / 不可用，维修 / 维护 / 维护中 / 维修中 / 保养；也兼容英文标准值 `active`/`inactive`/`maintain`。状态不能为空，不是“不填默认可用”。",
            ...
            _section(
                ...
                "\n".join(
                    [
                        ...
                        "- `状态`：必填，可填 `可用/启用/正常`、`停用/禁用/不可用`、`维修/维护/维护中/维修中/保养`；也兼容英文标准值 `active`/`inactive`/`maintain`。",
                    ]
                ),
            ),
    ...
    "excel_equipment_link": _topic(
        ...
        help_card=_card(
            ...
            "技能等级可填：初级 / 普通 / 熟练；也兼容 新手 / 一般 / 中级 / 高级 / 专家，并兼容英文标准值 `beginner`/`normal`/`expert`，不填默认普通。",
            "主操设备填 是/否、主操/非主操、主/非主；也支持 1/0，并兼容英文标准值 `yes`/`no`、`true`/`false`、`on`/`off`。同一人只能有 1 台主操设备，多了会报错。",
        ...
        sections=[
            ...
            _section(
                ...
                "\n".join(
                    [
                        "- `设备编号` 和 `工号` 都必填，且必须存在。",
                        "- `技能等级` 可填 `初级/普通/熟练`，也兼容 `新手/一般/中级/高级/专家`，并兼容英文标准值 `beginner`/`normal`/`expert`；留空默认普通。",
                        "- `主操设备` 可填 `是/否`、`主操/非主操`、`主/非主`、`1/0`，也兼容英文标准值 `yes`/`no`、`true`/`false`、`on`/`off`；同一个人只能设置 1 台主操设备。",
                        "- 适合在设备主管按设备视角维护关联时使用。",
                    ]
                ),
            ),

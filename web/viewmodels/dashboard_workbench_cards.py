...
def _resource_load_card(context: Dict[str, Any], resource_load_ratio: Optional[float]) -> Dict[str, Any]:
    if resource_load_ratio is None:
        return _risk_card(
            ...
            label="资源负荷",
            value="数据不足",
            helper_text="最新摘要里没有可安全展示的设备平均利用率。",
            severity="notice",
    ...
    *,
    ...
    return [
        ...
        _risk_card(
            ...
            helper_text="按最新排产摘要统计，同类超期只汇总显示。",
...
def build_dashboard_quick_links(context: Dict[str, Any]) -> List[Dict[str, Any]]:
    return [
        _link(context, "analysis", "排产分析"),
        _link(context, "gantt", "设备甘特图", view="machine"),
        _link(context, "resource_dispatch", "资源派工"),

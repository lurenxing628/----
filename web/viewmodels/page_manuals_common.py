...
SHARED_FRAGMENTS: Dict[str, str] = {
    ...
    "excel_check_result_meaning": "\n".join(
        [
            ...
            "- 错误：这行有必须先修的问题，例如必填项为空、编号找不到、日期写法不对、数量不是正数。只要还有错误，就不能确认写入。",
        ]
    ),
    "excel_skip_vs_error": "\n".join(
        [
    ...
    "excel_common_errors": "\n".join(
        [
            ...
            "- 必须填写的数字如果填成文字、负数或无穷大，通常会被拒绝；允许留空的数字列，会在对应页面单独说明默认值。",
    ...
    "excel_format_warning": "\n".join(
        [
            ...
            "- 不同模板里的编号列要按模板表头原样保留：工种批量表叫 `工种ID`，人员表叫 `工号`，设备表叫 `设备编号`，不要统一改成自己习惯的“编号”。",

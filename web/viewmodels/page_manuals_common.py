...
SHARED_FRAGMENTS: Dict[str, str] = {
    ...
    "excel_format_warning": "\n".join(
        [
            '- 编号/ID 类的列建议在 Excel 中设为"文本"格式再填写，否则前导零（如 001）会丢失变成 1。',
            "- 是/否类字段（如主操设备、允许普通件、允许急件）建议填 是/否；系统也兼容 1/0、true/false，主操设备还兼容 on/off。",
            "- 下载的模板和导出回填中，枚举字段优先填写中文值；系统仍兼容历史标准值。",
        ]
    ),
    "list_page_basics": "\n".join(
        [
    ...
    "scheduler_version_basics": "\n".join(
        [
            ...
            "- 不指定版本或选择 latest 时取最新历史；显式指定不存在的版本会提示缺失，不会自动切回最新。",
    ...
    "backup_risk_basics": "\n".join(
        [
            ...
            "- 恢复会覆盖现有数据；系统会在恢复前自动生成一份“恢复前自动备份”，便于回退。",
            "- 如果恢复过程失败，系统会尽最大努力自动回滚到恢复前备份，但仍建议重要操作前手动留一份备份。",

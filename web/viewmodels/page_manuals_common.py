...
SHARED_FRAGMENTS: Dict[str, str] = {
    "excel_common_flow": "\n".join(
        [
            "- 先下载模板，保持首个工作表和表头不变。",
            "- 单次上传文件大小不超过 16MB。",
            "- 上传 Excel 后先检查，确认没有错误或跳过风险，再执行正式写入。",
            "- 检查结果里还有错误时，不能确认写入。",
            "- 确认写入时如果又遇到问题，页面会提示失败原因；有的页面会整批回到导入前，有的页面会保留成功行，具体以页面提示为准。",
            "- 导入完成后建议再导出一次当前数据做复核。",
        ]
    ),
    "excel_import_modes": "\n".join(
        [
            "- 更新已有，新增缺少：编号相同的更新，不存在的新增。",
            "- 只导入新编号：已存在的数据跳过，只补新数据。",
            "- 清空本类数据后重导：先清空再导入，风险最高，使用前必须确认引用关系和备份。",
        ]
    ),
    "excel_date_format": "\n".join(
        [
    ...
    "excel_format_warning": "\n".join(
        [
            '- 编号类的列建议在 Excel 中设为"文本"格式再填写，否则前导零（如 001）会丢失变成 1。',

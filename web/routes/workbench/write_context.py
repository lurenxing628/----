...
def validate_write_context(token: str, subject_ref: str, action: str, current_snapshot: Any) -> None:
    ...
    """
    try:
        raw = resolve_public_token(_SCOPE, token, message="资料已变化，请重新读取并核对。", field="write_token")
    except ValidationError as exc:
        raise WorkbenchCommandRejected("stale_write", "资料已变化，请重新读取并核对。") from exc
    try:
        ...
        raise WorkbenchCommandRejected("stale_write", "资料已变化，请重新读取并核对。") from exc
    if not valid:
        raise WorkbenchCommandRejected("stale_write", "读取的资料与当前对象或操作不匹配，请重新读取并核对。")
    if binding.get("snapshot_hash") != input_fingerprint(current_snapshot):
        raise WorkbenchCommandRejected("stale_write", "资料已变化，请重新读取并核对。")

...
def _load_today_rows(services: Any, version: int, now: datetime) -> Tuple[List[Dict[str, Any]], str]:
    if version <= 0:
        return [], ""
    dr = _today_range(now)
    try:
...
def index():
    ...
    today_rows, today_rows_load_error = _load_today_rows(services, latest_version, now)

...
def _apply_yes_no_flags(row: Dict[str, Any]) -> Optional[str]:
    ...
    if row["允许普通件"] not in YESNO_VALUES:
        return "“允许普通件”这一列系统没认出来，可填写：是 / 否。以前的 Excel 如果写过 1 / 0 或英文，系统会尽量按中文意思读取；新文件请直接填中文。"
    row["允许急件"] = _normalize_yesno(row.get("允许急件"))
    if row["允许急件"] not in YESNO_VALUES:
        return "“允许急件”这一列系统没认出来，可填写：是 / 否。以前的 Excel 如果写过 1 / 0 或英文，系统会尽量按中文意思读取；新文件请直接填中文。"
    return None

...
python .windsurf/skills/aps-arch-audit/scripts/arch_audit.py

# 方式 2：pytest 架构适应度函数（9 条规则，PASS/FAIL 粒度）
...
4. `test_no_circular_service_dependencies` — 循环依赖检测
5. `test_no_wildcard_imports` — 禁止 import *
6. `test_no_silent_exception_swallow` — 禁止静默吞异常
7. `test_file_size_limit` — 文件大小限制
8. `test_cyclomatic_complexity_threshold` — 圈复杂度 ≤15（白名单机制）
9. `test_file_naming_snake_case` — 命名规范

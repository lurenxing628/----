...
description: Runs the Brandimarte FJSP benchmark (mk01/mk04/mk06/mk08/mk10) through the APS scheduler using tests/benchmark_fjsp.py, generates evidence/Benchmark/fjsp_benchmark_report.md, and summarizes makespan/utilization/gap. Use when the user mentions 基准测试/benchmark/FJSP/Brandimarte/makespan/排产数据集/调度数据集 or asks to run a scheduling benchmark.
---
...
- 默认（全量矩阵，推荐）：`python .windsurf/skills/aps-fjsp-benchmark/scripts/run_fjsp_benchmark.py`
- 标准（最小矩阵，更快）：`python .windsurf/skills/aps-fjsp-benchmark/scripts/run_fjsp_benchmark.py --mode standard`
- 快速 smoke（仅 mk01）：`python .windsurf/skills/aps-fjsp-benchmark/scripts/run_fjsp_benchmark.py --mode fast`
...
1. 运行 runner（默认 full 矩阵）：
   - `python .windsurf/skills/aps-fjsp-benchmark/scripts/run_fjsp_benchmark.py`
2. 读取并摘要报告：
   - `evidence/Benchmark/fjsp_benchmark_report.md`

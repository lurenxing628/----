...
description: 深度代码审查：引用链追踪、跨层边界检查、影响面评估、回归风险分析。适用于用户提到"深度review/深度审查/引用链检查/影响面分析/帮我review一下/仔细看看改了什么/排查一下这个问题"等场景。
---
...
python .windsurf/skills/aps-deep-review/scripts/reference_tracer.py

# 也可以以某个 base revision 为起点分析（常用：最近一次提交）
python .windsurf/skills/aps-deep-review/scripts/reference_tracer.py --commit HEAD~1

# 也可以分析指定文件
python .windsurf/skills/aps-deep-review/scripts/reference_tracer.py --file core/services/scheduler/batch_service.py
```
...
- 用户提到：**深度 review / 深度审查 / 引用链检查 / 帮我仔细看看 / 影响面分析 / 这个改动安全吗 / 排查一下 / 合并前检查 / 改了XXX会不会影响YYY**

...
def check_code_quality(repo_root, changed_files):
    ...
    for fpath in changed_files:
        ...
        if not fpath.endswith(".py") or not os.path.exists(abs_path):
            continue
        norm = fpath.replace("\\", "/")
        if norm.startswith("tests/") or norm.startswith(".cursor/"):
            continue
        try:
            with open(abs_path, encoding="utf-8") as f:
                content = f.read()
...
def main():
    ...
    if arch_violations:
        for v in arch_violations:
            print(f"  ✗ {v}")
    if arch_skipped:
        print(f"  ⚠ 跳过 {len(arch_skipped)} 个文件（读取失败）：")
        for s in arch_skipped[:10]:
            print(f"    - {s}")
        if len(arch_skipped) > 10:
            print(f"    ...（共 {len(arch_skipped)} 个）")
    ...
    if quality_issues:
        ...
        if len(quality_issues) > 15:
            print(f"  ...（共 {len(quality_issues)} 项）")
    if quality_info:
        print(f"  INFO：枚举集合比较 {len(quality_info)} 项（可能是输入校验，建议统一 normalize/enums）")
        for q in quality_info[:15]:
            print(f"    - {q}")
        if len(quality_info) > 15:
            print(f"    ...（共 {len(quality_info)} 项）")
    if quality_skipped:
        print(f"  ⚠ 跳过 {len(quality_skipped)} 个文件（读取失败）：")
        for s in quality_skipped[:10]:
            print(f"    - {s}")
        if len(quality_skipped) > 10:
            print(f"    ...（共 {len(quality_skipped)} 个）")
    ...
    if complexity_issues:
        ...
        if len(complexity_issues) > 10:
            print(f"  ...（共 {len(complexity_issues)} 个）")
    if complexity_skipped:
        print(f"  ⚠ 跳过 {len(complexity_skipped)} 个文件（未检查）：")
        for s in complexity_skipped[:10]:
            print(f"    - {s}")
        if len(complexity_skipped) > 10:
            print(f"    ...（共 {len(complexity_skipped)} 个）")

...
description: Runs a full self-test suite for this APS repository (smoke phases, web smoke, FullE2E, regression tests, and complex Excel cases) and produces a summary report. Use when the user asks for 全量自测/冒烟测试/smoke/E2E/回归测试/regression/复杂Excel用例，或要求“通读项目并跑全量自测”，且不需要打包。
---
...
- 运行：`python .windsurf/skills/aps-full-selftest/scripts/run_full_selftest.py`
- 可选：`python .windsurf/skills/aps-full-selftest/scripts/run_full_selftest.py --fail-fast --step-timeout 900`（每步超时秒数；<=0 表示不设超时）
...
- 推荐：直接运行 runner：
  - `python .windsurf/skills/aps-full-selftest/scripts/run_full_selftest.py`
